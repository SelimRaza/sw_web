                    ...
                    <div class="x_panel">
                        <div class="x_title">
                            @if($permission->wsmu_crat)
                                ...
                                <a class="btn btn-success btn-sm" href="{{ URL::to('/notetype/create')}}"><span
                                            class="fa fa-plus-circle" style="color: white; font-size: 1.3em"></span>&nbsp;&nbsp;<b>Add
                                        New</b></a>

                                <button class="btn btn-danger btn-sm" onclick="exportTableToCSV('note_type_<?php echo date('Y_m_d'); ?>.csv','datatables')"
                        ...
                        <div class="x_content">
                            ...
                            <table id="datatables" class="table font_color table-search">
                                <thead>
                                ...
                                <tbody>
                                @foreach($noteTypes as $index=>$data1)
                                    <tr  class="tbl_body_gray">
                                        <td class="cell_left_border ">{{$index+1}}</td>
                                        <td>{{$data1->ntpe_name}}</td>
                                        <td>{{$data1->ntpe_code}}</td>
                                        <td>
                                            @if($permission->wsmu_delt)
                                                <form style="display:inline"
                                                      action="{{route('notetype.destroy',$data1->id)}}"
