...
@section('content')
                            ...
                            <table id="datatabless" class="table search-table font_color" data-page-length='50'>
                                ...
                                <tr>
                                    ...
                                    @if($permission->wsmu_delt)
                                            <a href="#"
                                                class="btn btn-danger btn-xs" onclick="removeItem(this)" value="{{$d->id}}" dfdm_id="{{$d->mspm_id}}"><i class="fa fa-destroy"></i> Remove Item
                                            </a>
                                    @endif
                                    </td>
                                </tr>
    ...
    <script type="text/javascript">
        ...
        function removeItem(v){
            ...
            $.ajax({
                type:"GET",
                url:"{{URL::to('/')}}/msp/item-remove/"+id+"/"+dfdm_id,
                cache:"false",
                    ...
                    $('#cont').empty();
                        for(var i=0;i<data.length;i++){
                            html+='<tr><td>'+(i+1)+'</td>'+
                                    ...
                                    '<td>'+data[i].mspd_qnty+'</td>'+
                                    '<td>'+data[i].mspm_name+'</td>'+
                                    '<td>'+data[i].mspm_code+'</td>'+
                                    '<td><a href="#" class="btn btn-danger btn-xs" onclick="removeItem(this)" value="'+data[i].id+'" dfdm_id="'+data[i].mspm_id+'">Remove Item</tr>';
                        }
                    Swal.fire({
                        icon: 'success',
                        title: 'Success',
