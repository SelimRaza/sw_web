            ...
            <div class="page-title">
                <div class="title_left">
                    <ol class="breadcrumb">
                        ...
                        <li class="active">
                            <strong>All Location Company</strong>
                        </li>
                    </ol>
                </div>
                <form action="{{ URL::to('/location_company')}}" method="get">
                    ...
                    <div class="x_panel">
                        <div class="x_title">
                            @if($permission->wsmu_crat)
                                <a class="btn btn-success btn-sm" href="{{ URL::to('/location_company/create')}}"><span
                                            class="fa fa-plus-circle" style="color: white; font-size: 1.3em"></span> Add New</a>
                            @endif

                                <button class="btn btn-danger btn-sm" onclick="exportTableToCSV('Location_company_list_<?php echo date('Y_m_d'); ?>.csv','datatables')"
                            ...
                            <table id="datatables" class="table search-table font_color" data-page-length='100'>
                                ...
                                @foreach($locationData as $index => $locationData1)
                                    <tr class="tbl_body_gray">
                                        ...
                                        <td>{{$locationData1->lcmp_name}}</td>
                                        <td>{{$locationData1->lcmp_code}}</td>
                                        <td>
                                            @if($permission->wsmu_read)
                                                <a href="{{route('location_company.show',$locationData1->id)}}"
                                                   class="btn btn-primary btn-xs"><i class="fa fa-search"></i> View
                                                </a>&nbsp;|&nbsp;
                                            @endif
                                            @if($permission->wsmu_updt)
                                                <a href="{{route('location_company.edit',$locationData1->id)}}"
