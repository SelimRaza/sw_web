    ...
    <div class="right_col" role="main">
        ...
        <script type="text/javascript">
                            ...
                            items = `<tr id="item-row-${amim_id}">
                                ...
                                <td>
                                    ...
                                    <input type="number" readonly class="item-discount in_tg" style="width: 100%; text-align: end;"
                                        value="0" id="item-discount-${amim_id}" name="item_discounts[]">
                                </td>
                                <td>
                                    <input type="number" readonly class="item-subtotal in_tg" style="width: 100%; text-align: end;" id="item-subtotal-${amim_id}" name="item_prices[]">
                                </td>
            ...
            function getSubTotal(amim_id){
                ...
                calculatePromotion(amim_id, total_qty, sub_total)

                getTotal()
            }

            function calculatePromotion(item_id, order_qty, order_amnt){
                let country_id  = $('#country_id').val()
                let outlet_id   = outlets[0].id;

                if(outlet_id !='' && item_id !='' && country_id !='') {
                    $.ajax({
                        type: "POST",
                        url: "{{URL::to('/')}}/calculate-final-promotion",
                        data: {
                            country_id: country_id,
                            site_id: outlet_id,
                            amim_id: item_id,
                        ...
                        success: function (data) {
                            if(data.amount > 0){
                                $(`#item-discount-prom-${item_id}`).attr('disabled', false)
                                $(`#item-discount-prom-${item_id}`).val(data.prom_id)
                                $(`#item-discount-prom-${item_id}`).attr('name', `item_prom_ids[${item_id}]`)
                                $(`#item-discount-${item_id}`).val(data.amount)
                            }else if(data.amount === 0){
                                $(`#item-discount-prom-${item_id}`).attr('disabled', true)
                                $(`#item-discount-${item_id}`).val(0)
                            }else if(data.free_qty !== 0){
                                let item = data.promotion_item;
                                $(`#free-item-${item_id}`).remove();

                                $(`#item-discount-prom-${item_id}`).attr('disabled', false)
                                $(`#item-discount-prom-${item_id}`).attr('name', `item_prom_ids[${item_id}]`)
                                $(`#item-discount-prom-${item_id}`).val(data.prom_id)
                                let free_item = `<tr id="free-item-${item_id}" style="color: #0036fb;">
                                    <td>
                                        ${item.amim_code}-${item.amim_name}
                                        <input type="hidden" value="${item.id}" id="free-item-id-${item.id}" name="free_item_ids[]">
                                    </td>
                                    <td>
                                        <input type="hidden" id="free-item-qty-${item.id}" value="${data.prom_id}" name="free_item_prom_ids[]">
                                        <input type="number" value="${data.free_qty}" name="free_item_qtys[]" readonly
                                                class="in_tg" id="free-item-qty-${item.id}" style="width: 90%; text-align: end;">
                                    </td>
                               </tr>`

                                $(`#item-row-${item_id}`).after(free_item);
            ...
            function storeTeleOrder(){
                ...
                $.ajax({
                    type:"POST",
                    url:"{{URL::to('/')}}/tele-order-promotion-store",
                    data: order_info,
                    cache:"false",
                    success:function(data){
