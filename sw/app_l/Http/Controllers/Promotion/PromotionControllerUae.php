...
class PromotionControllerUae extends Controller
{
    ...
    // Outlet Operations in All Promotion
    public function outletInfo(){
        if ($this->userMenu->wsmu_updt) {
            return view('Promotion.outlets_sp');
        } else {
    ...
    // Search an Outlet in All Promotion
    public function promotionSearch(Request $request){
        $outlet_array = DB::connection($this->db)->select("SELECT `id` FROM `tm_site` WHERE site_code = {$request->outlet_code}");

        if(count($outlet_array) > 0){
            $outlet_id = $outlet_array[0]->id;

            return
            DB::connection($this->db)->select("SELECT t2.id, t1.prms_code, t1.prms_name, t3.site_code, t3.site_name, t1.prms_edat, t1.prmr_ctgp
                FROM tm_prmr t1
                inner join tl_prsm t2 on t1.id = t2.prmr_id
                inner join tm_site t3 on t2.site_id = t3.id
                where t2.site_id = {$outlet_id} and t1.prms_edat >= curdate()");
        }
    }


    // Outlet Delete From All Promotion
    public function outletDelete(Request $request){
        $prsm_infos =$request->input('outlet_column_ids');

        $prsm_ids = implode(',', $prsm_infos);

        try {
            DB::connection($this->db)->select("DELETE FROM tl_prsm WHERE `id` in ({$prsm_ids})");

            return response('Site Removed From Promotion', 200);
        }catch (\Exception $exception)
        {
            return response('Please Provide Valid Information', 400);
        }
    }


    // Assign Outlet To All Promotion
    public function outletAssign(Request $request){
        $outlet_code =$request->input('outlet_code');

        try {
            $outlet_array = DB::connection($this->db)->select("SELECT `id` FROM `tm_site` WHERE site_code = {$outlet_code}");

            if(count($outlet_array) > 0) {
                $outlet_id = $outlet_array[0]->id;

                DB::connection($this->db)->select("INSERT INTO `tl_prsm`( `prmr_id`, `site_id`, `cont_id`, `lfcl_id`, `aemp_iusr`, `aemp_eusr`)
                    SELECT id,{$outlet_id},{$this->cont_id},1,{$this->aemp_id},{$this->aemp_id}  FROM `tm_prmr` WHERE  id  not in (Select prmr_id from tl_prsm
                    WHERE site_id={$outlet_id}) AND prms_edat>=curdate()");

                return response('Outlet Assigned to All Promotion', 200);
            }else{
                return response('Please Provide Valid Information', 400);
            }
        }catch (\Exception $exception)
    ...
    public function promotionSP(Request $request)
    {
        if ($this->userMenu->wsmu_vsbl) {
            ...
            $acmp = DB::connection($this->db)->select("SELECT DISTINCT acmp_id as id, acmp_name, acmp_code FROM `user_group_permission` 
                                                    WHERE `aemp_id`='$empId'");
...
INNER JOIN tm_issc t2 ON t1.issc_id=t2.id WHERE t1.slgp_id='$slgp_id' ORDER BY `t2`.`issc_name` ASC");
    ...
    public function getCategoryItem(Request $request)
    {
        ...
        //             INNER JOIN tm_amim t2 ON t1.`amim_id`=t2.id inner join tm_issc t3 on t1.issc_id=t3.id WHERE t1.slgp_id='$slgp_id' AND t1.`issc_id`='$issc_id'
    ...
    public function promotionNewCreate()
    {
        if ($this->userMenu->wsmu_crat) {
            ...
            $district = DB::connection($this->db)->select("SELECT `id`, `dsct_name`, `dsct_code` FROM `tm_dsct` WHERE `lfcl_id`='1'");
            $thana = DB::connection($this->db)->select("SELECT `id`, `than_name`, `than_code` FROM `tm_than` WHERE lfcl_id='1'");
            $itsg = DB::connection($this->db)->select("SELECT `id`, `itsg_name`, `itsg_code` FROM `tm_itsg` WHERE lfcl_id='1'");
          ...
          //  $item_list=DB::connection($this->db)->select(" SELECT id,amim_code,amim_name FROM tm_amim WHERE lfcl_id=1 ORDER BY amim_code ASC;");
            $cat_list=DB::connection($this->db)->select(" SELECT id,itsg_code issc_code,itsg_name issc_name FROM tm_itsg WHERE lfcl_id=1 ORDER BY itsg_code ASC;");
    ...
    public function promotionNewStore(Request $request)
    {
        //dd($request);
            ...
            $site = "INSERT IGNORE INTO `tl_prsm`(`prmr_id`, `site_id`, `cont_id`, `lfcl_id`, `aemp_iusr`, `aemp_eusr`)
...
//                    $spds->save();
    ...
    public function getCategoryItemUAE(Request $request){
        ...
        //         ORDER BY t1.amim_code ASC");
                // $items=DB::connection($this->db)->select( "SELECT DISTINCT t2.id, t2.amim_name as item_name, t2.amim_code as item_code, t3.issc_name FROM `tl_sgit` t1 
                //                                             INNER JOIN tm_amim t2 ON t1.`amim_id`=t2.id
                //                                             inner join tm_issc t3 on t1.issc_id=t3.id WHERE t1.slgp_id='$slgp_id' AND t1.`issc_id`='{$issc_id}' AND t1.id not in (Select 
                //                                     t1.amim_id
                //                                     FROM tm_prmd t1
                //                                     INNER JOIN tm_prmr t2 ON t1.prmr_id=t2.id
                //                                     WHERE t2.lfcl_id=1 AND t2.prms_edat>=curdate()
    ...
    public function promotionByItem(Request $request){
        $item_code = $request->item_code;
        try {
            return DB::connection($this->db)->select("select t1.id, t1.prms_code, t1.prms_name, t3.amim_code, t3.amim_name from tm_prmr t1
                        inner join tm_prmd t2 on t1.id = t2.prmr_id
                        inner join tm_amim t3 on t3.id = t2.amim_id
                        where t3.amim_code = {$item_code} and t1.prms_edat>curdate() and t1.lfcl_id = 1");

            return response(['promotions' => $promotions], 200);
        }catch (\Exception $exception){
