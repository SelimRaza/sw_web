            ...
            try {
                ...
                if ($request->submit=='release') {
                    ...
                    if($orderMaster->lfcl_id==1){
                        ...
                        $cpcr_credit_count = DB::connection($this->db)->select("SELECT count(id) credit_count FROM `tl_cpcr` where site_id=$request->site_id AND sapr_amnt-scol_amnt>0.99;");
                        if($cpcr_credit_count[0]->credit_count >= 2){
                            $orderMaster->lfcl_id = 44;
                            $status = "danger";
                            $msg = " This site already credited over 2 times !!!";
                        }

                        // dd($ordd, $msg, $cpcr_credit_count[0]->credit_count,$orderMaster);
