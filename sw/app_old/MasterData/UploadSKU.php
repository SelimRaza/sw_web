...
class UploadSKU extends Model implements WithHeadings, ToArray, WithHeadingRow
{
    ...
    {
        return [
            'item_name', 'item_code', 'item_snme', 'factor', 'unit_id', 'price', 'company_id',  'category_code', 'class_code', 'status'
        ];
    }
        ...
        try {
            ...
            for($i=0;$i<count($infos);$i++) {
                ...
                if ($site != null) {
                    ...
                    $site->itsg_id   = $this->getCategoryId($infos[$i]['category_code']);
                    ...
                    $skus[] = [
                        ...
                        'itsg_id'   => $this->getCategoryId($infos[$i]['category_code']),
                        'itcl_id'   => $this->getClassId($infos[$i]['class_code']),
                        'lfcl_id'   => $infos[$i]['status'],
            ...
            if (!empty($skus)) {
                foreach (array_chunk($skus,1000) as $sku)
                {
                    DB::connection($this->connection)->table('tm_amim')->insert($sku);
                }
            }
        }catch(\Exception $e)
        {
