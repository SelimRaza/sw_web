...
class LeaveController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($this->userMenu->wsmu_vsbl) {
            $leaves = Leave::where('country_id', '=', $this->currentUser->employee()->cont_id)->orderBy('from_date', 'DESC')->get();
            return view('hr.Leave.index')->with('leaves', $leaves)->with('permission', $this->userMenu);
        } else {
    ...
    public function destroy($id)
    {
        if ($this->userMenu->wsmu_delt) {
            ...
            try {
                ...
                $leave->status_id = 5;
                $leave->updated_by = $this->currentUser->employee()->id;
                $leave->save();
...
FROM (
  ...
  INNER JOIN tblt_leave AS t2 ON t1.date BETWEEN t2.from_date AND t2.to_date
...
WHERE  t2.status_id = 5 AND t2.id = $id");
                foreach ($data1 as $request) {
                    ...
                    if ($attendanceProcess == null) {
                        ...
                        $attendanceProcess->leave_id = $request->leave_id;
                        $attendanceProcess->leave_reason = $request->leave_reason;
            ...
            /* } else {
                 return redirect()->back()->with('danger', 'Already Approved');
             }*/
