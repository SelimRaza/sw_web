...
class PrintController extends Controller
{
       ...
       // if (strpos( '3,4',"$country->id") !== false) {
        if ($country != false) {
            ...
            $orderMasterData = collect(DB::connection($country->cont_conn)->select("SELECT
                                  ...
                                  ifnull(t30.mother_site_code,t3.site_code)AS outlet_id,
                                ...
                                left join tl_site_party_mapping t30 ON t3.site_code=t30.site_code
            ...
            $orderLineData = DB::connection($country->cont_conn)->select("SELECT
                              ...
                              CASE 
                                  WHEN t2.ordd_spdc > 0 THEN 
                                      ((t2.ordd_opds + t2.ordd_spdc + t2.ordd_dfdo) * 100 / t2.ordd_oamt)
                                  ELSE 
                                      ((t2.ordd_opds + t2.ordd_spdo + t2.ordd_dfdo) * 100 / t2.ordd_oamt)
                              END AS ratio,
                              (t2.ordd_opds + 
                              CASE 
                                  WHEN t2.ordd_spdc > 0 THEN 
                                  ...
                                  ELSE 
                                      t2.ordd_spdo
                              END + t2.ordd_dfdo) AS Discount,
                              t2.ordd_duft                                                      AS ctn_size,
                              t2.prom_id                                                        AS promo_ref,
                              round(t2.ordd_texc,3)                                                      AS gst,
                              t2.ordd_ovat                                                      AS vat,
                              round(t2.ordd_tvat,3)                                             AS tvat,
                              round((t2.ordd_uprc*t2.ordd_qnty),3)-(t2.ordd_opds + t2.ordd_spdo + t2.ordd_dfdo)+
                              round((((t2.ordd_uprc*t2.ordd_qnty)-(t2.ordd_opds + t2.ordd_spdo + t2.ordd_dfdo)+(((t2.ordd_uprc*t2.ordd_qnty)*t2.ordd_excs)/100))*t2.ordd_ovat)/100,3)+
                              round(((t2.ordd_uprc*t2.ordd_qnty)*t2.ordd_excs)/100,3)AS net_amount
	...
	public function salesInvoicePrint_ou($cont_id, $order_id,$ou_id)
    {
		...
		if($cont_id==14){
			  $orderMasterData = collect(\DB::connection($country->cont_conn)->select("SELECT
                        ...
                        '0'                                                   AS discount,
                        t2.site_code                                          AS customer_number,
                        IFNULL(t10.mother_site_code,t2.site_code)             AS outlet_id,
                        t9.optp_name                                          AS Payment_Type,
                        ''                                                    AS Region_Name,
                        ''                                                    AS Zone_Name,
    ...
    public function returnPrint($cont_id, $order_id)
    {
        ...
        //if (strpos( '3,4',"$country->id") !== false) {
            ...
            $orderLineData = DB::connection($country->cont_conn)->select("SELECT
                              ...
                              t2.rtdd_excs                       AS gst,
                              t2.rtdd_ovat                       AS vat,
                              t2.`rtdd_texc`                     AS gst_amt,
                              t2.`rtdd_tvat`                     AS vat_amt
                            FROM tt_rtan AS t1
  ...
  t3.aemp_name                                          AS preseller_name,
  t3.aemp_mob1                                          AS preseller_mob,
  t1.rtan_pono                                          AS party_grv_number,
  t1.rtan_podt                                          AS party_grv_date,
  t4.id                                                 AS ou_id,
  t4.acmp_name                                          AS ou_name,
  t4.acmp_nexc                                          AS tax_number,
  t4.acmp_nvat                                          AS vat_number,
...
WHERE t1.rtan_rtnm = '$order_id'"))->first();
            ...
            $orderLineData = DB::connection($country->cont_conn)->select("SELECT
  concat(t3.id,'-',t3.amim_code)                         AS Product_id,
  ...
  t4.dprt_name                       AS reason,
  IFNULL(t6.ordd_dqty,0)             AS delivery_qty,
  IFNULL(t6.ordd_odat,0)             AS delivery_amt,
  IFNULL(t6.ordd_tvat,0)             AS total_vat_amt,
  t7.IBS_INVOICE                     AS IBS_INVOICE
FROM tt_rtan AS t1
  INNER JOIN tt_rtdd AS t2 ON t1.id = t2.rtan_id
  INNER JOIN tm_amim AS t3 ON t2.amim_id = t3.id
  JOIN tm_dprt t4 ON t2.`dprt_id`=t4.id
  LEFT JOIN tt_ordd t6 ON t2.ordm_ornm=t6.ordm_ornm and t2.amim_id=t6.amim_id
  LEFT JOIN dm_trip_master t7 ON t6.ordm_ornm=t7.ORDM_ORNM  
