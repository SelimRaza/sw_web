...
class VCCoolerReportController extends Controller
{
    private $access_key = 'VCCoolerReportController';
    private $currentUser;
    private $userMenu;
    private $db;
...
FROM tm_slgp ");
    ...
    public function mapMarkeReportShow(Request $request)
    {
        ...
        return view('report.vcCooler.vc_cooler_outlet_list')
            ->with('districts', $districts)
            ->with('thanas', $thanas);
    }

    public function jsonLoadVoutletGoogleMap(Request $request)
    {
        $district = $request->district_id;
        $thana_id = $request->thana_id;
        if ((!empty($request->thana_id))) {
            return $results = DB::connection($this->db)->select("SELECT t1.`geo_lat` as lat, t1.`geo_lon` as lng, 
 t1.`site_code`, t1.`site_name`,t1.`site_olnm`, t1.`site_adrs`,t1.`site_olad`, t1.`site_mob1`, t2.otcg_name, t3.mktm_name,
  t5.than_name, t6.dsct_name, t1.`site_ownm`, t1.`site_olon`, t1.`created_at` 
 FROM `tm_site` t1 INNER JOIN tm_mtcg t2 ON t1.`otcg_id`=t2.id 
   INNER JOIN tm_mktm t3 ON t1.mktm_id = t3.id INNER JOIN tm_ward t4 ON t3.ward_id=t4.id INNER JOIN tm_than t5 ON t4.than_id=t5.id 
   INNER JOIN tm_dsct t6 ON t5.dsct_id=t6.id WHERE t1.attr3='1' AND t5.id='$thana_id'");
 ...
 FROM `tm_site` t1 INNER JOIN tm_mtcg t2 ON t1.`otcg_id`=t2.id 
   INNER JOIN tm_mktm t3 ON t1.mktm_id = t3.id INNER JOIN tm_ward t4 ON t3.ward_id=t4.id INNER JOIN tm_than t5 ON t4.than_id=t5.id 
   INNER JOIN tm_dsct t6 ON t5.dsct_id=t6.id WHERE t1.attr3='1' AND t6.id='$district'");
        } else {
            return $results = DB::connection($this->db)->select("SELECT t1.`geo_lat` AS lat, t1.`geo_lon` AS lng, 
 t1.`site_code`, t1.`site_name`,t1.`site_olnm`, t1.`site_adrs`, t1.`site_olad`, t1.`site_mob1`, t2.otcg_name, t3.mktm_name,
  t5.than_name, t6.dsct_name, t1.`site_ownm`, t1.`site_olon`, t1.`created_at` 
 FROM `tm_site` t1 INNER JOIN tm_mtcg t2 ON t1.`otcg_id`=t2.id 
   INNER JOIN tm_mktm t3 ON t1.mktm_id = t3.id INNER JOIN tm_ward t4 ON t3.ward_id=t4.id INNER JOIN tm_than t5 ON t4.than_id=t5.id 
   INNER JOIN tm_dsct t6 ON t5.dsct_id=t6.id WHERE t1.attr3='1'");
        }
        ...
        /*if ((!empty($request->thana_id)) && (!empty($request->thana_id))) {
            ...
            return $results = DB::connection($this->db)->table('tm_msit as t1')
                ...
                ->select('t4.id as thana_id', 't5.id as distirct_id', 't1.geo_lat AS lat', 't1.geo_lon AS lng',
                    't1.site_name AS name', 't1.site_adrs AS address', 't1.site_code AS site_code')
...
ORDER BY MIN(t1.attn_time) ASC";
    ...
    public function orderReportVcCooler()
    {
        $empId = $this->currentUser->employee()->id;
        $acmp = DB::connection($this->db)->select("SELECT DISTINCT acmp_id as id, acmp_name, acmp_code FROM `user_group_permission` WHERE `aemp_id`='$empId'");
        $region = DB::connection($this->db)->select("SELECT DISTINCT `dirg_id` as id, dirg_name, dirg_code FROM `user_area_permission` WHERE `aemp_id`='$empId'");
        ...
        $results = [];
        return view('report.activity.order_report_vc_cooler')
            ->with('acmp', $acmp)
            ->with('region', $region);
    }

    public function orderReportVcCoolerFilter(Request $request)
    {
        ...
        $query = "select " . $start_date . " as ordm_date, t1.dsct_name, t1.than_name, t1.t_outlet, 
IFNULL(sum(t2.PBL),'0') as PBL, IFNULL(sum(t2.PBL_ordm_amt),'0') as PBL_amnt,
 IFNULL(sum(t2.PDL),'0') as PDL, IFNULL(sum(t2.PDL_ordm_amt),'0') as PDL_amnt, 
 IFNULL(sum(t2.AMCL),'0') as AMCL, IFNULL(sum(t2.AMCL_ordm_amt),'0') as AMCL_amnt,
  IFNULL(sum(t2.PFF),'0') as PFF,IFNULL(sum(t2.PFF_ordm_amt),'0') as PFF FROM  
(SELECT
  t5.id        AS dsct_id,
  t5.dsct_name,
  t4.id,
  t4.than_name,
  count(t1.id) AS t_outlet
FROM `tm_site` t1
...
WHERE t1.attr3 = '1'
GROUP BY t4.id) t1 LEFT JOIN 
  (SELECT
  t6.ordm_date,
  t3.than_id,
  IF(t6.acmp_id = '1', COUNT(t6.`ordm_id`), '0')  AS PBL,
  IF(t6.acmp_id = '6', COUNT(t6.`ordm_id`), '0')  AS PDL,
  IF(t6.acmp_id = '8', COUNT(t6.`ordm_id`), '0')  AS AMCL,
  IF(t6.acmp_id = '10', COUNT(t6.`ordm_id`), '0') AS PFF,
  IF(t6.acmp_id = '1', round(SUM(t6.`ordm_amt`),2), '0')  AS PBL_ordm_amt,
  IF(t6.acmp_id = '6', round(SUM(t6.`ordm_amt`),2), '0')  AS PDL_ordm_amt,
  IF(t6.acmp_id = '8', round(sum(t6.`ordm_amt`),2), '0')  AS AMCL_ordm_amt,
  IF(t6.acmp_id = '10', round(sum(t6.`ordm_amt`),2), '0') AS PFF_ordm_amt
FROM `tm_site` t1 INNER JOIN tm_mktm t2 ON t1.mktm_id = t2.id
  INNER JOIN tm_ward t3 ON t2.ward_id = t3.id
  INNER JOIN ordm_d_1_m t6 ON t1.id = t6.site_id
  INNER JOIN tm_acmp t7 ON t7.id = t6.acmp_id
WHERE t1.`attr3` = '1' AND t6.ordm_date = '$start_date'
GROUP BY t3.than_id, t6.acmp_id ) t2 ON t1.id = t2.than_id group by t1.than_name ORDER BY t1.dsct_name";
