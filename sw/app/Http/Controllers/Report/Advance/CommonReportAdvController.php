...
Use Excel;
class CommonReportAdvController extends Controller
{
    private $access_key = 'advance_report';
    private $currentUser;
    private $userMenu;
    private $db;
    private $aemp_id;
    ...
    public function index()
    {
        ...
        return view('report.Advance.common_report_adv',['acmp'=>$acmp,'region'=>$region,'dsct'=>$dsct,'dsct1'=>$dsct,'role_id'=>$user_role_id,
                    'emid'=>$empId,'zone'=>$zone,'asset'=>$asset,'depo_list'=>$depo_list]);
    }
    public function getTripData(Request $request){
        ...
        switch($reportType){
            case "trip_print":
                $trip_details=array();
                $dm_id='';
                $trip_data=DB::connection($this->db)->select("CALL getTripDataAdvDisc1(?)",[$trip_no]);
                ...
                $other_trip_collection_data=DB::connection($this->db)->select("CALL getOtherTripCollectionAdv(?,?,?)",[$trip_no,$coll_date,$dm_id]);
                $trip_result=collect($trip_data);
                $trip_other_result=collect($other_trip_collection_data);
                $gt_trip=$trip_result->pipe(function($result){
                    return collect([
                        't_ORDD_AMNT'=>round($result->sum('ORDD_AMNT'),2),
                        't_INV_AMNT'=>round($result->sum('INV_AMNT'),2),
                        't_DELV_AMNT'=>round($result->sum('DELV_AMNT'),2),
                        't_COLLECTION_AMNT'=>round($result->sum('COLLECTION_AMNT'),2),
                        't_RTAN_AMNT'=>round($result->sum('RTAN_AMNT'),2),
                        't_CRED_AMNT'=>round($result->sum('CRED_AMNT'),2),
                        't_GRV_AMNT'=>round($result->sum('GRV_AMNT'),2),
                        't_GD_GRV'=>round($result->sum('GD_GRV'),2),
                        't_BAD_GRV'=>round($result->sum('BAD_GRV'),2),
                        't_Cash'=>round($result->sum('Cash'),2),
                        't_Cheque'=>round($result->sum('Cheque'),2),
                        't_Online'=>round($result->sum('Online'),2),
                        't_DISCOUNT'=>round($result->sum('DISCOUNT'),2)
                    ]);                       
                });
                $gt_other_trip=$trip_other_result->pipe(function($result){
                    return collect([
                        't_ORDD_AMNT'=>round($result->sum('ORDD_AMNT'),2),
                        't_INV_AMNT'=>round($result->sum('INV_AMNT'),2),
                        't_DELV_AMNT'=>round($result->sum('DELV_AMNT'),2),
    ...
    public function commonReportFilter99(Request $request){
        ...
        if($module==2){
            $data=DB::connection($this->db)->select("SELECT t1.ordm_date, t5.slgp_name,
                ...
                and t1.slgp_id IN (".implode(',',$sales_group_id).") ".$zone_q."
            ...
            $data=DB::connection($this->db)->select("SELECT t1.ordm_date, t5.slgp_name,
                ...
                INNER JOIN tm_slgp t5 ON t1.slgp_id=t5.id
                WHERE t1.ordm_date BETWEEN '$start_date' AND '$end_date' 
                and t1.slgp_id (".implode(',',$sales_group_id).") ".$zone_q."
...
public function getAccountsData(Request $request){
    $year_mnth=$request->year_mnth;
    $reportType=$request->reportType;
    $year = date('Y', strtotime($year_mnth));
    $mnth = date('m', strtotime($year_mnth));
    $budget_details_sum=$request->budget_details_sum;
    $data=array();
    switch($reportType){
        case "special_budget":
            if($budget_details_sum==1){
                $data=DB::connection($this->db)->select("SELECT 
                   t1.id spbm_id, CONCAT(t1.spbm_year,'-',t1.spbm_mnth)budget_period,t2.aemp_usnm,t2.aemp_name,t1.spbm_limt,round(t1.spbm_avil,2)spbm_avil,round(t1.spbm_amnt,2)spbm_amnt
                    FROM tt_spbm t1
                    INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                    WHERE t1.spbm_year={$year} AND t1.spbm_mnth={$mnth} ORDER BY t2.aemp_usnm,t2.aemp_name");
            }
            else{
                $data=DB::connection($this->db)->select("SELECT 
                        t4.ordm_date,t9.aemp_usnm apr_id,t9.aemp_name apr_name,t7.aemp_usnm sr_id,t7.aemp_name sr_name,t8.zone_name,
                        t5.ordd_oamt,t5.ordd_inty,
                        t3.ordm_ornm,t6.amim_code,t6.amim_name,ROUND(t5.ordd_spdi,2)ordd_spdi,ROUND(t5.ordd_spdc,2)ordd_spdc
                        FROM tt_spbm t1
                        INNER JOIN tt_spbd t3 ON t1.id=t3.spbm_id
                        INNER JOIN tt_ordm t4 ON t3.ordm_ornm=t4.ordm_ornm
                        INNER JOIN tt_ordd t5 ON t4.id=t5.ordm_id
                        INNER JOIN tm_amim t6 ON t5.amim_id=t6.id
                        INNER JOIN tm_aemp t7 ON t4.aemp_id=t7.id
                        INNER JOIN tm_zone t8 ON t7.zone_id=t8.id
                        INNER JOIN tm_aemp t9 ON t1.aemp_id=t9.id
                        WHERE t1.spbm_year={$year} AND t1.spbm_mnth={$mnth} AND t3.spbd_type ='Approved' AND t3.spbd_amnt>0 AND t5.ordd_spdc>0
                        GROUP BY t9.aemp_usnm,t9.aemp_name,t7.aemp_usnm,t7.aemp_name,t8.zone_name,t3.ordm_ornm,t6.amim_code,t6.amim_name
                        ORDER BY t4.ordm_date, t9.aemp_usnm,t7.aemp_usnm;");
            }
        ...
        case "credit_budget":
            if($budget_details_sum==1){
                $data=DB::connection($this->db)->select("SELECT 
                        t1.id,t3.zone_name,t2.aemp_usnm,t2.aemp_name,ROUND(t1.spbm_limt,2)spbm_limt,
                        ROUND(t1.spbm_avil,2)spbm_avil,ROUND(t1.spbm_amnt,2)spbm_amnt
                        FROM tm_scbm t1
                        INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                ...
                $data=DB::connection($this->db)->select("SELECT 
                        t4.ORDM_ORNM,t4.ORDM_DATE,t4.ORDM_DRDT,t6.aemp_usnm SR_ID,t6.aemp_name SR_NAME,
                        t5.site_code SITE_CODE,t5.site_name SITE_NAME,ROUND(t3.sapr_amnt,2) SAPR_AMNT,
                        t4.DM_CODE,t4.TRIP_NO,t7.aemp_usnm APR_ID,t7.aemp_name APR_NAME
                        FROM tm_scbm t1
                        INNER JOIN tl_cpcr t3 ON t1.id=t3.spbm_id
                        INNER JOIN dm_trip_master t4 ON t3.ordm_ornm=t4.ORDM_ORNM
                        INNER JOIN tm_site t5 ON t4.SITE_ID=t5.id
                        INNER JOIN tm_aemp t6 ON t4.AEMP_ID=t6.id
                        INNER JOIN tm_aemp t7 ON t1.aemp_id=t7.id
                        WHERE  t3.trnt_id=4 AND t3.sapr_amnt !=t3.scol_amnt
                        ORDER BY t4.ORDM_DATE,t5.site_code;");
            }

            break;
        case "tgt_achmnt":
            if($budget_details_sum==1){
                $sdat=$request->chq_sdat;
                $edat=$request->chq_edat;
                $data=DB::connection($this->db)->select("SELECT t7.slgp_name GROUP_NAME,t6.zone_code ZONE_CODE,
                        CONCAT(t1.trgt_year,'-','0',t1.trgt_mnth) TARGET_PERIOD, t5.aemp_usnm SV_ID,
                        t5.aemp_name SV_NAME, t4.aemp_usnm SR_ID,t4.aemp_name SR_NAME,
                        ROUND(sum(t1.trgt_tamt),2) TARGET_AMNT,
                        ROUND(sum(t2.ordr_amnt),2) ORDER_AMNT,
                        ROUND(sum(t2.deli_amnt),2) DELI_AMNT
                        FROM tt_trgt t1 
                        LEFT JOIN 
                        ( 
                            SELECT aemp_id,amim_id,order_qnty,ordr_amnt,deli_amnt,deli_qnty FROM
                            (SELECT t1.aemp_id,t2.amim_id,sum(ordd_inty) order_qnty,t2.ordd_duft ,round(sum(t2.ordd_oamt),2)ordr_amnt,
                                    round(sum(t2.ordd_odat),2) deli_amnt,
                                    sum(t2.ordd_dqty) deli_qnty
                            FROM tt_ordm t1 
                            INNER JOIN tt_ordd t2 ON t1.id=t2.ordm_id
                            WHERE t1.ordm_date between '{$year_mnth}-01' AND '{$year_mnth}-31' AND t1.lfcl_id not in (15,16,21) GROUP BY t1.aemp_id,t2.amim_id
                            )
                            t1 GROUP BY aemp_id,amim_id
                        ) t2 ON t1.aemp_susr=t2.aemp_id AND t1.amim_id=t2.amim_id 
                            INNER JOIN tm_amim t3 ON t1.amim_id=t3.id
                            INNER JOIN tm_aemp t4 ON t1.aemp_susr=t4.id
                            INNER JOIN tm_aemp t5 ON t1.aemp_vusr=t5.id
                            INNER JOIN tm_zone t6 ON t4.zone_id=t6.id
                            INNER JOIN tm_slgp t7 ON t4.slgp_id=t7.id
                            INNER JOIN tm_itsg t8 ON t3.itsg_id=t8.id
                            INNER JOIN tm_itcg t9 ON t8.itcg_id=t9.id
                            INNER JOIN tm_itcl t10 ON t3.itcl_id=t10.id
                            WHERE t1.trgt_year={$year} AND t1.trgt_mnth={$mnth} 
                            GROUP BY GROUP_NAME, ZONE_CODE, TARGET_PERIOD, SV_ID, SV_NAME, SR_ID, SR_NAME;");

            }else{
                $data=DB::connection($this->db)->select("SELECT t7.slgp_name GROUP_NAME,t6.zone_code ZONE_CODE,
                        CONCAT(t1.trgt_year,'-','0',t1.trgt_mnth) TARGET_PERIOD, t5.aemp_usnm SV_ID,
                        t5.aemp_name SV_NAME, t4.aemp_usnm SR_ID,t4.aemp_name SR_NAME,
                        t10.itcl_name CLASS_NAME,t9.itcg_name CATEGORY_NAME,
                        t8.itsg_name SUBCAT_NAME,
                        t3.amim_code ITEM_CODE,
                        t3.amim_name ITEM_NAME,
                        ROUND(t1.trgt_rqty/t3.amim_duft,2) TARGET_CTN,
                        ROUND(t1.trgt_tamt,2) TARGET_AMNT,
                        IFNULL(ROUND(t2.order_qnty/t3.amim_duft,2),0) ORDER_CTN ,
                        IFNULL(t2.ordr_amnt,0) ORDER_AMNT,
                        IFNULL(ROUND(t2.deli_qnty/t3.amim_duft,2),0) DELI_CTN ,
                        IFNULL(t2.deli_amnt,0) DELI_AMNT
                        FROM tt_trgt t1 
                        LEFT JOIN 
                        ( 
                            SELECT aemp_id,amim_id,order_qnty,ordr_amnt,deli_amnt,deli_qnty FROM
        ...
        case "chq_collection":
            ...
            $edat=$request->chq_edat;
            $data=DB::connection($this->db)->select("SELECT 
                t1.COLL_DATE,t1.COLL_NUMBER,t3.aemp_usnm DM_ID,t3.aemp_name DM_NAME,t2.site_code SITE_CODE,t2.site_name SITE_NAME,t1.COLLECTION_AMNT,
                IF(t1.COLL_REC_HO>0,'Received By HO','Not Received')COLL_REC_HO,
                t1.COLLECTION_TYPE,
                t1.CHECK_NUMBER,t1.BANK_NAME,t4.lfcl_name
                FROM `dm_collection` t1
                INNER JOIN tm_site t2 ON t1.SITE_ID=t2.id
                INNER JOIN tm_aemp t3 ON t1.DM_CODE=t3.aemp_usnm
                INNER JOIN tm_lfcl t4 ON t1.STATUS=t4.id
                WHERE  COLLECTION_TYPE='Cheque' AND COLL_DATE BETWEEN '$sdat' AND '$edat';");
            return $data;
...
public function getSpecialBudgetDetailsData($id){
    $user_info=DB::connection($this->db)->select("SELECT 
                t2.aemp_usnm,t2.aemp_name,t3.zone_name
                FROM tt_spbm t1
                INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                INNER JOIN tm_zone t3 ON t2.zone_id=t3.id
                WHERE t1.id={$id}");
    $data=DB::connection($this->db)->select("SELECT 
            t7.aemp_usnm,t7.aemp_name,t8.zone_name,t3.ordm_ornm,t6.amim_code,t6.amim_name,ROUND(t5.ordd_spdi,2)ordd_spdi,ROUND(t5.ordd_spdc,2)ordd_spdc
            FROM tt_spbm t1
            INNER JOIN tt_spbd t3 ON t1.id=t3.spbm_id
            INNER JOIN tt_ordm t4 ON t3.ordm_ornm=t4.ordm_ornm
            INNER JOIN tt_ordd t5 ON t4.id=t5.ordm_id
            ...
            INNER JOIN tm_aemp t7 ON t4.aemp_id=t7.id
            INNER JOIN tm_zone t8 ON t7.zone_id=t8.id
            WHERE  t1.id={$id} AND t3.spbd_type ='Approved' AND t3.spbd_amnt>0 AND t5.ordd_spdc>0
            GROUP BY t3.ordm_ornm,t6.amim_code,t6.amim_name,t5.ordd_spdi,t5.ordd_spdc;");
    return view('report.Advance.special_budget_details')->with("data", $data)->with('user_info',$user_info); 
}
public function getCreditBudgetDetailsData($id){
    $user_info=DB::connection($this->db)->select("SELECT 
                t2.aemp_usnm,t2.aemp_name,t3.zone_name
                FROM tm_scbm t1
                INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
    ...
    $data=DB::connection($this->db)->select("SELECT 
            t4.ORDM_ORNM,t4.ORDM_DATE,t4.ORDM_DRDT,t6.aemp_usnm SR_ID,t6.aemp_name SR_NAME,
            t5.site_code SITE_CODE,t5.site_name SITE_NAME,ROUND(t3.sapr_amnt,2) SAPR_AMNT,
            t4.DM_CODE,t4.TRIP_NO
            FROM tm_scbm t1
            INNER JOIN tl_cpcr t3 ON t1.id=t3.spbm_id
            INNER JOIN dm_trip_master t4 ON t3.ordm_ornm=t4.ORDM_ORNM
            INNER JOIN tm_site t5 ON t4.SITE_ID=t5.id
            INNER JOIN tm_aemp t6 ON t4.AEMP_ID=t6.id
            WHERE t1.id={$id} AND t3.trnt_id=4 AND t3.sapr_amnt !=t3.scol_amnt
            ORDER BY t4.ORDM_DATE,t5.site_code;");
    return view('report.Advance.credit_budget_details')->with("data", $data)->with('user_info',$user_info); 
}
public function getSurveyImage($id){
    $data=DB::connection($this->db)->select("SELECT svmg_imge nimg_imag FROM tl_svmg WHERE itsv_id=$id ORDER BY id ASC");
    return $data;
}
...
//All report section start
    public function getSummaryData(Request $request)
    {
      ...
      //  $slgp=DB::connection($this->db)->select("Select acmp_name,slgp_name from user_group_permission where acmp_id=$acmp_id and slgp_id IN (".implode(',',$dirg_id).")$sales_group_id limit 1");
     ...
     //   $oid_min_max=DB::connection($this->db)->select("Select min(id) as min_id,max(id) as max_id FROM tt_ordm where ordm_date BETWEEN '$start_date' AND '$end_date'");
        ...
        // return [
        //     'company acmp_id'=> $acmp_id,
        //     'sales_group_id'=> $sales_group_id,
        //     'Region/Zone dirg_id'=> $dirg_id,
        //     'zone id' => $zone_id,
        //     'zone_list' => $zone_list,
        ...
        switch($reportType){
            ...
            case "sr_activity_hourly_order":
                        if($time_period==0){
                            ...
                            $q9_s="";
                            if($dirg_id !=""){
                                $q_z=" AND t2.zone_id IN (".implode(',',$zone_list).")";
                            }
                            if ($zone_id != "") {
                                $q_z= " AND t2.zone_id IN (".implode(',',$zone_id).")";
                            }
                            if ($sales_group_id != "") {
                                $q2_g= " AND t1.slgp_id IN (".implode(',',$sales_group_id).")";
                            }
                        ...
                        return $final;
                            ...
                            $q2_g="";
                            if($dirg_id !=""){
                                $q_z=" AND t1.zone_id IN (".implode(',',$zone_list).")";
                            }
                            if ($zone_id != "") {
                                $q_z= " AND t1.zone_id IN (".implode(',',$zone_id).")";
                            }
                            if ($sales_group_id != "") {
                                $q2_g= " AND t1.slgp_id IN (".implode(',',$sales_group_id).")";
                            }
                            ...
                            $data=DB::connection($this->db)->select("SELECT act_date,slgp_name,concat(t2.zone_code,'-',t2.zone_name)zone_name,aemp_usnm,replace(aemp_name,',','.')aemp_name,aemp_mob1,9am,
                                ...
                                INNER JOIN tm_zone t2 ON t1.zone_id=t2.id
                                WHERE act_date BETWEEN '$start_date' AND '$end_date'".$q_z.$q2_g."
            ...
            case "sr_hourly_activity":
                ...
                $data=DB::connection($this->db)->select("SELECT  `act_date`, `acmp_id`, `slgp_id`, `zone_id`, `acmp_name`, `slgp_name`,
                        ...
                        ROUND(`9exp_a`,2)9exp_a,
                        ROUND(`10exp_a`,2)10exp_a, 
                        ROUND(`11exp_a`,2)11exp_a, 
                        ROUND(`12exp_p`,2)12exp_p, 
                        ROUND(`1exp_p`,2)1exp_p,
                        ROUND(`2exp_p`,2)2exp_p,
                        ROUND(`3exp_p`,2)3exp_p, 
                        ROUND(`4exp_p`,2)4exp_p, 
                        ROUND(`5exp_p`,2)5exp_p,
                        ROUND(`6exp_p`,2)6exp_p, 
                        ROUND(`7exp_p`,2)7exp_p, 
                        ROUND((`8exp_p`+`9exp_p`),2)8exp_p, 
                        ROUND(`t_exp`,2)t_exp,
            ...
            case "assign_task_stat":
                ...
                $data=DB::connection($this->db)->select("SELECT  
                        CONCAT(t2.aemp_usnm,'-',t2.aemp_name) OWNER_NAME,CONCAT(t3.aemp_usnm,'-',t3.aemp_name) ASSIGNED_NAME,
                        t4.note_body NOTE_BODY,t4.note_date NOTE_DATE,
                        IF(t1.nasn_life=0,'Pending','Completed') NOTE_STAT,
                        t1.nasn_comn NOTE_REMARK,
                        t5.ntpe_name NOTE_TYPE,
                        t1.enmp_date MAPPING_DATE
                        FROM `tl_nasn` t1
                        INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                        INNER JOIN tm_aemp t3 ON t1.asin_user =t3.id
                        INNER JOIN tt_note t4 ON t1.note_id=t4.id
                        INNER JOIN tm_ntpe t5 ON t4.ntpe_id=t5.id
                        WHERE t1.enmp_date bETWEEN '$start_date' AND '$end_date' " . $q1.$q2."
                        ");
                return array('data'=>$data,'start_date'=>$start_date,'end_date'=>$end_date);
                break;
            case "zone_summary":
                ...
                break;
            case "sr_item_survey":
                $data=DB::connection($this->db)->select("SELECT t1.`id`,t2.`site_code`,t2.site_name, `item_name`, `country_name`, `acmp_name`, `item_weit`, `item_tppr`,
                        `item_mrpp`, `mnfc_date`, `expr_date`, `prom_desc`,date(t1.`created_at`) sv_date, t3.aemp_usnm sr_id,t3.aemp_name sr_name 
                        FROM `tm_itsv` t1 
                        INNER JOIN tm_site t2 ON t1.site_id=t2.id INNER JOIN tm_aemp t3 ON t1.aemp_iusr=t3.id WHERE date(t1.created_at) BETWEEN '$start_date' AND '$end_date' ORDER BY t1.id ");
                return $data;


                break;
            case "sr_indv_summary":
                $q1='';
                $q2='';
                if($dirg_id !=""){
                    $q1=" AND t4.zone_id IN (".implode(',',$zone_list).")";
                ...
                $data=DB::connection($this->db)->select("SELECT 
                        t5.aemp_usnm,t5.aemp_name,t5.aemp_mob1,t1.*,t2.target,t3.t_visit,t3.all_visit,t4.rpln_site,round(t1.t_sku/t1.t_memo,2)lpc,
                        t6.slgp_name,t7.zone_name,t7.zone_code,round(t1.t_memo*100/t3.all_visit,2) strikeRate,t3.work_days,t8.attn_days,
                        round(t_memo/t8.attn_days,2)avg_memo,if(round(100*t1.act_deli/ifnull(target,1),2)>100,100,round(100*t1.act_deli/ifnull(target,1),2))achv_prcnt,
                        t9.aemp_usnm sv_id,t9.aemp_name sv_name
                        FROM 
                        (SELECT 
                        t1.*,t2.DELV_AMNT act_deli
                        FROM 
                        (SELECT 
                        t1.aemp_id,COUNT(DISTINCT t1.site_id)s_olt,COUNT(DISTINCT t1.id)t_memo,ROUND(SUM(t1.ordm_amnt),2)order_amnt,ROUND(SUM(t3.INV_AMNT),2)inv_amnt,ROUND(SUM(t3.DELV_AMNT),2)delv_amnt,
                        ROUND(SUM(t3.COLLECTION_AMNT),2) coll_amnt,sum(t1.ordm_icnt)t_sku
                        FROM tt_ordm t1
                        LEFT JOIN dm_trip_master t3 ON t1.ordm_ornm=t3.ORDM_ORNM
                        WHERE t1.ordm_date between '$start_date' AND '$end_date'
                        GROUP BY t1.aemp_id)t1
                        LEFT JOIN (
                        SELECT AEMP_ID,ROUND(SUM(DELV_AMNT),2)DELV_AMNT FROM dm_trip_master 
                        WHERE ORDM_DRDT BETWEEN '$start_date' AND '$end_date' GROUP BY AEMP_ID)t2 ON t1.aemp_id=t2.AEMP_ID)t1
                        LEFT JOIN 
                        (SELECT aemp_susr,round(sum(trgt_tamt),2)target  FROM tt_trgt  WHERE trgt_year=YEAR('$start_date') AND trgt_mnth=MONTH('$start_date') GROUP BY aemp_susr)t2 ON t1.aemp_id=t2.aemp_susr
                        INNER JOIN 
                        (SELECT aemp_id,count(distinct site_id)t_visit,count(site_id)all_visit,count(distinct ssvh_date)work_days FROM th_ssvh WHERE ssvh_date BETWEEN  '$start_date' AND '$end_date' AND ssvh_ispd<2 GROUP BY aemp_id)t3 ON t1.aemp_id=t3.aemp_id
                        INNER JOIN 
                        (SELECT aemp_id,count(distinct t2.site_id)rpln_site  FROM tl_rpln t1 INNER JOIN tl_rsmp t2 ON t1.rout_id=t2.rout_id GROUP BY aemp_id)t4 ON t1.aemp_id=t4.aemp_id
                        ...
                        LEFT JOIN (Select aemp_id,count(distinct attn_date)attn_days FROM tt_attn WHERE attn_date BETWEEN '$start_date' AND '$end_date' GROUP BY aemp_id)t8 ON t1.aemp_id=t8.aemp_id
                        INNER JOIN tm_aemp t9 ON t5.aemp_mngr=t9.id
                        WHERE t5.role_id=1
                        ORDER BY t1.act_deli DESC");
                return $data;
                break;
            case "sr_productivity":
                ...
                if($rtype==3){
                    ...
                    $data =DB::connection($this->db)->select("SELECT
                            ...
                            COUNT(DISTINCT t1.aemp_code)                        AS aemp_id,
                            t8.t_sr,
                            ''                                		            AS aemp_name,
                            ''                                           		AS aemp_mobile,
                            ...
                            WHERE t1.date >= '$start_date' AND t1.date <= '$end_date'  AND t2.aemp_id = $empId AND
                                t3.aemp_id =$empId  and t2.acmp_id=$acmp_id AND t1.s_outet>0 " .$q1.$q2.  "
                            GROUP BY  t7.aemp_usnm,t7.aemp_name
                            ORDER BY t3.zone_code,t7.aemp_usnm,t7.aemp_name ASC;");
                    $result=collect($data);
                    $summary = $result->pipe(function ($result) {
                        $t_olt=$result->sum('t_outlet')>0?$result->sum('t_outlet'):1;
                        return collect([
                            'total_sr'=>$result->sum('t_sr'),
                            'total_psr'=>$result->sum('aemp_id'),
            ...
            case "sr_wise_order_details":
                ...
                $quer = "SELECT 
                        t7.zone_name,t6.aemp_usnm sv_id,t6.aemp_name sv_name,t2.ordm_date, t5.aemp_usnm,t5.aemp_name, t4.site_code, t4.site_name,
                        t3.amim_code,t3.amim_name,t8.itsg_name, t1.ordd_qnty, t1.ordd_oamt,t2.ordm_time,t3.amim_duft
                        FROM tt_ordm t2
            ...
            case "sr_wise_item_summary_quatar":
                ...
                if($module==2){
                    $data=DB::connection($this->db)->select("SELECT t1.ordm_date, t5.slgp_name,
                        ...
                        concat(t3.amim_code, ' - ',t3.amim_name) as item_name, t3.amim_duft,
                        (sum(t2.ordd_qnty)%(t3.amim_duft)) 'pics',
                        FLOOR(sum(t2.ordd_qnty)/(t3.amim_duft)) as order_qnty,
                        round(sum(t2.ordd_opds + t2.ordd_spdi + t2.ordd_dfdo),2) as discount,
                        round(SUM(t2.ordd_oamt),2) as order_amnt FROM `tt_ordm` t1 
                        INNER JOIN tt_ordd t2 ON t1.id=t2.ordm_id 
                        INNER JOIN tm_amim t3 ON t2.amim_id=t3.id 
            ...
            case "sr_summary_by_group":
                ...
                GROUP BY  t1.slgp_id) t4 ON t1.slgp_id = t4.slgp_id GROUP BY t1.date,
                t1.slgp_name");
                return $data;
                break;
            case "market_outlet_sr_outlet":
                $data=DB::connection($this->db)->select("SELECT t1.dsct_name,t1.than_name,t1.ward_name,
                        ...
                        LEFT JOIN(SELECT mktm_id,slgp_out FROM tbl_mktm_slgp_wise_outlet WHERE slgp_id IN (".implode(',',$sales_group_id).") )  t2 
            ...
            case 'outlet_coverage':
                ...
                $data=DB::connection($this->db)->select("SELECT t2.slgp_name SALES_GROUP,t1.dsct_name DISTRICT_NAME,t1.than_name THANA_NAME,
            ...
            case "item_coverage":
                ...
                $data=DB::connection($this->db)->select("SELECT 
                        t3.slgp_name,t2.aemp_usnm,t2.aemp_name,COUNT(DISTINCT amim_id) cov_item,round(sum(t1.order_amnt),2)order_amnt
                        FROM `tbl_itm_cov_details` t1
                        INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                        INNER JOIN tm_slgp t3 ON t1.slgp_id=t3.id
                        WHERE  t1.ordm_date between '$start_date' AND '$end_date' " .$zone_q."
            ...
            case "item_summary":
                ...
                $data=DB::connection($this->db)->select("Select 
                        ...
                        ROUND(sum(order_amnt),2)exp,sum(order_qnty)qnty,sum(t1.attr1) count_site
                        FROM
            ...
            case "item_summary_depo_wise":
                ...
                $data=DB::connection($this->db)->select("SELECT 
                        t5.id,t5.amim_code,t5.amim_name,t4.dlrm_code,t4.dlrm_name,t6.itsg_name,t6.itsg_code,t7.itcl_name,t7.itcl_code,ROUND(SUM(t2.ordd_oamt),2) order_amnt,
                        (SUM(t2.ordd_inty)%t5.amim_duft) order_pics,SUBSTRING_INDEX(SUM(t2.ordd_inty)/t5.amim_duft,'.',1) order_ctn,
                        ROUND(SUM(t2.ordd_odat)/1000,2)deli_amnt,t5.amim_duft,SUM(t2.ordd_inty)total_quantity
                        FROM tt_ordm t1
                        INNER JOIN tt_ordd t2 ON t1.id=t2.ordm_id
                        ...
                        GROUP BY t5.id,t5.amim_code,t5.amim_name,t4.dlrm_code,t4.dlrm_name,t6.itsg_name,t6.itsg_code,t7.itcl_name,t7.itcl_code,t5.amim_duft
            ...
            case "weekly_outlet_summary":
                ...
                $sr_id_query='';
                if($sales_group_id){
                    $filter=" AND slgp_id IN (".implode(',',$sales_group_id).")";
                }
                if($sr_id){
                    $sr_id_query .=" AND aemp_id IN (".implode(',',$sr_id).")";
                }
                if($dirg_id !=""){
                    $q1=" AND t3.zone_id IN (".implode(',',$zone_list).")";
                }          
                ...
                for($i=0;$i<count($all_week);$i++){
                    ...
                    $select_q.=",ROUND(sum(if(weeks=$week,order_amnt,0)),2) `amnt$week`,sum(if(weeks=$week,visit,0)) `v$week`,sum(if(weeks=$week,memo,0)) `m$week`";
                }
                ...
                if(!$sr_id){
                    $data=DB::connection($this->db)->select("               
                        ...
                        ROUND(SUM(ordm_amnt),2) order_amnt,
                        count(site_id) visit,
                    ...
                    //     ");
                    $data=DB::connection($this->db)->select("SELECT t4.aemp_usnm,t4.aemp_name,t3.site_code,t3.site_name,t1.site_id " . $select_q ." ,IFNULL(ROUND(sum(order_amnt),2),0)t_amnt,IFNULL(SUM(memo),0)t_memo,IFNULL(SUM(visit),0)t_visit
                        FROM 
                        (
                        SELECT 
                        aemp_id,site_id
                        ...
                        SUM(IF(ordm_amnt>0,1,0)) memo
                        FROM tbl_olt_cov_details t1
                        WHERE  DATE_FORMAT(ordm_date, '%Y-%m') ='$period' " . $sr_id_query. "
                        GROUP BY aemp_id,site_id,weeks
                        ORDER BY site_id) t2 ON t1.aemp_id=t2.aemp_id AND t1.site_id=t2.site_id
                        INNER JOIN tm_site t3 ON t1.site_id=t3.id 
                        INNER JOIN tm_aemp t4 ON t1.aemp_id=t4.id
            ...
            case "outlet_dues_report":
                ...
                $q1='';
                if($sales_group_id){
                    $filter=" AND t2.slgp_id IN (".implode(',',$sales_group_id).")";
                }
                if($dirg_id !=""){
                    $q1=" AND t4.zone_id IN (".implode(',',$zone_list).")";
                }          
                ...
                $data=DB::connection($this->db)->select("SELECT
                        t9.slgp_name,t8.zone_code,t8.zone_name,
                        t3.dlrm_name,t4.aemp_usnm sr_id,t4.aemp_name sr_name,
                        t5.aemp_usnm sv_id,t5.aemp_name sv_name,
                        IF(t1.aemp_eusr !=t1.aemp_iusr,t6.aemp_usnm,'')apr_id,
                        IF(t1.aemp_eusr !=t1.aemp_iusr,t6.aemp_name,'')apr_name,
                        t7.site_code,t7.site_name,t1.sreq_amnt,t1.sapr_amnt,t1.scol_amnt,
                        t1.cpcr_cdat,t2.ordm_date,t1.ordm_amnt,t1.ordm_ornm
                        FROM `tl_cpcr` t1
                        INNER JOIN tt_ordm t2 ON t1.ordm_ornm=t2.ordm_ornm
                        INNER JOIN tm_dlrm t3 ON t2.dlrm_id=t3.id
                        INNER JOIN tm_aemp t4 ON t2.aemp_id=t4.id
                        INNER JOIN tm_aemp t5 ON t4.aemp_mngr=t5.id
                        INNER JOIN tm_aemp t6 ON t1.aemp_eusr=t6.id
                        INNER JOIN tm_site t7 ON t2.site_id=t7.id
                        INNER JOIN tm_zone t8 ON t4.zone_id=t8.id
                        INNER JOIN tm_slgp t9 ON t2.slgp_id=t9.id
                        WHERE t1.trnt_id=4 AND  t2.ordm_date between '$start_date' AND '$end_date' ". $q1.$filter.   " ;");
                return $data;
                break;
            ...
            case "outlet_dues_report_credit":
                ...
                if(Auth::user()->country()->id == 15){
                    $data=DB::connection($this->db)->select("SELECT t5.zone_code,t5.zone_name,t4.aemp_usnm sv_id,t4.aemp_name sv_name,t3.aemp_usnm sr_id,t3.aemp_name sr_name,t7.site_code,t7.site_name,
                        t6.mother_site_code,t6.mother_site_name,
                        t2.ORDM_DRDT,
                        t2.ORDM_ORNM,ROUND(t2.ORDD_AMNT,2)ORDD_AMNT,ROUND(t2.INV_AMNT,2)INV_AMNT,ROUND(t2.DELV_AMNT,2)DELV_AMNT,ROUND(t2.COLLECTION_AMNT,2)COLLECTION_AMNT,ROUND(t2.DELV_AMNT-t2.COLLECTION_AMNT,2)DUES_AMNT,
                        t1.stcm_days,
                        ...
                        INNER JOIN tm_site t7 ON t2.SITE_ID=t7.id
                        where t2.DELV_AMNT-t2.COLLECTION_AMNT>.9 ". $query. $q ."
            ...
            case "outlet_crq_history":
                ...
                $data=DB::connection($this->db)->select("SELECT 
                    ...
                    -- ELSE IF(t1.sapr_amnt>0,'Due','') 
                        ...
                        LEFT JOIN dm_collection t9 ON t9.COLL_NUMBER=t8.MAP_ID
                        INNER JOIN user_group_permission t6 ON t5.slgp_id=t6.slgp_id
                        WHERE t6.aemp_id=$aemp_id AND t7.aemp_id=$aemp_id AND date(t1.created_at) BETWEEN '$start_date' AND '$end_date' ".$query. $q ."
                        GROUP BY t5.ORDM_DATE,t6.slgp_name,t1.ordm_ornm,t2.site_code,t2.site_name,t1.ordm_amnt,
                        t13.dirg_code,t13.dirg_name,t11.aemp_usnm,t11.aemp_name,
                        t5.TRIP_NO,t1.sreq_amnt,t1.scol_amnt,t1.sapr_amnt,t1.spbd_type,t3.aemp_usnm ,t3.aemp_name,t1.cpcr_cdat,t7.zone_name,t5.COLLECTION_AMNT,t5.INV_AMNT,t5.DELV_AMNT;");
                return $data;
            ...
            case "fms_dues":
                ...
                if($dirg_id !=""){
                    $q=" AND t3.id IN (".implode(',',$zone_list).")";
                }          
                if ($zone_id != "") {
                    $q = " AND t3.id IN (".implode(',',$zone_id).") ";
                }
               
                $data=DB::connection($this->db)->select("
                        SELECT COLL_NUMBER,t2.aemp_usnm,t2.aemp_name,t1.DM_CODE,
                        t4.slgp_name,t3.zone_code,t3.zone_name,t5.site_code,t5.site_name,
                        t1.COLLECTION_AMNT,t1.COLL_REC_HO,ROUND(t1.COLLECTION_AMNT-t1.COLL_REC_HO,2)dues_amnt 
                        FROM `dm_collection` t1 
                        INNER JOIN tm_aemp t2 ON t1.AEMP_ID=t2.id 
                        INNER JOIN tm_zone t3 ON t2.zone_id=t3.id 
                        INNER JOIN tm_slgp t4 ON t1.slgp_id=t4.id 
                        INNER JOIN tm_site t5 ON t1.SITE_ID=t5.id 
                        where COLL_NUMBER LIKE 'DNO%' AND COLLECTION_AMNT-COLL_REC_HO>0.99 ".$query. $q ."");
                return $data;
                break;
            case "order_vs_delivery_adv":
                ...
                $data=DB::connection($this->db)->select("SELECT 
                        ...
                        t6.site_code,t6.site_name,t1.ordm_date,t1.ordm_ornm,ROUND(t1.ordm_amnt,2)ordm_amnt,
                ...
                return $data;
                break;
            case "tgt_achmnt_order":
                // return $end_date;
                $year = date('Y', strtotime($start_date));
                $mnth = date('m', strtotime($start_date));
                $q1='';
                $q2='';
                if($dirg_id !=""){
                    $q1=" AND t6.id IN (".implode(',',$zone_list).")";
                }          
                if ($zone_id != "") {
                    $q1 = " AND t6.id IN (".implode(',',$zone_id).") ";
                }
                if ($sales_group_id != "") {
                    $q2 = " AND t7.id IN (".implode(',',$sales_group_id).")";
                }
                DB::connection($this->db)->select(DB::raw("SET sql_mode=''"));
                $data=DB::connection($this->db)->select("SELECT t7.slgp_name GROUP_NAME,CONCAT(t6.zone_name,'-',t6.zone_code) ZONE_CODE,
                        CONCAT('$start_date',' to ','$end_date') TARGET_PERIOD, t5.aemp_usnm SV_ID,
                        t5.aemp_name SV_NAME, t4.aemp_usnm SR_ID,t4.aemp_name SR_NAME,
                        ROUND(sum(t1.trgt_tamt),2) TARGET_AMNT,
                        ROUND(sum(t2.ordr_amnt),2) ORDER_AMNT,
                        ROUND(sum(t2.deli_amnt),2) DELI_AMNT
                        ...
                        ( 
                            ...
                            FROM tt_ordm t1 
                            INNER JOIN tt_ordd t2 ON t1.id=t2.ordm_id
                            WHERE t1.ordm_date BETWEEN '$start_date' AND '$end_date' AND t1.lfcl_id not in (15,16,21) GROUP BY t1.aemp_id,t2.amim_id
                            )
                            t1 GROUP BY aemp_id,amim_id
                        ) t2 ON t1.aemp_susr=t2.aemp_id AND t1.amim_id=t2.amim_id 
                            INNER JOIN tm_amim t3 ON t1.amim_id=t3.id
                            ...
                            INNER JOIN tm_itcg t9 ON t8.itcg_id=t9.id
                            INNER JOIN tm_itcl t10 ON t3.itcl_id=t10.id
                            WHERE t1.trgt_year={$year} AND t1.trgt_mnth={$mnth} " .$q1.$q2. "
                            GROUP BY GROUP_NAME, ZONE_CODE, TARGET_PERIOD, SV_ID, SV_NAME, SR_ID, SR_NAME;");
                        // dd($data);
                return $data;
                break;
            case "sr_wise_order_delivery":
                ...
                $data=DB::connection($this->db)->select("SELECT 'acmp_name' acmp_name,t7.slgp_name,
                        ...
                        and (t1.ordm_date between '$start_date' AND '$end_date')
                                    and t1.id=t2.ordm_id 
                        ".$q2.$zone_query."  and t1.aemp_id=t5.id  and t5.zone_id=t6.id AND t1.slgp_id=t7.id
            ...
            case "zone_wise_order_delivery_summary":
                ...
                $data=DB::connection($this->db)->select("SELECT 'acmp_name' acmp_name,t7.slgp_name,
                        ...
                        t6.zone_name,
                        t6.zone_code,
                        ROUND(SUM(t2.ordd_oamt),2) AS ordd_oamt,
                        ROUND(SUM(t2.ordd_odat),2) AS deli_amt
                        FROM `tt_ordm` as t1,`tt_ordd`as t2,
                        tm_aemp as t5 ,tm_zone as t6,tm_slgp t7
                        where t1.id between $min_id and $max_id
        ...
        switch ($request->reportType){
            case "sr_activity_sales_hierarchy":
                if($details==1){
                    ...
                    if(!$id && $slgp_id){
                        $sr_query.=" AND t1.slgp_id  IN (".implode(',',$slgp_id).")";
                    }
                    DB::connection($this->db)->select(DB::raw("SET sql_mode=''"));
                    $data=DB::connection($this->db)->select("SELECT 
                    ...
                    $slgp_id=$request->slgp_id;
                    if($id){
                        $sr_query1=" AND t1.aemp_id IN (".implode(',',$id).")";
                        $sr_query=" AND t1.aemp_id IN (".implode(',',$id).")";
                    }
                    if(!$id && $slgp_id){
                        $sr_query1.=" AND t2.slgp_id IN (".implode(',',$slgp_id).")";
                        $sr_query.=" AND t1.slgp_id IN (".implode(',',$slgp_id).")";
                    }
                    $data=DB::connection($this->db)->select("SELECT t1.*,t2.t_market,t2.t_ward,t2.t_thana,t2.t_disct,'$start_date' dhbd_date,'$end_date' dhbd_date1,t2.mktm_name,t2.ward_name,t2.than_name,t2.ward_name,t2.than_name,
                            t2.dsct_name
                            FROM 
                            (SELECT 
                            t1.aemp_id aemp_id,t1.aemp_code aemp_usnm,
                            t1.aemp_name,
                            SUM(t1.t_outlet) rout_olt,
                            SUM((t1.`v_outlet`+t1.`s_outet`)) t_visit,
                            SUM(t1.`t_memo`)t_memo,SUM(t1.ro_visit) rout_visit,
                            ifnull(round(SUM(t1.`t_memo`)*100/SUM(t1.`v_outlet`+t1.`s_outet`),2),0) strikeRate,
                            ifnull(round(SUM(t1.t_sku)/SUM(t1.t_memo),2),0) lpc,
                            round(SUM(t_amnt)/1000,2) t_amnt,
                            t2.zone_name
                            FROM 
                            tt_aemp_summary1 t1
                            INNER JOIN tm_zone t2 ON t1.zone_id=t2.id
                            WHERE   t1.date between '$start_date' AND '$end_date' ". $sr_query.  "
                            GROUP BY t1.aemp_id,t1.aemp_code,
                            t1.aemp_name,t2.zone_name) t1
                            INNER JOIN (
                            SELECT t1.aemp_id,
                            count(distinct t4.id)t_market,count(DISTINCT t5.id)t_ward,count(DISTINCT t6.id)t_thana,count(distinct t7.id)t_disct,
                            GROUP_CONCAT(DISTINCT t4.id)mktm_name,
                            GROUP_CONCAT(DISTINCT t5.id)ward_name,
                            GROUP_CONCAT(DISTINCT t6.id)than_name,
                            GROUP_CONCAT(DISTINCT t7.id)dsct_name
                            FROM `th_ssvh` t1 
                            INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                            INNER JOIN tm_site t3 ON t1.site_id=t3.id
                            ...
                            where t1.ssvh_date between '$start_date' AND '$end_date' ". $sr_query1.  " AND curdate() AND t1.ssvh_ispd <2
                            GROUP BY t1.aemp_id) t2 ON t1.aemp_id=t2.aemp_id
                            ORDER BY t1.aemp_id;");
                }
            ...
            case "activity_summary":
                ...
                if($id==''){
                    ...
                    $slgp=DB::connection($this->db)->select("SELECT t1.id from tm_aemp t1
                            where t1.lfcl_id=1 AND t1.role_id>1 AND t1.role_id<=6 AND t1.slgp_id IN (".implode(',',$slgp_id).")
                            GROUP BY t1.id,aemp_name,t1.aemp_usnm ORDER BY t1.role_id DESC");
                    for($i=0;$i<count($slgp);$i++){
                        if($i ==count($slgp)-1){
                            $sv .=$slgp[$i]->id;
                  ...
                  select distinct month(attn_date) mnth,id,attn_date,aemp_usnm,aemp_name,ward_name,than_name,dsct_name 
                    from(        
                            ...
                            LEFT join tm_ward_ward_lat_lon t6  on
                                ...
                                where t1.aemp_id IN (".implode(',',$id).") and t1.attn_date between '$start_date' AND '$end_date' 
                            UNION ALL
                            SELECT distinct t1.note_date attn_date,t2.id, t2.aemp_usnm,t2.aemp_name,t3.edsg_name,t1.note_dtim attn_time,
                                SUBSTRING(t1.`geo_lat`, 1, 5) geo_lat1 ,
                                SUBSTRING(t1.`geo_lon`, 1, 4)  geo_lon1,t6.than_name,t6.dsct_name,t6.ward_name
                                ...
                                point((t1.geo_lon - 1 / 111.1), 		(t1.geo_lat - 1 / 111.1)) 
                                           ...
                                           and SUBSTRING(t1.`geo_lon`, 1, 3) = SUBSTRING(t6.`geo_lon`, 1, 3)                    
                                where t1.aemp_id IN (".implode(',',$id).") and t1.note_date between '$start_date' AND '$end_date' AND t1.site_code=''
                            UNION ALL
                            SELECT DISTINCT
                                t1.note_date attn_date,
                                t7.id,
                                ...
                                INNER JOIN tm_aemp AS t7 ON t1.aemp_id=t7.id
                                INNER JOIN tm_edsg AS t8 ON t7.edsg_id=t8.id
                                WHERE t1.note_date between '$start_date' AND '$end_date' AND t1.aemp_id  IN (".implode(',',$id).") AND t1.site_code !=''
                      ) gr )gr  
                      left join ( 
                          select month(dt) dt,aemp_id,sum(Total_note) T_note,sum( retailVisit) Rvisit,sum( OtherlVisit) Otvisit from(
                                            select  note_date dt, aemp_id, count(*) Total_note, 
                                            ...
                                            sum(case when ntpe_id<>1 then 1 ELSE 0 END) OtherlVisit
                                            from tt_note,
                                            tm_aemp where note_date between '$start_date' AND '$end_date' and  tm_aemp.id= tt_note.aemp_id and tm_aemp.id  IN (".implode(',',$id).")
                                            group by aemp_id,note_date
                                        ) ntc
                                        group by month(dt),aemp_id 
                      ) dg on dg.dt = gr.mnth  AND dg.aemp_id=gr.id
