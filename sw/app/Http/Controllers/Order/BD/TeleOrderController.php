...
//                    'staff_id' => $staff_id
    ...
    public function getOutletInfo(Request $request)
    {
        ...
        if ($this->userMenu->wsmu_read) {
            ...
            return DB::connection($this->db)->select("SELECT t1.id,t1.site_code, t1.site_name, CONCAT(t4.than_name,',', t5.dsct_name) 'thana', t1.site_adrs,
                        CONCAT({$code},RIGHT(REPLACE(t1.site_mob1,'-',''),{$digit})) 'mobile1', CONCAT({$code},REGEXP_REPLACE(REGEXP_REPLACE(t1.site_mob1, '[^a-zA-Z0-9]', ''), '^{$code}','' ))'mobile',  t5.dsct_name 'district',t1.site_olnm,t6.dm_name,t1.site_ownm,
                        t6.rout_day,t6.rout_name,t7.key_note,t7.birth_date
                        FROM tm_site t1
