            ...
            if ($db_conn != '') {
                ...
                try {
                    ...
                    if ($site1 == null) {
                        ...
                        //  $existId=Site::on($db_conn)->where('site_mob1', $outletData->Mobile_No_2)->first(['']);
                        // $site4 = Site::on($db_conn)->where('id', $existId->id)->delete();
                        // return Site::on($db_conn)->where('site_mob1', $outletData->Mobile_No_2)->delete();
                        // $site4 = DB::connection($db_conn)->table('tm_site')->where('site_mob1', $outletData->Mobile_No_2)->delete();
                        //  $site4 =  Site::on($db_conn)->where(['site_mob1' => $outletData->Mobile_No_2])->delete();
                        // $site4->delete();
                        //  $site5 = Site::on($db_conn)->where(['site_mob1' => $outletData->Mobile_No_2])->first();
...
");
    ...
    public
    function masterData_Common(Request $request)
    {
        $data1 = array();
        $data2 = array();
        $data3 = array();
...
WHERE t1.cont_id = $request->country_id");
            ...
            $bank_list = DB::connection($db_conn)->select("SELECT
...
FROM tm_bank;");
        ...
        return array(
            ...
            "grv_reason" => $data7,
            "msp_item_data" => $data8,
            "order_cancel_reason" => $tst,
            "bank_list" => $bank_list,
        );/* return Array(
            "SR_Group_Wise_Route_Table" => array("data" => $data1, "action" => $request->country_id),
            "tbld_note_type" => array("data" => $data2, "action" => $request->country_id),
            "tbld_promotion_gallery" => array("data" => $data3, "action" => $request->country_id),
            "Distribution_Info_Table" => array("data" => $data4, "action" => $request->country_id),
            "Non_Productive_Reason" => array("data" => $data5, "action" => $request->country_id),
            "Web_URL" => array("data" => $data6, "action" => $request->country_id),
            "Grv_Reason" => array("data" => $data7, "action" => $request->country_id),
...
SELECT
  ...
  t3.amim_name                                                                        AS Item_Name,
  round(t2.pldt_tppr,4)                                                               AS Item_Rate,
  round(t2.pldt_tppr,4)                                                               AS Grv_Item_Price,
  t2.amim_duft                                                                        AS Item_Factor,
...
GROUP BY t1.plmt_id, t3.id, t5.issc_name, t3.amim_name, t2.pldt_tppr, t2.amim_duft, t5.issc_seqn,t2.amim_dunt,t2.amim_runt,t3.amim_code ");
    ...
    public function dmSiteWiseCollectionDetailsData_role_wh(Request $request)
    {
        ...
        if ($db_conn != '') {
            ...
            where t1.`SITE_ID`=$site_id and t1.DELV_AMNT-t1.COLLECTION_AMNT>0.1 and t1.DELIVERY_STATUS=11 AND t1.DM_CODE='$dm_code'
            ...
            t1.STATUS IN(11) AND 
            t1.STATUS!=26 AND
            t1.INVT_ID IN(5) AND t1.slgp_id='$slgp_id'
            UNION ALL 
            SELECT t1.`ACMP_CODE`,
            t1.COLL_NUMBER AS ORDM_ORNM,t1.AEMP_ID,t1.`AEMP_USNM`,t3.aemp_name,
            t1.`WH_ID`,t1.`SITE_ID`,t1.`SITE_CODE`,t2.site_name,t1.`DM_CODE`,
    ...
    public function groupPartyCollectionDetailsData_role_wh(Request $request)
    {
        ...
        if ($db_conn != '') {
            ...
            if ($role_id == 10) { //DM
                ...
                $Coll_Data = DB::connection($db_conn)->select("
                                ...
                                and t1.DELV_AMNT-t1.COLLECTION_AMNT>0.1 and t1.DELIVERY_STATUS=11 AND t1.DM_CODE='$dm_code'
                                and t6.lfcl_id IN(39,11)
                                UNION ALL
                                SELECT t1.`ACMP_CODE`,t1.`ORDM_ORNM`,t1.`AEMP_ID`,t1.`AEMP_USNM`,t4.aemp_name,
                                t1.`WH_ID`,t1.`SITE_ID`,t1.`SITE_CODE`,t3.site_name,t1.`DM_CODE`,t1.`IBS_INVOICE`,t1.slgp_id, 1 AS t_Type, t5.optp_id as p_type_id,
                                ...
                                and t1.DELV_AMNT-t1.COLLECTION_AMNT>0.1 and t1.DELIVERY_STATUS=11 and t3.`site_code`=$site_code AND t1.DM_CODE='$dm_code'
                                and t6.lfcl_id IN(39,11)
                                UNION ALL
                                SELECT t1.`ACMP_CODE`,
                                t1.COLL_NUMBER AS ORDM_ORNM,t1.AEMP_ID,t1.`AEMP_USNM`,t3.aemp_name,
                                ...
                                JOIN tm_aemp t3 ON(t1.AEMP_ID=t3.id)
                                JOIN tl_stcm t5 ON(t2.id=t5.site_id AND t5.slgp_id = t1.slgp_id)
                                WHERE t1.SITE_ID=$site_id AND t1.slgp_id=$slgp_id AND          
                                t1.STATUS IN(11) AND 
                                t1.STATUS!=26 AND
                                t1.INVT_ID IN(5)
                                UNION ALL 
...
SELECT
  ...
  t4.site_code                                 AS Outlet_Code,
  t4.site_name                                 AS Outlet_Name,
  t4.site_olnm                                 AS Outlet_Name_Bn,
  t4.site_ownm                                 AS Owner_Name,
  t2.slgp_id                                   AS Group_ID,
  t3.rspm_serl                                 AS outlet_serial,
  t4.site_mob1                                 AS Mobile_No,
  t4.site_adrs                                 AS Outlet_Address,
  t4.site_olad                                 AS Outlet_Address_Bn,
  t4.geo_lat                                   AS geo_lat,
  t4.geo_lon                                   AS geo_lon
FROM tl_rpln AS t1
  INNER JOIN tl_sgsm AS t2 ON t1.aemp_id = t2.aemp_id
...
GROUP BY t1.prmr_id, t1.amim_id, t2.prms_sdat, t2.prms_edat, t1.prmr_id, t1.amim_id, t1.prmd_modr");
 ...
 where MBRContains(st_makeEnvelope (
  ...
  point(($request->geo_lon - 0.08 / 111.1), ($request->geo_lat - 0.08 / 111.1)) 
), POINT( geo_lon, geo_lat ))AND t1.lfcl_id=1 order by distance_in_km asc limit 30;
 ");
            /*$data1 = DB::connection($db_conn)->select("
     SELECT   t1.id                                                   AS Outlet_ID,
         t1.site_code                                                 AS Outlet_Code,
    ...
    t1.geo_lat,
    t1.geo_lon
    FROM tm_site_registration AS t1 where lfcl_id=1   HAVING distance_in_km < 0.05
...
GROUP BY t2.ordm_date,t2.site_id
       ");
            ...
            foreach ($order_master as $index => $data1) {
                ...
                $data2 = DB::connection($db_conn)->select("
SELECT t2.id AS Order_ID,t2.ordm_ornm AS Order_Code,t2.ordm_date
FROM  tm_site t1 JOIN  tt_ordm t2 ON(t1.id=t2.site_id)
where t2.ordm_date = '$start_date'
      AND t2.aemp_id='$aemp_id'
      AND t2.lfcl_id=1
