...
class AttendanceData extends Model implements FromCollection, WithHeadings, WithHeadingRow
{
    ...
    public function headings(): array
    {
        ...
        return ['Date', 'Group', 'Region', 'Zone', 'User_ID',
            ...
            'Status', 'Out_Time', 'Status','Loc_Adrs'];
    }
...
FROM tm_aemp AS t1
  ...
  LEFT JOIN tl_sgsm AS t3 ON t1.id = t3.aemp_id
  LEFT JOIN tm_slgp AS t4 ON t3.slgp_id = t4.id
  LEFT JOIN tm_zone AS t5 ON t3.zone_id = t5.id
  LEFT JOIN tm_edsg AS t6 ON t1.edsg_id = t6.id
...
GROUP BY t2.`aemp_id`,t2.atten_atyp,t2.`attn_date`
");*/
$dataRow = DB::connection($this->connection)->select("
SELECT
  t2.attn_date,
  ...
  t1.aemp_mob1,
  t6.edsg_name,
  count( DISTINCT t7.note_tokn )      AS note_cont,
  min(t2.attn_time) AS start_time,
   if(t11.id=1, 'Start Work', t11.atyp_name) AS start_status,
  max(t2.attn_time)                               AS end_time,
   if(t11.id=1, 'End Work', t11.atyp_name)   AS end_status,
t2.site_name AS Loc_Adrs   
FROM `tt_attn`t2 
JOIN tm_aemp t1 ON t1.id = t2.aemp_id
 LEFT JOIN tl_sgsm AS t3 ON t1.id = t3.aemp_id
  LEFT JOIN tm_slgp AS t4 ON t3.slgp_id = t4.id
  LEFT JOIN tm_zone AS t5 ON t3.zone_id = t5.id
  LEFT JOIN tm_edsg AS t6 ON t1.edsg_id = t6.id
  ...
  LEFT JOIN tm_dirg AS t10 ON t5.dirg_id = t10.id
  LEFT JOIN tm_atyp t11 ON t2.atten_atyp=t11.id
WHERE t2.`attn_date` BETWEEN '$this->start_date' and '$this->end_date'
GROUP BY t2.`aemp_id`,t2.atten_atyp,t2.`attn_date`;
");
