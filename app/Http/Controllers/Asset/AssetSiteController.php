...
class AssetSiteController extends Controller
{
        ...
        if ($this->userMenu->wsmu_read) {
            ...
            $mappings = $this->paginateArray(
                DB::connection($this->db)
                        ->select("select t1.id, t1.lfcl_id, t2.astm_name, t3.slgp_code, t3.slgp_name,
    ...
    {
        try {
            $asset = AssetSite::on($this->db)->findorfail($request->id);
            $asset->lfcl_id = intval($request->status);
            $asset->save();
            return $asset;
        } catch (\Throwable $th) {
