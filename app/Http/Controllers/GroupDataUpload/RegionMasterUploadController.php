...
class RegionMasterUploadController extends Controller
{
    private $access_key = 'tbld_RegionMasterUploadController_format';
    private $currentUser;
    private $userMenu;
    ...
    public function regionUploadFormatGen(Request $request)
    {
        if ($this->userMenu->wsmu_crat) {
            return Excel::download(new Region(), 'region_upload_format_' . date("Y-m-d H:i:s") . '.xlsx');
        } else {
            return redirect()->back()->with('danger', 'Access Limited');
        }
    }

    public function regionMasterUpload()
    {
        if ($this->userMenu->wsmu_crat) {
            return view('master_data.upload.region_master_upload')->with('permission', $this->userMenu);
        } else {
            return redirect()->back()->with('danger', 'Access Limited');
        }
    }
