...
class RoutePlanUploadController extends Controller
{
    private $access_key = 'tbld_RoutePlanUploadController_format';
    private $currentUser;
    private $userMenu;
...
WHERE t2.emp_id = $emp_id and t1.country_id=$country_id");
            return view('master_data.upload.emp_route_format')->with('salesGroups', $userSaleGroups);
        } else {
    ...
    {
        if ($this->userMenu->wsmu_crat) {
            return Excel::download(PJP::create($request->sales_group_id, $request->emp_id), 'employee_route_' . date("Y-m-d H:i:s") . '.xlsx');
        } else {
            return redirect()->back()->with('danger', 'Access Limited');
        }
    }
    ...
    public function empRouteUpload()
    {
        if ($this->userMenu->wsmu_crat) {
            ...
            return view('master_data.upload.emp_route_upload');
        } else {
            return redirect()->back()->with('danger', 'Access Limited');
        }
    }
