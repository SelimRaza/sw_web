...
class RouteMasterUploadController extends Controller
{
    private $access_key = 'tbld_RouteUploadController_format';
    private $currentUser;
    private $userMenu;
    ...
    {
        if ($this->userMenu->wsmu_crat) {
            return view('master_data.upload.route_master_upload')->with('permission', $this->userMenu);
        } else {
            return redirect()->back()->with('danger', 'Access Limited');
        }
    }
