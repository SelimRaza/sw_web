...
use App\MasterData\SiteMobileInfo;
...
class SiteController extends Controller
{
    ...
    public function unverifiedSite(){
        ...
        $erpzn=array();
        if($this->cont_id==14 || $this->cont_id==3 || $this->cont_id==15){
    ...
    public function mobileBulkEdit(){
        if ($this->userMenu->wsmu_updt) {
            return view('master_data.site.mobile_bulk_edit')->with('permission', $this->userMenu);
        } else {
            return redirect()->back()->with('danger', 'Access Limited');
        }
    }

    public function mobileEditFormat()
    {
        if ($this->userMenu->wsmu_crat) {
            return Excel::download(new SiteMobileInfo(), 'outlet_mobile_edit_format_' . date("Y-m-d H:i:s") . '.xlsx');
        } else {
            return redirect()->back()->with('danger', 'Access Limited');
        }
    }

    public function mobileBulkInsert(Request $request)
    {
        if ($this->userMenu->wsmu_crat) {
            if ($request->hasFile('import_file')) {
                DB::beginTransaction();
                try {
                    Excel::import(new SiteMobileInfo(), $request->file('import_file'));
                    DB::commit();
                    return redirect()->back()->with('success', ' Successfully Uploaded');
                } catch (\Exception $e) {
                    DB::rollback();
    ...
    public function mobileNoUpdate(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'site_code.required' => 'The outlet code is required.',
			...
			try {
                ...
                if(!$site){
                    return redirect()->back()->with('danger', ' Outlet code not fund');
                }
                $site->update([
                    'site_mob1' => $request->mobile_no,
                    'aemp_eusr' => $this->currentUser->employee()->id
                ]);

                $outlet = Outlet::on($this->db)->find($site->outl_id);
                if($outlet){
                    $outlet->update([
                        'oult_mob1' => $request->mobile_no,
                        'aemp_eusr' => $this->currentUser->employee()->id
                    ]);
                }
                
				DB::commit();
				return redirect()->back()->with('success', ' Mobile Number Successfully Updated');
			} catch (\Throwable $th) {
				DB::rollback();
