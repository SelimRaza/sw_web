        ...
        if ($this->userMenu->wsmu_vsbl) {
            ...
            if ($request->has('search_text')) {
                ...
                $employees = DB::connection($this->db)->table('tm_aemp AS t1')
                    ...
                    ->join('tm_slgp AS t8', 't1.slgp_id', '=', 't8.id')
                    ->join('tm_acmp AS t9', 't8.acmp_id', '=', 't9.id')
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($manager != null && $lineManager != null) {
            ...
            try {
                ...
                // if ($emp->isDirty(['role_id', 'master_role_id'])) {
                if ($emp->edsg_id != $request['role_id']) {
                    $dm_trip_data = DB::connection($this->db)->select("SELECT * FROM dm_trip WHERE DM_ID='$emp->aemp_usnm' AND STATUS = 'N'");
                    if($dm_trip_data){
                        return redirect()->back()->with('danger', 'You have an open trip')->with('btn_position', 'btn_employee1');
                    }else{
                        $emp->edsg_id = $request['role_id'];
                        $emp->role_id = $request['master_role_id'];
                    }
