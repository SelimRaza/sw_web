...
use App\BusinessObject\DOBKeyNote;
        ...
        if($info['status'] == 200) {
            ...
            try {
                ...
                if(!$has_order){
                    ...
                    Attendance::insert($attendance_info);
                }
...
//                'error' => $e->getMessage(),
    ...
    public function promotionNew(Request $request){
       ...
       // return $request->all();
        ...
        try {
            ...
            // New Update Mobile number,DOB, Key Note
              $req_site_id=$request->outlet_id;
              $req_site_mobile=$request->outlet_mobile;
              $key_note=$request->key_note;
              $birth_date=$request->dob;
              //site master update
              $update_number=Site::on($this->db)->find($req_site_id);
              $update_number->site_mob1=$req_site_mobile;
              $update_number->aemp_eusr=$employee_id;
              $update_number->save();
              if($birth_date || $key_note){
                $dob=DOBKeyNote::on($this->db)->where(['site_id'=>$req_site_id])->first();
                if($dob){
                  $dob->key_note=$key_note;
                  $dob->birth_date=$birth_date;
                  $dob->aemp_eusr=$employee_id;
                  $dob->save();
                }
                else{
                  $dob_key=new DOBKeyNote();
                  $dob_key->setConnection($this->db);
                  $dob_key->site_id=$req_site_id;
                  $dob_key->key_note=$key_note;
                  $dob_key->birth_date=$birth_date;
                  $dob_key->aemp_iusr=$employee_id;
                  $dob_key->aemp_eusr=$employee_id;
                  $dob_key->save();
                }
              }
        ...
        //            }
           // return $request->all();
                // if ($request->country_id == 2 || $request->country_id == 5) {
            if ($request->country_id == 5 || $request->country_id == 2) {
                $data = [];


                if (isset($request->item_slgp_ids)) {

                    foreach ($request->item_slgp_ids as $key => $slgp_id) {
                        $data[$slgp_id]['total']              = $request->total;
                        $data[$slgp_id]['outlet_id']          = $request->outlet_id;
                        $data[$slgp_id]['non_p_or_order']     = $request->non_p_or_order;
                        $data[$slgp_id]['country_id']         = $request->country_id;
                        $data[$slgp_id]['acmp_id']            = $request->acmp_id;
                        $data[$slgp_id]['nopr_id']            = $request->nopr_id;
                        $data[$slgp_id]['npro_note']          = $request->npro_note;
                        $data[$slgp_id]['order_note']         = $request->order_note;
                        $data[$slgp_id]['ntpe_id']            = $request->ntpe_id;
                        $data[$slgp_id]['rout_id']            = $request->item_rout_ids[$key];
                        $data[$slgp_id]['sr_id']              = $request->item_sr_ids[$key];
                        $data[$slgp_id]['item_ids'][]         = $request->item_ids[$key];
                        $data[$slgp_id]['item_prices'][]      = $request->item_prices[$key];
                        $data[$slgp_id]['slgp_id']            = $slgp_id;
                        $data[$slgp_id]['rout_id']            = $request->item_rout_ids[$key];
                        $data[$slgp_id]['plmt_id']            = $request->item_plmt_ids[$key];
                        $data[$slgp_id]['total_qtys'][]       = $request->total_qtys[$key];
                        $data[$slgp_id]['item_dufts'][]       = $request->item_dufts[$key];
                        $data[$slgp_id]['item_unit_prices'][] = $request->item_unit_prices[$key];
                        $data[$slgp_id]['item_discounts'][]   = $request->item_discounts[$key];
                        $data[$slgp_id]['item_special_discounts'][]   = $request->item_special_discounts[$key];
                        $data[$slgp_id]['item_prom_ids'][$request->item_ids[$key]]    = $request->item_prom_ids[$request->item_ids[$key]] ?? 0;
                        $data[$slgp_id]['dlrm_id']            = $this->getDistributorId($request->item_sr_ids[$key]);
                    ...
                    $item_slgp_ids = array_unique($request->item_slgp_ids);
                    foreach ($item_slgp_ids as $key => $slgp_id) {

                        if (isset($data[$slgp_id]['non_p_or_order']) && $data[$slgp_id]['non_p_or_order'] == "1" && isset($data[$slgp_id]['item_ids'])){
                        
                            $data[$slgp_id]['order_number'] = $this->getOrderNumber($data[$slgp_id]['sr_id'], $data[$slgp_id]['country_id'], $employee_id);
                        
                            $order_store_id = $this->storeOrderMaster($data[$slgp_id], $employee_id, $teleOrderTrackingId);
                        
                            $this->storeOrderLine($data[$slgp_id], $order_store_id, $employee_id);
                           // return "This block";
                        
                        }
                        else if(isset($data[$slgp_id]['non_p_or_order']) && $data[$slgp_id]['non_p_or_order'] == "0"){
                        
                            if(isset($data[$slgp_id]['nopr_id']) && !empty($data[$slgp_id]['nopr_id'])) {
                                $non_productive = $this->storeNonProductiveOutlet($data[$slgp_id], $employee_id, $teleOrderTrackingId);
                            }else{
            ...
            if($request->country_id != 9 && $request->country_id != 2 &&  $request->country_id != 5) {
                return response([
    ...
    public function storeOrderLine($data, $order_store_id, $employee_id){
       ...
       // return $data;
        ...
        foreach ($data['item_ids'] as $key => $id){
            ...
            $ordd_excs=($item->amim_pexc*100)/($data['item_unit_prices'][$key]*$data['item_dufts'][$key]);
            $ordd_texc=$data['item_unit_prices'][$key]*$data['total_qtys'][$key]*$ordd_excs/100;
            ...
            $ordd_opds=$data['item_discounts'][$key]??0;
            $ordd_tvat=($ordd_texc+($data['item_unit_prices'][$key]*$data['total_qtys'][$key])-($ordd_spdi+$ordd_opds))*$item->amim_pvat/100;
            $order_details[] = [
                'ordm_id' => $order_store_id,
    ...
    public function getSpecialDiscount($country_id, $slgp_id, $outlet_id, $item_special_discount, $item_subtotal){
        ...
        if($item_special_discount == 0){
            return 0;
        }else if($this->is_bangladesh($country_id)){
            if($slgp_id==11 || $slgp_id==12)
            {
            ...
            else if($payment_type->optp_id == 1){
                return $item_special_discount;
    ...
    public function storeAttendance($data, $employee_id){
        ...
        try{
            if(!$exist) {
                $sales_group=SalesGroup::on($this->db)->where('id',$slgp_id)->first();
                $acmp_id=$sales_group->acmp_id??0;
                $attendance_info = [
                    'slgp_id' => $data['slgp_id'],
                    'aemp_id' => $employee_id,
                    'site_id' => $data['outlet_id'],
    ...
    public function storeDataInDashboard(){
    ...
    public function teleOrderTracking($data){
        ...
        TeleOrderTracking::insert($order_tracking);
    ...
    public function getCompanies(Request $request)
    {
        if ($this->userMenu->wsmu_read) {
            ...
            if($request->country_id==9 || $request->country_id==2){
            ...
            try {
                ...
                return response(['companies' => $companies, 'minOrderAmount' => $minOrderAmount,'con' => $this->db], 200);
        ...
        if ($this->userMenu->wsmu_read) {
            ...
            $srs = DB::connection($this->db)
                ->select("select t2.id, t2.aemp_usnm, t2.aemp_name from tl_sgsm t1
                        ...
                        where t1.slgp_id={$slgp_id} and t1.zone_id={$zone_id} and t1.cont_id={$country_id} and t2.role_id=1 and t2.lfcl_id = 1 GROUP BY t2.id, t2.aemp_usnm, t2.aemp_name");
...
//        $staff_id = Employee::on($this->db)->where('aemp_usnm', auth()->user()->employee()->aemp_usnm)->first()->id;
        ...
        if ($this->userMenu->wsmu_read) {
            $this->db = $request->con ?? $this->db;
            $outlets = DB::connection($this->db)->select("SELECT t2.id, t2.site_code, t2.site_name,REPLACE(t2.site_mob1, '+', '')site_mob1 FROM tl_rsmp t1
                            INNER JOIN tm_site t2 ON t2.id = t1.site_id
...
//                    'staff_id' => $staff_id
    ...
    public function getOutletInfo(Request $request)
    {
        ...
        if ($this->userMenu->wsmu_read) {
            ...
            else if($request->country_id==6){
                $code='65';
                $digit=8;
            }
            else if($request->country_id==17){
                $code='965 ';
                $digit=8;
            }
            else if($request->country_id==24){
                $code='1';
                $digit=10;
            }
            else if($request->country_id==16){
                $code='39';
            }
            else if($request->country_id==27){
                $code='61';
            }
            $scmp=DB::connection($this->db)->select("SELECT cont_id FROM tl_scmp WHERE site_id={$outlet_id}");
            $outlet_nationality='';
            if($scmp){
                $cont_id=$scmp[0]->cont_id??0;
                $cont_data=DB::select("SELECT cont_name FROM tl_cont WHERE id={$cont_id}");
                $outlet_nationality=$cont_data[0]->cont_name??'';
            }
            return DB::connection($this->db)->select("SELECT t1.id,t1.site_code, t1.site_name, CONCAT(t4.than_name,',', t5.dsct_name) 'thana', t1.site_adrs,t1.site_mob1,
                        CONCAT({$code},REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(t1.site_mob1, '[^a-zA-Z0-9]', ''), '^{$code}','' ),'^{$code}',''))'mobile',  t5.dsct_name 'district',t1.site_olnm,t6.dm_name,
                        CONCAT(t1.site_ownm,'-','{$outlet_nationality}')site_ownm,
                        t6.rout_day,t6.rout_name,t7.key_note,t7.birth_date,'$outlet_nationality' outlet_nationality
                        FROM tm_site t1
                        ...
                        LEFT JOIN tl_tele_note_dob t7 ON t1.id=t7.site_id
    ...
    public function getOrderInfo(Request $request)
    {
        ...
        $order_master= DB::connection($this->db)->select("SELECT t1.id, t1.ordm_ornm, t1.ordm_date, t1.ordm_amnt, t2.lfcl_name 'order_status'
                        FROM tt_ordm t1
                        inner join tm_lfcl t2 on t1.lfcl_id = t2.id
                        WHERE  t1.site_id= {$site_id}  and t1.ordm_date >= curdate() - interval 60 day
                        ORDER BY t1.ordm_date desc");
    ...
    public function getSubCategories(Request $request)
    {
        if ($this->userMenu->wsmu_read) {
            ...
            $this->db = $country->cont_conn;
            //if($country_id == 2 || $country_id == 5)
            if($country_id == 5 || $country_id == 2){
                
                $categories = DB::connection($this->db)->select("SELECT t5.id, concat(t5.issc_name,'(',t7.slgp_name,')') cat_name, t5.issc_code cat_code
                                    ...
                                    INNER JOIN tm_slgp t7 ON t3.slgp_id=t7.id
                                    where t1.site_id={$site_id} and t6.lfcl_id = 1 AND t2.aemp_id={$sr_id}
                                    GROUP BY t7.slgp_name,t5.id,t5.issc_name,t5.issc_code");
    ...
    public function getItems(Request $request)
    {
        if ($this->userMenu->wsmu_read) {
            ...
            //     INNER JOIN tm_pldt AS t2 ON t1.plmt_id = t2.plmt_id AND t2.amim_id = t4.amim_id
            //     INNER JOIN tm_amim AS t3 ON t2.amim_id = t3.id 
            //     where t1.aemp_id = {$sr_id} AND t5.id={$category_id}
            //     group by t3.id ,t3.amim_code,t3.amim_name,t2.pldt_tppr,t3.amim_duft,t2.pldt_tpgp");
            // }
            else{
                $items = DB::connection($this->db)->select("SELECT
                    ...
                    INNER JOIN tl_sgit t3 ON t2.amim_id=t3.amim_id
                    INNER JOIN tm_amim t4 ON t3.amim_id=t4.id
                    WHERE t4.lfcl_id=1 AND t3.issc_id= {$category_id} AND t1.site_id = {$site_id} AND t1.slgp_id={$request->slgp_id}
    ...
    public function getGrvItems(Request $request)
    {
        if ($this->userMenu->wsmu_read) {
            ...
            if($country_id == 5){
            ...
            else if($country_id == 9 || $country_id == 2){
                $items = DB::connection($this->db)->select("SELECT
                    t3.id amim_id,t3.amim_code,t3.amim_name,t2.pldt_tppr,t3.amim_duft,t2.pldt_tpgp
                    FROM tl_sgsm AS t1
                    INNER JOIN tl_sgit AS t4 ON t1.slgp_id = t4.slgp_id 
                ...
                $items = DB::connection($this->db)->select("SELECT
                    ...
                    INNER JOIN tl_sgit t3 ON t2.amim_id=t3.amim_id
                    INNER JOIN tm_amim t4 ON t3.amim_id=t4.id
                    WHERE  t3.issc_id= {$category_id} AND t1.site_id = {$site_id}
                    group by t4.id ,t4.amim_code,t4.amim_name,t2.pldt_tppr,t4.amim_duft,t2.pldt_tpgp");
            }
       ...
       // DB::connection($this->db)->select(DB::raw("SET sql_mode=''"));
        $outlet_info = DB::connection($this->db)->select("
                SELECT t3.id,t3.site_code, t3.site_name,
	                ...
	                CONCAT({$code},RIGHT(REPLACE(t3.site_mob1,'-',''),{$digit})) 'mobile',t3.site_adrs,t8.rout_day,t8.rout_name,t3.site_ownm
                FROM tl_rpln t1
                ...
                LEFT JOIN tl_dmst_rout t8 ON t3.id=t8.site_id
                WHERE  t3.id = {$site_id}
                group by t3.id,t3.site_code, t3.site_name,
	                t6.than_name, t7.dsct_name,t3.site_adrs,t8.rout_day,t8.rout_name,t3.site_ownm");
...
//                where t1.aemp_id = {$employee_id}
    ...
    public function loadTeleOrderDetails(Request $request){
        ...
        if($request->country_id != 5 && count($order_details) > 0){
...
//        return response([$request->all()], 400);
    ...
    public function getBDInvoice(Request $request){
        try{
            $country = Country::findOrFail($request->country_id);
            $this->db = $country->cont_conn;
            $order=DB::connection($this->db)->select("SELECT id,attr4,site_id,aemp_iusr FROM tt_ordm WHERE id={$request->order_id}");
                $site_id=$order[0]->site_id;
                $aemp_id=$order[0]->aemp_iusr;
                $tracking_id=$order[0]->attr4;
                $data=DB::connection($this->db)->select("
                            SELECT t1.id order_id,CONCAT(t2.slgp_name,'-',t1.ordm_ornm)ordm_ornm,t2.slgp_name,t1.ordm_amnt,t1.ordm_icnt
                            FROM tt_ordm t1
                            INNER JOIN tm_slgp t2 ON t1.slgp_id=t2.id
                            WHERE t1.site_id={$site_id} AND t1.ordm_date=curdate() AND t1.aemp_iusr={$aemp_id} AND t1.attr4={$tracking_id} 
                            ");
            return $data;
        }
        catch(\Exception $e){
    ...
    public function getOrderAsPdf(Request $request){
        ...
        $this->db = $country->cont_conn;
        if($request->country_id==111111){
            $code='880';
            $order=DB::connection($this->db)->select("SELECT id,attr4,site_id,aemp_iusr FROM tt_ordm WHERE id={$request->order_id}");
            $site_id=$order[0]->site_id;
            $aemp_id=$order[0]->aemp_iusr;
            $tracking_id=$order[0]->attr4;
            $user_list=DB::connection($this->db)->select("
                        SELECT 
                        t2.id,t2.aemp_usnm,t2.aemp_name,t3.slgp_name,t4.acmp_name,t5.zone_name,t6.site_code,t6.site_name,t6.site_mob1,t6.site_adrs,
                        t1.id ordm_id,t1.ordm_ornm,t1.ordm_amnt,t1.ordm_icnt,t1.ordm_date
                        FROM tt_ordm t1
                        INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                        INNER JOIN tm_slgp t3 ON t1.slgp_id=t3.id
                        INNER JOIN tm_acmp t4 ON t3.acmp_id=t4.id
            ...
            for($i=0;$i<count($user_list);$i++){
                $id=$user_list[$i]->ordm_id;
                $single_order=DB::connection($this->db)->select("SELECT 
                                t2.amim_code,t2.amim_name,t1.ordd_inty,round(t1.ordd_spdc+t1.ordd_dfdo,2)discount,t2.amim_duft,round(t1.ordd_oamt,2)ordd_oamt
                                FROM tt_ordd t1
                                INNER JOIN tm_amim t2 ON t1.amim_id=t2.id
                                WHERE t1.ordm_id={$id}
                                ORDER BY  t2.amim_code,t2.amim_name ASC;");
                $single_user=array(
                    'id'            =>$user_list[$i]->id,
                    'ordm_ornm'     =>$user_list[$i]->ordm_ornm,
                    'aemp_usnm'     =>$user_list[$i]->aemp_usnm,
                    'aemp_name'     =>$user_list[$i]->aemp_name,
                    'slgp_name'     =>$user_list[$i]->slgp_name,
                    'acmp_name'     =>$user_list[$i]->acmp_name,
                    'zone_name'    =>$user_list[$i]->zone_name,
                    'site_code'    =>$user_list[$i]->site_code,
                    'site_name'    =>$user_list[$i]->site_name,
                    'site_mob1'    =>$user_list[$i]->site_mob1,
                    'site_adrs'    =>$user_list[$i]->site_adrs,
                    'ordm_date'    =>$user_list[$i]->ordm_date,
                    'ordm_amnt'    =>$user_list[$i]->ordm_amnt,
                    'ordm_icnt'    =>$user_list[$i]->ordm_icnt,
                    'amim_list'     =>$single_order,
                );
                array_push($data,$single_user);
            }
            // Multiple PDF
            $response = collect();
            $result=$data;
            foreach ($result as $d) {
                $data=$d;
              //  return $data['amim_list'];
                
                $order_pdf = PDF::loadView('report.TeleSales.order_pdf', compact('data'));
                $pdfPath = 'invoices/'.$d->ordm_ornm. '.pdf';
                Storage::put($pdfPath, $order_pdf->output());
                $response->push(Storage::download($pdfPath, $d->ordm_ornm . '.pdf')->deleteFileAfterSend(true));
            }
            return $response;
            // $order_pdf = PDF::loadView('report.TeleSales.order_pdf', compact('data'));
        ...
        if($request->country_id==25555){
            ...
            $tracking_id=$order[0]->attr4;
            $user_list=DB::connection($this->db)->select("
                        SELECT t1.id ordm_id
                        FROM tt_ordm t1");
            $data=array();
            for($i=0;$i<count($user_list);$i++){
                $id=$user_list[$i]->ordm_id;
                $single_order=DB::connection($this->db)->select("select t1.ordm_ornm, t1.ordm_date, t1.ordm_icnt, t1.ordm_amnt, t3.amim_name,
                                            t4.acmp_name, t4.acmp_code, t2.ordd_inty, t2.ordd_oamt, t2.ordd_spdo, t2.ordd_uprc, t5.aemp_name, t5.aemp_usnm, t6.site_name,
                                            t7.dlrm_code, t7.dlrm_name, t8.slgp_code, t8.slgp_name, t2.ordd_duft, t6.site_code, t3.amim_code, t1.ordm_drdt,t6.site_adrs,
                                            CONCAT({$code},RIGHT(REPLACE(t6.site_mob1,'-',''),{$digit})) 'mobile'
                                            from tt_ordm t1
        ...
        $order_items = DB::connection($this->db)->select("select t1.ordm_ornm, t1.ordm_date, t1.ordm_icnt, t1.ordm_amnt, t3.amim_name,
            t4.acmp_name, t4.acmp_code, t2.ordd_inty, t2.ordd_oamt, t2.ordd_spdo,round((t2.ordd_opds+t2.ordd_spdi+t2.ordd_dfdo),2)discount, t2.ordd_uprc, t5.aemp_name, t5.aemp_usnm, t6.site_name,
            t7.dlrm_code, t7.dlrm_name, t8.slgp_code, t8.slgp_name, t2.ordd_duft, t6.site_code, t3.amim_code, t1.ordm_drdt,t6.site_adrs,
            t6.site_mob1 AS 'mobile'
            from tt_ordm t1
            inner join tt_ordd t2 on t2.ordm_id = t1.id
            inner join tm_amim t3 on t2.amim_id = t3.id
            inner join tm_acmp t4 on t1.acmp_id = t4.id
    ...
    public function generatePDFs($data)
    {
        ...
        // Set the appropriate response headers for ZIP file download
        $headers = [
        ...
        $zipFilePath = tempnam(sys_get_temp_dir(), 'documents');

        if ($zip->open($zipFilePath, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== true) {
            return response()->json(['error' => 'Failed to create ZIP archive'], 500);
        }

        foreach ($data as $index => $pdfData) {
            $pdf = PDF::loadView('Order.tele_order_pdf', $pdfData);

            // Add the PDF to the ZIP archive
            $zip->addFromString("document{$index}.pdf", $pdf->output());
        }

        $zip->close();

        // Stream the ZIP archive and delete the temporary file after sending
        return response()->download($zipFilePath, 'documents.zip', $headers)->deleteFileAfterSend(true);
    }
    ...
    public function getTeleOrderPdf($order_id){
        ...
        if($country==2){
            $code='880';
            $order=DB::connection($this->db)->select("SELECT id,attr4,site_id,aemp_iusr FROM tt_ordm WHERE id={$order_id}");
            $site_id=$order[0]->site_id;
            $aemp_id=$order[0]->aemp_iusr;
            $tracking_id=$order[0]->attr4;
            $user_list=DB::connection($this->db)->select("
        ...
        try {
            ...
            if(!$exist) {
                ...
                $attendance_info = [
                    ...
                    'site_id' =>0,
                    'site_name' => $site_info->site_name ?? '',
                    'geo_lat' => $site_info->geo_lat ?? 0,
                    'geo_lon' => $site_info->geo_lon ?? 0,
                    'attn_time' => date('Y-m-d H:i:s', strtotime('-10 minutes')),
                    'attn_date' => date('Y-m-d'),
                    'attn_mont' => 1,
                    'atten_type' => 1,
