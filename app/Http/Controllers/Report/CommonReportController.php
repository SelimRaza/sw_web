...
class CommonReportController extends Controller
{
    ...
    public function commonReportFilterOrderDetails(Request $request){
        ...
        $quer = "SELECT 
            t2.ordm_date Date, t5.aemp_usnm,t5.aemp_name, t4.site_code, t4.site_name,
            t3.amim_code,t3.amim_name, t1.ordd_qnty, t1.ordd_oamt,t2.ordm_time
            FROM tt_ordm t2
    ...
    public function commonReportFilter(Request $request)
    {
        ...
        switch($reportType){
            ...
            case "sr_asset_stat":
                ...
                $asset='';
                if($dirg_id !=""){
                    $zone_q=" AND asset_zone IN (".implode(',',$zone_list).")";
                }
                if ($zone_id != "") {
                    $zone_q= " AND asset_zone = '$zone_id' ";
                }
                if($astm_id){
                    $asset .=" AND astm_id=$astm_id ";
                }
                // $rtype=1 Details, 2 Summary, 3 SV Summary
                $data=array();
                DB::connection($this->db)->select(DB::raw("SET sql_mode=''"));
                if($rtype==1){
                    // $data=DB::connection($this->db)->select("SELECT 
                    //         t4.aemp_usnm sv_id,t4.aemp_name sv_name,t2.aemp_usnm,t2.aemp_name,t3.zone_name,t1.*
                    //         FROM 
                    //         (SELECT t1.slgp_id,t1.aemp_id,t1.asset_zone,max(t1.asset_exp)asset_exp,
                    //         count(DISTINCT t2.site_id)total_site,IF(t1.site_cov>count(DISTINCT t3.site_id),t1.site_cov,count(DISTINCT t3.site_id)) asset_site,t1.site_cov
                    //         FROM 
                    //         (SELECT 
                    //         t1.slgp_id,t1.aemp_id,t1.rout_id,t1.asset_zone,ROUND(SUM(t1.asset_exp),2)asset_exp, COUNT(DISTINCT t1.site_id)site_cov
                    //         FROM tbl_asset_new t1
                    //         WHERE t1.ordm_date BETWEEN '$start_date' AND '$end_date' AND t1.slgp_id=$sales_group_id  " .$zone_q ."
                    //         GROUP BY t1.slgp_id,t1.aemp_id,t1.rout_id,t1.asset_zone)t1
                    //         INNER JOIN tl_rsmp t2 ON t1.rout_id=t2.rout_id
                    //         LEFT JOIN tl_assm t3 ON t2.site_id=t3.site_id AND t3.slgp_id=$sales_group_id AND t3.astm_id=$astm_id
                    //         GROUP BY t1.slgp_id,t1.aemp_id,t1.asset_zone) t1
                    //         INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                    //         INNER JOIN tm_zone t3 ON t1.asset_zone=t3.id
                    //         INNER JOIN tm_aemp t4 ON t2.aemp_mngr=t4.id ORDER BY t4.id ASC");
                    $data=DB::connection($this->db)->select("SELECT 
                            t4.aemp_usnm sv_id,t4.aemp_name sv_name,t2.aemp_usnm,t2.aemp_name,t3.zone_name,t1.*
                            FROM 
                            (SELECT t1.slgp_id,t1.aemp_id,t1.asset_zone,max(t1.asset_exp)asset_exp,
                                count(DISTINCT t2.site_id)total_site,IF(t1.site_cov>count(DISTINCT t3.site_id),t1.site_cov,count(DISTINCT t3.site_id)) asset_site,t1.site_cov,
                                GROUP_CONCAT(DISTINCT t4.rout_name)rout_name
                                FROM 
                                (SELECT 
                                    t1.slgp_id,t1.aemp_id,t1.rout_id,t1.asset_zone,ROUND(SUM(t1.asset_exp),2)asset_exp, COUNT(DISTINCT t1.site_id)site_cov
                                    FROM tbl_asset_new t1
                                    WHERE  t1.ordm_date BETWEEN '$start_date' AND '$end_date' AND t1.slgp_id=$sales_group_id " .$zone_q ."
                                    GROUP BY t1.slgp_id,t1.aemp_id,t1.rout_id,t1.asset_zone
                                )t1
                                INNER JOIN tl_rsmp t2 ON t1.rout_id=t2.rout_id
                                INNER JOIN tm_rout t4 ON t1.rout_id=t4.id
                                LEFT JOIN tl_assm t3 ON t2.site_id=t3.site_id AND t3.slgp_id=$sales_group_id " .$zone_q .  $asset."
                                GROUP BY t1.slgp_id,t1.aemp_id,t1.asset_zone
                            ) t1
                            INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                            INNER JOIN tm_zone t3 ON t1.asset_zone=t3.id
                            INNER JOIN tm_aemp t4 ON t2.aemp_mngr=t4.id ORDER BY t4.id ASC;");
                    ...
                    $data=DB::connection($this->db)->select("SELECT 
                            t3.zone_name,sum(t1.total_site)total_site,sum(t1.asset_site)asset_site,sum(t1.site_cov)site_cov,ROUND(SUM(t1.asset_exp),2)asset_exp
                            FROM 
                            (SELECT t1.slgp_id,t1.aemp_id,t1.asset_zone,max(t1.asset_exp)asset_exp,
                            count(DISTINCT t2.site_id)total_site,IF(t1.site_cov>count(DISTINCT t3.site_id),t1.site_cov,count(DISTINCT t3.site_id)) asset_site,t1.site_cov
                            FROM 
                            ...
                            GROUP BY t1.slgp_id,t1.aemp_id,t1.asset_zone) t1
                            INNER JOIN tm_zone t3 ON t1.asset_zone=t3.id
                            GROUP BY t3.zone_name ORDER BY t3.zone_code;");
                }
                else{
                    $data=array();
                }
            ...
            case "sr_hourly_activity":
                ...
                $data=DB::connection($this->db)->select("SELECT  t1.`act_date`, t1.`acmp_id`, t1.`slgp_id`, t1.`zone_id`, t1.`acmp_name`, t1.`slgp_name`,
                       t1. `zone_code`, t1.`zone_name`, t1.`aemp_id`, t1.`aemp_usnm`, t1.`aemp_name`,t1. `aemp_mob1`, t1.`t_outlet`, `visit_olt`, `success_olt`,
                        ...
                        than_name,than_olt FROM `tbl_sr_hourly_summary` t1
                        INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                        WHERE t1.slgp_id=$sales_group_id AND act_date between '$start_date' AND '$end_date' AND t2.role_id=1
                        " .$zone_q. "
                          ");
                return $data;
                break;
               ...
               // $end_date='2023-03-31';
                ...
                if($rtype==1){
                    $data=DB::connection($this->db)->select("SELECT 
                        ...
                        GROUP BY t2.zone_id,t4.zone_name,t1.dhbd_date,t3.slgp_name order by t4.zone_code,t1.dhbd_date asc");
                }else{
                    $data=DB::connection($this->db)->select("SELECT 
                        t4.zone_name zone_name,
                        t3.slgp_name slgp_name,
            ...
            case 'outlet_coverage':
                ...
                $data=DB::connection($this->db)->select("SELECT '$slgp_name' SALES_GROUP,t1.dsct_name DISTRICT_NAME,t1.than_name THANA_NAME,
                        ...
                        IFNULL(SUM(t2.memo),0)MEMO,SUM(t2.frequency)FREQUENCY
                        FROM tbl_mktm_wise_total_outlet t1 
                        LEFT JOIN
                        (
                            SELECT p2.id slgp_id,p2.slgp_name,p1.mktm_id,slgp_out,Visited_Sites,ordm_amnt,memo,frequency FROM tbl_mktm_slgp_wise_outlet p1 
                            inner join tm_slgp p2 ON p1.slgp_id=p2.id
                            LEFT JOIN (Select 
                                        slgp_id,mktm_id,count(distinct site_id) Visited_Sites,sum(ordm_amnt)ordm_amnt,
                                        SUM(IF(ordm_amnt>0,1,0))memo,count(site_id) frequency
                ...
                $summary=$result->pipe(function($result){
                    return collect([
                        ...
                        'memo'=>$result->sum('MEMO'),
                        'frequency'=>$result->sum('FREQUENCY'),
            ...
            case "outlet_stat":
                ...
                if($dist_id){
                    $query=$const." t1.dsct_id=$dist_id";
                    $adv_query .=" AND dsct_id={$dist_id} ";
                    $adv_query1 .=" AND DSCT_ID={$dist_id} ";
                }
                if($than_id){
                    $query.=" AND t1.than_id=$than_id";
                    $adv_query .=" AND than_id ={$than_id} ";
                    $adv_query1 .=" AND THAN_ID={$than_id} ";
                }
                // Visit
                if($outlet_stat_vo==1) {                
                    if($time_period==11 || $time_period==12){
                        $data=DB::connection($this->db)->select("SELECT   
                                t2.SLGP_NAME SALES_GROUP,t1.than_name THANA_NAME,t1.dsct_name DISTRICT_NAME,AVAILABLE_OUTLET,t2.SLGP_OUT ROUTED_OUTLET,
                                t2.`Visited_Sites` VISITED_SITES, t2.`ZERO_VISIT`, t2.`ONE_VISIT`, t2.`TWO_VISIT`, t2.`THREE_VISIT`, t2.`FOUR_VISIT`
                                FROM (SELECT than_id,dsct_id, than_name,dsct_name,SUM(total_outlet)AVAILABLE_OUTLET FROM   tbl_mktm_wise_total_outlet WHERE 1 " .$adv_query. "
                                        GROUP BY than_id,dsct_id,than_name,dsct_name
                                    ) t1
                                LEFT JOIN( SELECT THAN_ID,SLGP_NAME,SLGP_OUT,Visited_Sites,ZERO_VISIT,ONE_VISIT,TWO_VISIT,THREE_VISIT,FOUR_VISIT
                                            FROM tbl_out_stat  WHERE SLGP_ID={$sales_group_id} AND VISIT_FALG=1 AND TIME_PERIOD={$time_period} " .$adv_query1. "
                                        )t2 ON t1.than_id=t2.THAN_ID 
                                        
                                        
                                ORDER BY t1.than_name");
                    }
                    else{
                        $data=DB::connection($this->db)->select("SELECT MAX(t2.slgp_name) SALES_GROUP,t1.dsct_name DISTRICT_NAME,t1.than_name THANA_NAME,
                    ...
                    if($time_period==11 || $time_period==12){
                        $data=DB::connection($this->db)->select("SELECT   
                                ...
                                LEFT JOIN( SELECT THAN_ID,SLGP_NAME,SLGP_OUT,Visited_Sites,ZERO_VISIT,ONE_VISIT,TWO_VISIT,THREE_VISIT,FOUR_VISIT
                                            FROM tbl_out_stat WHERE SLGP_ID={$sales_group_id} AND VISIT_FALG=2 AND TIME_PERIOD={$time_period} " .$adv_query1. "
                                        )t2 ON t1.than_id=t2.THAN_ID ORDER BY t1.than_name");
                    }
                    else{
                        $data=DB::connection($this->db)->select("SELECT MAX(t2.slgp_name) SALES_GROUP,t1.dsct_name DISTRICT_NAME,t1.than_name THANA_NAME,
                            sum(t1.total_outlet)AVAILABLE_OUTLET,ifnull(sum(t2.slgp_out),0)ROUTED_OUTLET,ifnull(sum(t2.Visited_Sites),0) VISITED_SITES,
            ...
            //         $query.=" AND t1.than_id=$than_id";
            //     }
            //     $data=DB::connection($this->db)->select("SELECT '$acmp_name' COMPANY_NAME, '$slgp_name' SALES_GROUP,t1.dsct_name DISTRICT_NAME,t1.than_name THANA_NAME,
            //             sum(t1.total_outlet)AVAILABLE_OUTLET,ifnull(sum(t2.slgp_out),0)ROUTED_OUTLET,ifnull(sum(t2.Visited_Sites),0) VISITED_SITES,round(sum(t2.ordm_amnt)/1000,2)ordm_amnt,
            //             IFNULL(SUM(t2.memo),0)MEMO,SUM(t2.frequency)FREQUENCY
            //             FROM tbl_mktm_wise_total_outlet t1 
            //             LEFT JOIN
    ...
    public function getTrackingRecordGvt(Request $request){
        ...
        for($i=0;$i<count($slgp);$i++){
           ...
           $q1.=",IFNULL( max(`solt$slgp_id`),0) as `solt$slgp_id`,IFNULL(`v$slgp_id`,0)`v$slgp_id`,IFNULL(`rv$slgp_id`,0)`rv$slgp_id`,IFNULL(`m$slgp_id`,0)`m$slgp_id`
                    ,IFNULL(ROUND(`o$slgp_id`/1000,2),0) as `o$slgp_id`,IFNULL(ROUND(`d$slgp_id`/1000,2),0) as `d$slgp_id`";
           $q3.=",SUM(CASE WHEN t1.slgp_id=$slgp_id THEN slgp_out ELSE 0 END)`solt$slgp_id`";
            

        }
        // return "SELECT p2.disn_id,p.disn_name,p2.total_outlet ".$q1." FROM 
        // (
        ...
        $data=DB::connection($this->db)->select("SELECT p2.disn_id,p2.disn_name,p2.total_outlet " .$q1 .  " FROM 
                (SELECT t3.disn_name,t2.disn_id,sum(total_outlet)total_outlet FROM tbl_mktm_wise_total_outlet t1 
                INNER JOIN tm_dsct t2 ON t1.dsct_id=t2.id
                INNER JOIN tm_disn t3 ON t2.disn_id=t3.id
                group by t2.disn_id,t3.disn_name)p2
                LEFT JOIN 
                (
                SELECT t4.id disn_id "  .$q3 . " FROM tbl_mktm_slgp_wise_outlet t1 INNER JOIN tbl_mktm_wise_total_outlet t2 ON t1.mktm_id=t2.mktm_id
                INNER JOIN tm_dsct t3 On t2.dsct_id=t3.id INNER JOIN tm_disn t4 ON t3.disn_id=t4.id where t1.slgp_id IN (".implode(',',$slgp_ids).") group by t4.id,t1.slgp_id
                )stt ON p2.disn_id=stt.disn_id
                LEFT JOIN
                (
                    SELECT t2.disn_id,t2.disn_name " .$q2.  "
    ...
    public function getGvtDeeperSalesData(Request $request){
        ...
        for($i=0;$i<count($slgp);$i++){
            ...
            $q2.=",sum(CASE WHEN t2.slgp_id=$slgp_id THEN t2.visit ELSE 0 END)`v$slgp_id`,sum(CASE WHEN t2.slgp_id=$slgp_id THEN t2.ro_visit ELSE 0 END)`rv$slgp_id`,
                    sum(CASE WHEN t2.slgp_id=$slgp_id THEN t2.memo ELSE 0 END)`m$slgp_id`,sum(CASE WHEN t2.slgp_id=$slgp_id THEN t2.ord_amount ELSE 0 END)`o$slgp_id`,
                    sum(CASE WHEN t2.slgp_id=$slgp_id THEN t2.delivery_amount ELSE 0 END)`d$slgp_id`";
            $q1.=",IFNULL( max(`solt$slgp_id`),0) as `solt$slgp_id`,IFNULL(MAX(`v$slgp_id`),0)`v$slgp_id`,IFNULL(MAX(`rv$slgp_id`),0)`rv$slgp_id`,IFNULL(MAX(`m$slgp_id`),0)`m$slgp_id`
                    ,IFNULL(ROUND(max(`o$slgp_id`)/1000,2),0) as `o$slgp_id`,IFNULL(ROUND(max(`d$slgp_id`)/1000,2),0) as `d$slgp_id`";
            $q3.=",SUM(CASE WHEN t1.slgp_id=$slgp_id THEN slgp_out ELSE 0 END)`solt$slgp_id`";
        ...
        switch($stage){
            ...
            case 0:
                ...
                //     ");
                $data=DB::connection($this->db)->select("SELECT p2.dsct_id,p2.dsct_name,p2.total_outlet " .$q1 . " FROM 
                        (SELECT t2.id dsct_id,t2.dsct_name,sum(total_outlet)total_outlet FROM tbl_mktm_wise_total_outlet t1 
                        INNER JOIN tm_dsct t2 ON t1.dsct_id=t2.id
                        WHERE t2.disn_id=$id
                        group by t2.id,t2.dsct_name)p2
                        LEFT JOIN 
                        (
                            SELECT t3.id dsct_id "  .$q3 . " 
                            FROM tbl_mktm_slgp_wise_outlet t1 
                            INNER JOIN tbl_mktm_wise_total_outlet t2 ON t1.mktm_id=t2.mktm_id
                            INNER JOIN tm_dsct t3 On t2.dsct_id=t3.id  where t3.disn_id=$id AND t1.slgp_id IN (".implode(',',$slgp_ids).") group by t3.id,t1.slgp_id
                        )stt ON p2.dsct_id=stt.dsct_id
                        LEFT JOIN
                        (
                            SELECT t2.dsct_id,t2.dsct_name " .$q2.  "
                        ...
                        )p ON p.dsct_id=stt.dsct_id
                        GROUP BY p2.dsct_id,p2.dsct_name,p2.total_outlet;
                    ");
                break;
            case 1:
                $data=DB::connection($this->db)->select("SELECT p2.than_id,p2.than_name,p2.total_outlet " .$q1 . " FROM 
                        (SELECT t1.than_id,t1.than_name,sum(total_outlet)total_outlet FROM tbl_mktm_wise_total_outlet t1 
                        WHERE t1.dsct_id=$id
                        group by t1.than_id,t1.than_name)p2
                        LEFT JOIN 
                        (
                            SELECT t2.than_id  "  .$q3 . " 
                            FROM tbl_mktm_slgp_wise_outlet t1 
                            INNER JOIN tbl_mktm_wise_total_outlet t2 ON t1.mktm_id=t2.mktm_id
                            where t2.dsct_id=$id AND t1.slgp_id IN (".implode(',',$slgp_ids).") group by t2.than_id,t1.slgp_id
                        )stt ON p2.than_id=stt.than_id
                        LEFT JOIN
                        (
                        ...
                        )p ON p.than_id=stt.than_id
                        GROUP BY p2.than_id,p2.than_name,p2.total_outlet");
                    break;
            case 2:
                $data=DB::connection($this->db)->select("SELECT p2.ward_id,p2.ward_name,p2.total_outlet " .$q1 . " FROM 
                        (SELECT t1.ward_id,t1.ward_name,sum(total_outlet)total_outlet FROM tbl_mktm_wise_total_outlet t1 
                        WHERE t1.than_id=$id
                        group by t1.ward_id,t1.ward_name)p2
                        LEFT JOIN 
                        (
                            SELECT t2.ward_id "  .$q3 . " 
                            FROM tbl_mktm_slgp_wise_outlet t1 
                            INNER JOIN tbl_mktm_wise_total_outlet t2 ON t1.mktm_id=t2.mktm_id
                            where t2.than_id=$id AND t1.slgp_id IN (".implode(',',$slgp_ids).") group by t2.ward_id,t1.slgp_id
                        )stt ON p2.ward_id=stt.ward_id
                        LEFT JOIN
                        (
                            SELECT t2.ward_id " .$q2.  "                                    
                        ...
                        )p ON p.ward_id=stt.ward_id
                        GROUP BY p2.ward_id,p2.ward_name,p2.total_outlet;");
                    break;
            case 3:
                $data=DB::connection($this->db)->select("SELECT p2.mktm_id,p2.mktm_name,p2.total_outlet " .$q1 . " FROM 
                        (SELECT t1.mktm_id,t1.mktm_name,sum(total_outlet)total_outlet FROM tbl_mktm_wise_total_outlet t1 
                        WHERE t1.ward_id=$id
                        group by t1.mktm_id,t1.mktm_name)p2
                        LEFT JOIN 
                        (
                            SELECT t2.mktm_id "  .$q3 . " 
                            FROM tbl_mktm_slgp_wise_outlet t1 
                            INNER JOIN tbl_mktm_wise_total_outlet t2 ON t1.mktm_id=t2.mktm_id
                            where t2.ward_id=$id AND t1.slgp_id IN (".implode(',',$slgp_ids).") group by t2.mktm_id,t1.slgp_id
                        )stt ON p2.mktm_id=stt.mktm_id
                        LEFT JOIN
                        (
                            SELECT t2.mktm_id " .$q2.  "                                    
                        ...
                        )p ON p.mktm_id=stt.mktm_id
                        GROUP BY p2.mktm_id,p2.mktm_name,p2.total_outlet;
                                    ");
                break;
            case 4:
                $data="";
    ...
    public function getSRVisitHierarchyDetails(Request $request){
        $type=$request->type;
        ...
        if($type==1){
            $data=DB::connection($this->db)->select("SELECT 
                t3.mktm_name,count(DISTINCT t1.site_id) t_visit
                FROM `th_ssvh` t1
                INNER JOIN tm_site t2 ON t1.site_id=t2.id
                ...
                GROUP BY t3.id,t3.mktm_name ORDER BY t3.mktm_name ASC;");
            return $data;
        }
        else if($type==2){
            $data=DB::connection($this->db)->select("SELECT 
                t4.ward_name mktm_name,count(DISTINCT t1.site_id) t_visit
                FROM `th_ssvh` t1
                INNER JOIN tm_site t2 ON t1.site_id=t2.id
                INNER JOIN tm_mktm t3 ON t2.mktm_id=t3.id
                INNER JOIN tm_ward t4 ON t3.ward_id=t4.id
                WHERE t1.aemp_id=$aemp_id AND t1.ssvh_date between '$start_date' AND '$end_date'
                GROUP BY t4.id,t4.ward_name ORDER BY t4.ward_name ASC;");
            return $data;
        }
        else if($type==3){
            $data=DB::connection($this->db)->select("SELECT 
                t5.than_name mktm_name,count(DISTINCT t1.site_id) t_visit
                FROM `th_ssvh` t1
                INNER JOIN tm_site t2 ON t1.site_id=t2.id
                INNER JOIN tm_mktm t3 ON t2.mktm_id=t3.id
                INNER JOIN tm_ward t4 ON t3.ward_id=t4.id
                ...
                WHERE t1.aemp_id=$aemp_id AND t1.ssvh_date between '$start_date' AND '$end_date'
                GROUP BY t5.id,t5.than_name ORDER BY t5.than_name ASC;");
            return $data;
        }
        else if($type==4){
            $data=DB::connection($this->db)->select("SELECT 
                t6.dsct_name mktm_name,count(DISTINCT t1.site_id) t_visit
                FROM `th_ssvh` t1
                INNER JOIN tm_site t2 ON t1.site_id=t2.id
                INNER JOIN tm_mktm t3 ON t2.mktm_id=t3.id
                INNER JOIN tm_ward t4 ON t3.ward_id=t4.id
                ...
                WHERE t1.aemp_id=$aemp_id AND t1.ssvh_date between '$start_date' AND '$end_date'
                GROUP BY t6.id,t6.dsct_name ORDER BY t6.dsct_name ASC;");
            return $data;
        }
    }
        ...
        switch ($request->reportType){
            ...
            case "sr_activity_sales_hierarchy":
                if($request->dtls_sum==1){
                    ...
                    if(!$id && $slgp_id){
                        $sr_query1.=" AND t2.slgp_id=$slgp_id";
                        $sr_query.=" AND t1.slgp_id=$slgp_id";
                    }
            ...
            case "outlet_vs_item_coverage":
                ...
                $data=DB::connection($this->db)->select("SELECT t2.id aemp_id, t2.aemp_usnm sr_id, t2.aemp_name sr_name, t1.t_outlet rout_outlet, 
                    (t1.v_outlet+t1.s_outet) visited_olt, t1.t_amnt order_value, t1.t_memo total_orders,
                    t3.dist_olt 
                    FROM tt_aemp_summary1 t1
                    INNER JOIN tm_aemp t2 on t1.aemp_id = t2.id
                    INNER JOIN 
                    ( SELECT aemp_id, count(DISTINCT site_id) dist_olt
                    FROM th_ssvh WHERE
                    ssvh_date BETWEEN '{$start_date}' AND '{$end_date}'
                    GROUP BY aemp_id) t3 on t3.aemp_id = t1.aemp_id
                    WHERE t1.date BETWEEN '{$start_date}' AND '{$end_date}' AND t1.aemp_id in ({$ids})");
                break;
            default:
                $data="";
    ...
    public function getActivitySummaryDetails($aemp_usnm,$start_date,$end_date){
        ...
        $data=DB::connection($this->db)->select("select '$id' aemp_id,max(aemp_usnm)aemp_usnm,monthname(attn_date) mnth,attn_date,GROUP_CONCAT( DISTINCT than_name )than_name,DATE_FORMAT(min(s_time), '%r')s_time,
                DATE_FORMAT(max(e_time),'%r')e_time,
                GROUP_CONCAT( DISTINCT dsct_name)dsct_name,max(Total_note)Total_note,max(ifnull(retailVisit,0))retailVisit,max(ifnull(OtherVisit,0))OtherVisit from(
    ...
    public function getSrOutletCoverageDetails(Request $request){
        $staff_id = $request->staff_id;
        $start_date = $request->start_date;
        $end_date = $request->end_date;


        $data=DB::connection($this->db)->select("SELECT t2.amim_name, t2.amim_code, t1.order_amnt, t1.attr1 touch_olt
                        FROM tbl_itm_cov_details t1
                        INNER JOIN tm_amim t2 ON t1.amim_id = t2.id
                        WHERE t1.ordm_date BETWEEN '{$start_date}' AND '{$end_date}'
                        AND t1.aemp_id = {$staff_id}");
    ...
    public function getGroupOutletCoverageDetails(Request $request){
        $sr_ids     = $request->sr_ids;
        $start_date = $request->start_date;
        $end_date   = $request->end_date;

//        $old_data = DB::connection($this->db)->select("SELECT t2.id aemp_id, t2.aemp_usnm sr_id, t1.t_outlet rout_outlet,
//                    (t1.v_outlet+t1.s_outet) visited_olt, t3.dist_olt
//                    FROM tt_aemp_summary1 t1
//                    INNER JOIN tm_aemp t2 on t1.aemp_id = t2.id
//                    INNER JOIN
//                    ( SELECT aemp_id, count(DISTINCT site_id) dist_olt
...
//                    ssvh_date BETWEEN '{$start_date}' AND '{$end_date}'
//                    GROUP BY aemp_id) t3 on t3.aemp_id = t1.aemp_id
//                    WHERE t1.date BETWEEN '{$start_date}' AND '{$end_date}' AND t1.aemp_id in ({$sr_ids})");

        // old query
//        $data = DB::connection($this->db)->select("SELECT t2.amim_name, t2.amim_code, t3.aemp_name, t3.id aemp_id, t1.order_amnt total_amnt,
//                        t1.attr1 touch_olt
//                        FROM tbl_itm_cov_details t1
//                        INNER JOIN tm_amim t2 ON t1.amim_id = t2.id
//                        INNER JOIN tm_aemp t3 ON t1.aemp_id = t3.id
//                        WHERE t1.ordm_date BETWEEN '{$start_date}' AND '{$end_date}'
//                        AND t1.aemp_id in  ({$sr_ids})");

        // merged query

        $data = DB::connection($this->db)->select("SELECT count(distinct t2.id) total_aemp, sum(t1.t_amnt) total_amnt,
            sum(t1.v_outlet+t1.s_outet) total_visited, sum(t3.dist_olt) unique_visit,
            t4.amim_name, t4.amim_code, sum(t4.total_amnt) order_amnt, sum(t4.touch_olt) touch_olt
            FROM tt_aemp_summary1 t1
            INNER JOIN tm_aemp t2 on t1.aemp_id = t2.id
            INNER JOIN 
            ( SELECT aemp_id, count(DISTINCT site_id) dist_olt
            ...
            GROUP BY aemp_id) t3 on t3.aemp_id = t1.aemp_id
            INNER JOIN 
            (SELECT t2.amim_name, t2.amim_code, t1.aemp_id, t1.order_amnt total_amnt,
            t1.attr1 touch_olt
            FROM tbl_itm_cov_details t1
            INNER JOIN tm_amim t2 ON t1.amim_id = t2.id
            WHERE t1.ordm_date BETWEEN '{$start_date}' AND '{$end_date}'
            AND t1.aemp_id in ({$sr_ids})) t4 ON t4.aemp_id = t1.aemp_id
            WHERE t1.date BETWEEN '{$start_date}' AND '{$end_date}' AND t1.aemp_id in ({$sr_ids})
            group by t4.amim_name, t4.amim_code");
...
//        return [$old_data, $data];
    ...
    public function getThana($dist_id){
        $data=DB::connection($this->db)->select("Select id,than_name,than_code FROM tm_than WHERE dsct_id={$dist_id} ORDER BY than_name ASC");
        return $data;
    }
}