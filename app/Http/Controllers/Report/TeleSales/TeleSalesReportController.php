       ...
       // return $request->all();
        switch ($request->reportType){
            ...
            case "np_summary":
                ...
                if($details==1){
                    ...
                    $users = DB::connection($this->db)->select("SELECT t1.id, t1.aemp_name, t1.aemp_usnm, t3.aemp_name manager FROM tm_aemp t1
                        ...
                        where t4.acmp_id=$acmp_id   $zone_q");
                    ...
                    foreach ($users as $user){
                        ...
                        FROM tt_ordm t1
                        INNER JOIN tbl_tele_users t2 ON t1.aemp_id=t2.aemp_id
                        WHERE t1.ordm_date between '$start_date' AND '$end_date' AND t1.lfcl_id !=16 AND t2.aemp_id = {$user->id} AND t1.acmp_id=$acmp_id");
                    }
                ...
                break;
            case "order_pdf":
                $user_list=DB::connection($this->db)->select("SELECT 
                            t2.id,t2.aemp_usnm,t2.aemp_name,t3.slgp_name,t4.zone_name
                            FROM tbl_tele_users t1
                            INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                            INNER JOIN tm_slgp t3 ON t2.slgp_id=t3.id
                            INNER JOIN tm_zone t4 ON t2.zone_id=t4.id
                            GROUP BY t2.id,t2.aemp_usnm,t2.aemp_name,t3.slgp_name,t4.zone_name;");
                $data=array();
                for($i=0;$i<count($user_list);$i++){
                    $id=$user_list[$i]->id;
                    $single_order=DB::connection($this->db)->select("SELECT 
                                ordm_ornm,ordm_amnt,ordm_icnt,ordm_date,id
                                FROM tt_ordm 
                                WHERE aemp_id={$id} AND ordm_date BETWEEN '$start_date' AND '$end_date' AND lfcl_id=1 ORDER BY ordm_ornm,aemp_id,site_id");
                    $single_user=array(
                        'id'            =>$user_list[$i]->id,
            ...
            case "efficiency_metrics":
                ...
                if($slgp_id){
                    $slgp .= " AND t3.slgp_id = $slgp_id";
                }
                if($acmp_id){
                    $slgp .= " AND t10.acmp_id = $acmp_id";
                }
                $data=DB::connection($this->db)->select("SELECT t3.slgp_name user_slgp,t4.zone_code,t4.zone_name,t2.aemp_usnm,t2.aemp_name,t1.* FROM (
                            SELECT 
                            t3.aemp_id,t3.ordm_date,t3.ordm_time ordm_time,t4.site_code,t4.site_name,t4.site_mob1,'Productive' order_status,t3.ordm_amnt,'' np_reason,t6.rcds_file,t10.slgp_name
                            FROM tbl_tele_users t1
                            INNER JOIN tt_ordm t3 ON t1.aemp_id=t3.aemp_id
                            INNER JOIN tm_site t4 ON t3.site_id=t4.id
                            INNER JOIN tt_tltr t5 ON t1.aemp_id=t5.aemp_id AND t3.ordm_date=t5.tltr_date AND t3.site_id=t5.site_id
                            INNER JOIN tm_slgp t10 ON t3.slgp_id=t10.id
                            LEFT JOIN tm_rcds t6 ON t5.id=t6.tltr_id
                            WHERE t3.ordm_date BETWEEN '$start_date' AND '$end_date' " . $slgp.  "
                            UNION ALL
                            SELECT 
                            t3.aemp_id,t3.npro_date ordm_date ,t3.npro_time ordm_time,t4.site_code,t4.site_name,t4.site_mob1,'Non Productive' order_status,0 ordm_amnt,t7.nopr_name np_reason,t6.rcds_file,t10.slgp_name
                            FROM tbl_tele_users t1
                            INNER JOIN tt_npro t3 ON t1.aemp_id=t3.aemp_id
                            INNER JOIN tm_site t4 ON t3.site_id=t4.id
                            INNER JOIN tt_tltr t5 ON t1.aemp_id=t5.aemp_id AND t3.npro_date=t5.tltr_date AND t3.site_id=t5.site_id
                            INNER JOIN tm_nopr t7 ON t3.nopr_id=t7.id
                            INNER JOIN tm_slgp t10 ON t3.slgp_id=t10.id
                            LEFT JOIN tm_rcds t6 ON t5.id=t6.tltr_id
                            WHERE t3.npro_date BETWEEN '$start_date' AND '$end_date' " .$slgp .  "
                            )t1
                            INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                            INNER JOIN tm_slgp t3 ON t2.slgp_id=t3.id
                            INNER JOIN tm_zone t4 ON t2.zone_id=t4.id
                            WHERE 1 " .$zone_q .  "
                            ORDER BY t1.ordm_time ASC;");
                return $data;
                break;
            ...
            case "order_report":
                if($details==0){
                    ...
                    $data=DB::connection($this->db)->select("SELECT 
                            ...
                            SUM( IF(t6.TRIP_STATUS='D',1,0)) delv_sku,IFNULL(t5.DELV_AMNT,0)DELV_AMNT,IFNULL(t5.INV_AMNT,0)INV_AMNT, ROUND(IFNULL(t5.COLLECTION_AMNT,0),2)COLLECTION_AMNT
                            FROM tbl_tele_users t1
            ...
            case "non_productive_reason_outlet_summary":
                ...
                $data=DB::connection($this->db)->select("SELECT t2.npro_date,t9.dsct_name,t8.than_name,t7.ward_name,t6.mktm_name,
                            ...
                            CONCAT({$code},RIGHT(REPLACE(t5.site_mob1,'-',''),{$digit})) 'site_mob11',t5.site_mob1
                            FROM tbl_tele_users t1
...
public function getStartAndEndDate($time_period){
    ...
    public function getAllOrdersOfTwoParticularDate($start_date,$end_date){
        $user_list=DB::connection($this->db)->select("SELECT 
                            t2.id,t2.aemp_usnm,t2.aemp_name,t3.slgp_name,t4.zone_name,t6.site_code,t6.site_name,t6.site_mob1,t5.ordm_ornm,t5.ordm_date,t5.ordm_amnt,t5.ordm_icnt,t5.id ordm_id,t6.site_adrs,t7.acmp_name
                            FROM tbl_tele_users t1
                            INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                            INNER JOIN tm_zone t4 ON t2.zone_id=t4.id
                            INNER JOIN tt_ordm t5 ON t1.aemp_id=t5.aemp_id
                            INNER JOIN tm_slgp t3 ON t5.slgp_id=t3.id
                            INNER JOIN tm_site t6 ON t5.site_id=t6.id
                            INNER JOIN tm_acmp t7 ON t3.acmp_id=t7.id
                            WHERE t5.ordm_date BETWEEN '$start_date' AND '$end_date' AND t5.lfcl_id=1
                            ORDER BY t5.ordm_date,t5.site_id,t5.aemp_id ASC
                            ;");
        $data=array();
        for($i=0;$i<count($user_list);$i++){
            $id=$user_list[$i]->ordm_id;
            $single_order=DB::connection($this->db)->select("SELECT 
                            t2.amim_code,t2.amim_name,t1.ordd_inty,round((t1.ordd_opds+t1.ordd_spdi+t1.ordd_dfdo),2)discount,t2.amim_duft,round(t1.ordd_oamt,2)ordd_oamt,round(t1.ordd_uprc,2)ordd_uprc
                            FROM tt_ordd t1
        ...
        return view('report.TeleSales.order_pdf',['data'=>$data]);
    }
    public function getSingleOrder($ordm_ornm){
        $user_list=DB::connection($this->db)->select("SELECT 
                             t2.id,t2.aemp_usnm,t2.aemp_name,t3.slgp_name,t4.zone_name,t6.site_code,t6.site_name,t6.site_mob1,t5.ordm_ornm,t5.ordm_date,t5.ordm_amnt,t5.ordm_icnt,t5.id ordm_id,t6.site_adrs,t7.acmp_name
                            FROM tbl_tele_users t1
                            INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
    ...
    public function getAllOrdersOfTwoParticularDateAndEmployee($aemp_id,$start_date,$end_date){
        $user_list=DB::connection($this->db)->select("SELECT 
                             t2.id,t2.aemp_usnm,t2.aemp_name,t3.slgp_name,t4.zone_name,t6.site_code,t6.site_name,t6.site_mob1,t5.ordm_ornm,t5.ordm_date,t5.ordm_amnt,t5.ordm_icnt,t5.id ordm_id,t6.site_adrs,t7.acmp_name
                            FROM tbl_tele_users t1
                            INNER JOIN tm_aemp t2 ON t1.aemp_id=t2.id
                            ...
                            INNER JOIN tm_site t6 ON t5.site_id=t6.id
                            INNER JOIN tm_acmp t7 ON t3.acmp_id=t7.id
                            WHERE t1.aemp_id={$aemp_id} AND t5.ordm_date BETWEEN '$start_date' AND '$end_date'
                            ORDER BY t5.ordm_date,t5.site_id,t5.aemp_id ASC
                            ;");
        $data=array();
        for($i=0;$i<count($user_list);$i++){
    ...
    public function getDashboardData(){
        $data=DB::select("SELECT t2.id,t2.cont_name,t2.cont_code,sum(tltr_amnt)t_amnt, SUM(IF(tltr_ordr=1,1,0)) t_memo, SUM(IF(tltr_ordr=0,1,0)) t_npcl,
                    COUNT(t1.id)t_call FROM `tt_tltr` t1 INNER JOIN tm_cont t2 ON t1.cont_id=t2.id WHERE t1.tltr_date=curdate() GROUP BY t1.cont_id ORDER BY t1.cont_id ASC; ");
