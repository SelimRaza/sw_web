...
class BlockOrderController extends Controller
{
    ...
    public function filterMaintainBlock(Request $request)
    {
        ...
        if($start_date != ''){
            $end_date   =   $request->end_date ? $request->end_date : date('Y-m-d');
            $query.=" AND t1.ordm_date between '$start_date' AND '$end_date'";
        }
        ...
        $data=DB::connection($this->db)->select("SELECT
                ...
                WHERE  t6.aemp_id='$this->aemp_id' ".$query."
                GROUP BY 
                t1.id,
                t1.ordm_ornm,
                t1.ordm_amnt,
        ...
        if ($this->userMenu->wsmu_delt) {
            ...
            try {
                ...
                if ($request->submit=='release') {
                    $ouSiteMapping = CompanySiteBalance::on($this->db)->where(['site_id' => $request->site_id, 'slgp_id' => $request->slgp_id])->first();
                    $specialBudgetMaster = SpecialBudgetMaster::on($this->db)->where(['aemp_id' =>$this->aemp_id,'spbm_mnth' => date('m'), 'spbm_year' => date('Y')])->first();
                   // dd($specialBudgetMaster != null && $ouSiteMapping != null && $specialBudgetMaster->spbm_amnt >= array_sum($request->discountAmount));
                    if ($specialBudgetMaster != null && $ouSiteMapping != null && $specialBudgetMaster->spbm_amnt >= array_sum($request->discountAmount)) {
                        ...
                        if($orderMaster->lfcl_id==1){
                            DB::connection($this->db)->select("Update dm_trip_detail SET TRIP_STATUS='N' WHERE ORDM_ORNM='$ornm'");
                            DB::connection($this->db)->select("Update dm_trip_master SET DELIVERY_STATUS=0 WHERE ORDM_ORNM='$ornm'");
                        }
    ...
    public function oneTimeDueRelease($id)
    {
        if ($this->userMenu->wsmu_delt) {
            $orderMaster = collect(DB::connection($this->db)->select("SELECT
            t1.id               AS id,
            ...
            return view('blockOrder.one_time_credit_release')->with('orderMaster', $orderMaster)->with('orderLine', $orderLine)->with('cancelReason', $cancelReason)->with('permission', $this->userMenu);
    ...
    public function oneTimeDueReleaseAction(Request $request, $id)
    {
        // dd($request->all());
        if ($this->userMenu->wsmu_delt) {
            DB::connection($this->db)->beginTransaction();
            ...
            try {
                ...
                if ($request->submit=='release') {
                    ...
                    if($orderMaster->lfcl_id==1){
                        ...
                        $ordd=DB::connection($this->db)->select("Select ordd_spdi FROM tt_ordd WHERE ORDM_ORNM='$ornm'");
                        foreach ($ordd as  $value) {
                            $spdi = 0;
                            $spdi += $value->ordd_spdi;
                            if($spdi > 0){
                                $orderMaster->lfcl_id=17;
                                $status = "danger";
                                $msg = " Special discount block added in this site !!!";
                            }
                        }

                        // $cpcr_credit_count = DB::connection($this->db)->select("SELECT count(id) credit_count, ordm_ornm FROM `tl_cpcr` where site_id=$request->site_id AND sapr_amnt-scol_amnt>0.99;");
                        $credit_count = DB::connection($this->db)->select("SELECT id, ordm_ornm FROM `tl_cpcr` where site_id=$request->site_id AND sapr_amnt-scol_amnt>0.99;");
                        if(count($credit_count) >= 2){
                            $orderMaster->lfcl_id = 44;
                            
                            $orderNumber = '';
                            foreach($credit_count as $credit){
                                $orderNumber .= $credit->ordm_ornm . ', ';
                            }
                            
                            $status = "danger";
                            $number = rtrim($orderNumber, ', ');
                            $msg = " This site already credited over 2 times, Here is the order number : " . $number;                           
