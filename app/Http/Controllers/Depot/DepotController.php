...
class DepotController extends Controller
{
    ...
    public function filterDepotDetails(Request $request)
    {
        ...
        $query = "SELECT t1.id as id, t2.id as slgp_id, t2.slgp_name, concat(t4.than_code, ' - ',t4.than_name) as than_name, t5.base_name, t1.`dlrm_code`,
                    t1.`dlrm_name`, t1.`dlrm_ownm`, t1.`dlrm_adrs`, t1.`dlrm_mob1`, t1.dlrm_akey, t1.lfcl_id, t1.cont_id, t6.id as zone_id, t6.zone_name, t7.dirg_name FROM `tm_dlrm` t1 
    ...
    public function dealerLoginCreate(Request $request)
    {
        try {
            ...
            $id = $request->id;
            $this->currentUser = Auth::user();
            $dealer = DB::connection($this->db)->select("SELECT `dlrm_name`, `dlrm_code`, `dlrm_adrs`,`dlrm_mob1`, `dlrm_emal`, `id`, slgp_id, cont_id FROM `tm_dlrm` WHERE lfcl_id='1' and id='$id'");
            ...
            if ($cont_id=='2' || $cont_id=='5'){
                ...
                $employee = Employee::on($this->db)->where('aemp_usnm',$code)->first();
                if($employee){
                    return 'Dealer already created';
                }
                $emp = new Employee();
    ...
    public function dealerLoginCreateOld(Request $request)
    {
