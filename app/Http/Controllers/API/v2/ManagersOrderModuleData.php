...
");
    ...
    public function GetManagers_SR(Request $request)
    {
        ...
        if ($db_conn != '') {
            if($role_id > 0){
                $data1 = DB::connection($db_conn)->select("
                    SELECT t1.`id`AS Sr_id,t1.`aemp_usnm`AS Sr_Code,t1.`aemp_name`AS Sr_Name,t1.aemp_mob1 AS Sr_Mobile,t1.role_id
                    FROM `tm_aemp` t1 WHERE t1.`aemp_mngr`=$request->MG_EmpId AND t1.`lfcl_id` = 1 ORDER BY role_id DESC
                ");
            }else{
                $data1 = DB::connection($db_conn)->select("
                    SELECT t1.`id`AS Sr_id,t1.`aemp_usnm`AS Sr_Code,t1.`aemp_name`AS Sr_Name,t1.aemp_mob1 AS Sr_Mobile,t1.role_id
                    FROM `tm_aemp` t1 WHERE t1.`role_id`= 1 AND t1.`aemp_mngr`=$request->MG_EmpId AND t1.`lfcl_id` = 1 ORDER BY role_id DESC
                ");
            }
