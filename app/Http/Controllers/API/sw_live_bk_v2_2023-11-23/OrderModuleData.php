...
WHERE t2.rtan_rtnm='$order_code';
    ...
    public function dmSiteWiseCollectionDetailsData_role(Request $request)
    {
        ...
        if ($db_conn != '') {
            ...
            case when (t1.COLLECTION_AMNT>0 and t2.sapr_amnt>0) then 1  else 0 end CredStatus ,t1.ORDM_DRDT AS del_date,t1.DELV_AMNT as actual_inv_amt
			...
			8 CredStatus ,t1.COLL_DATE AS del_date,t1.COLLECTION_AMNT as actual_inv_amt
			...
			left join tl_cpcr t6 on t1.SITE_ID=t6.site_id and t1.COLL_NUMBER=t6.ordm_ornm
            ...
            0 reqAmnt,0 apprv,8 CredStatus ,t1.COLL_DATE AS del_date,t1.COLLECTION_AMNT as actual_inv_amt
            FROM `dm_collection`t1
            JOIN tm_site t2 ON(t1.SITE_ID=t2.id)
            JOIN tm_aemp t3 ON(t1.AEMP_ID=t3.id)
    ...
    public function CollectionDetailsData(Request $request)
    {
        ...
        if ($db_conn != '') {
				...
				$tm_aemp = DB::connection($db_conn)->table('tm_aemp')->where('aemp_usnm', $dm_code)->first();
				   if ($tm_aemp->edsg_id == 14) { 
				    $Coll_Data = DB::connection($db_conn)->select("
                                ...
                                FROM `dm_trip`t1 
                                JOIN dm_trip_master t6 ON(t1.`TRIP_NO`=t6.TRIP_NO)
                                JOIN tm_site t3 ON(t6.SITE_ID=t3.id)
                                JOIN tm_aemp t4 ON(t6.AEMP_USNM=t4.aemp_usnm)
                                JOIN dm_trip_detail as t2 ON (t2.SITE_ID= t6.SITE_ID) and t6.TRIP_NO=t2.TRIP_NO
                                join tl_stcm t5 ON(t5.site_id=t6.SITE_ID)AND t5.slgp_id=t6.slgp_id
            ...
            JOIN tl_rpln t5 ON(t4.id=t5.aemp_id)
            JOIN tl_rsmp t6 ON(t6.rout_id=t5.rout_id) AND t6.site_id=t1.`SITE_ID`
            WHERE t4.aemp_mngr=$emp_id AND (t1.COLLECTION_AMNT - t1.COLL_REC_HO)>0 AND t1.INVT_ID=5
            GROUP BY t1.slgp_id,t1.SITE_ID,t1.SITE_CODE,t2.site_name,t2.site_mob1 ,t1.AEMP_USNM,
            t4.id ,t4.aemp_name,t2.geo_lat,t2.geo_lon, t1.COLL_NUMBER,t3.optp_id
            UNION ALL 
            SELECT '' AS`TRIP_NO`,t1.slgp_id,t1.SITE_ID,t1.SITE_CODE,t2.site_name,t2.site_mob1 AS site_mobile,
  ...
  t1.role_id                              AS type_id,
  t2.role_name                            AS type_name,
   ( ACOS( COS( RADIANS( $geo_lat) )
                 * COS( RADIANS( t3.geo_lat ) )
                 * COS( RADIANS( t3.geo_lon ) - RADIANS( $geo_lon ) )
                 + SIN( RADIANS($geo_lat) )
                   * SIN( RADIANS( t3.geo_lat ) )
           )
           * 6371
         ) AS distance_in_km
FROM tm_aemp AS t1
...
WHERE t1.lfcl_id=1 and t3.lloc_date =curdate()
GROUP BY t3.aemp_id
...
ORDER BY `amount` DESC";
	 ...
	 public function user_wise_all_item_report(Request $request)
    {
        $country = (new Country())->country($request->country_id);
        $db_conn = $country->cont_conn;
        if ($db_conn != '') {
	 ...
	 public function user_wise_summary_report(Request $request)
    {
        $country = (new Country())->country($request->country_id);
        $db_conn = $country->cont_conn;
		$id=$request->aemp_id;
