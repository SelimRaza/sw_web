...
class CompanySiteMapping extends Model implements WithHeadings, ToModel, WithHeadingRow
{
    ...
    {
        return ['company_id', 'company_name', 'site_id', 'site_name', 'payment_type_id', 'price_list_id', 'credit_limit', 'limit_days'];
    }

    public function model(array $row)
    {
        $value = (object)$row;
        $companySiteMapping = CompanySiteMapping::where(['company_id' => $value->company_id, 'site_id' => $value->site_id])->first();
    ...
    public function outletPaymentType()
    {
        return OutletPaymentType::find($this->payment_type_id);
    }
