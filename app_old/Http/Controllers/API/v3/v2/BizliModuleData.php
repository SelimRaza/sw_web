...
class BizliModuleData extends Controller
{
    ...
    public function Submit_Update_Data(Request $request)
    {
        ...
        if ($db_conn != '') {
            DB::connection($db_conn)->beginTransaction();
            try {
                /* DB::connection($this->db)->table('tt_ordd')->where(['id' => $lineId
                 ])->update(['ordd_cqty' => $confirm_qty, 'ordd_ocat' => $total_amount,
                     'ordd_cpds' => $promo_discount_ratio * $confirm_qty,
                     'ordd_spdc' => $discount_ratio * $confirm_qty,
                     'ordd_dfdc' => $def_discount_ratio * $confirm_qty,]);*/
...
FROM `tm_bizli` t1
  ...
  LEFT JOIN tm_mtyp t7 ON (t1.dlrp_mtyp=t7.id)
           WHERE (t1.dlrp_cusr='$emp_id' OR t3.aemp_mngr='$emp_id')AND t1.dlrp_aprv=0
...
");
    ...
    public function MistiriSearch(Request $request)
    {
        $data1 = array();
        $country = (new Country())->country($request->country_id);
        $db_conn = $country->cont_conn;
        if ($db_conn != '') {
            $data1 = DB::connection($db_conn)->select("
SELECT t1.id AS from_id,t1.dlrp_frno,
  t1.dlrp_name,t1.dlrp_fnam,
  t1.dlrp_bldg,t1.dlrp_aprv,t1.dlrp_mobn,
  t1.dlrp_ocup,t1.dlrp_edob,t1.dlrp_educ,
  t1.dlrp_prad,t1.dlrp_pmad,t1.dlrp_nidn,
  t1.dlrp_expr,t1.dlrp_refn,t1.dlrp_dlcd,
  t1.dlrp_opnm,t1.dlrp_opid,t1.dlrp_orgn
FROM `tm_bizli`t1
WHERE t1.id='$request->From_id';
     ");
    ...
    public function aroundOutlet_UsingSearch(Request $request)
    {
        $data1 = array();
        $country = (new Country())->country($request->country_id);
        $db_conn = $country->cont_conn;
