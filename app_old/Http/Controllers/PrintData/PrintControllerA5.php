...
class PrintControllerA5 extends Controller
{
...
WHERE t1.ordm_ornm = '$order_id' AND t2.ordd_dqty != 0;");
	...
	public function salesInvoicePrint_ou_A5($cont_id, $order_id,$ou_id)
    {
        $country = (new Country())->country($cont_id);
		...
		if($cont_id==14){
          ...
          if($orderLineData){
		    ...
		    return view('PrintData.sales_invoice_print_ou_A5')->with('salesOrder', $orderMasterData)->with('salesOrderLine', $orderLineData);				
		}else{
		$orderMasterData = collect(\DB::connection($country->cont_conn)->select("SELECT
                        t1.ordm_ornm                                          AS Order_ID,
                        DATE_FORMAT(t1.ordm_date, '%b %d, %Y')                AS order_date,
           ...
           // return view('PrintData.sales_invoice_print')->with('salesOrder', $orderMasterData)->with('salesOrderLine', $orderLineData);	
	   	      if($cont_id==15){
			  return view('PrintData.sales_invoice_print_qr_ksa_A5')->with('salesOrder', $orderMasterData)->with('salesOrderLine', $orderLineData);		   	  
		    }else{			 		 
		      return view('PrintData.sales_invoice_print_uae_A5')->with('salesOrder', $orderMasterData)->with('salesOrderLine', $orderLineData);			   		   
	  	         }
...
WHERE t1.rtan_rtnm = '$order_id'"))->first();
  ...
  t3.amim_name                       AS Product_Name,
  t3.amim_olin                       AS sku_print_name,
  floor(t2.rtdd_qnty / t2.rtdd_qnty) AS ctn,
