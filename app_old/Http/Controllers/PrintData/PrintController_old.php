...
WHERE t1.ordm_ornm = '$order_id'");
  ...
  t5.vatn_icnt                                          AS serial_number,
  concat(t5.vatn_year, '-', LPAD(t5.vatn_icnt, 8, '0')) AS vat_sl_number,
...
FROM tt_ordm AS t1
  ...
  LEFT JOIN tm_vati AS t5 ON t1.id = t5.ordm_id
  LEFT JOIN tt_invc AS t6 ON t1.ordm_ornm = t6.invc_code
...
WHERE t1.ordm_ornm = '$order_id'"))->first();
  ...
  t2.ordd_odat                                                      AS Total_Item_Price,
  t2.ordd_uprc * t2.ordd_duft                                       AS Rate,
  ...
  t2.ordd_excs                                                      AS gst,
  t2.ordd_ovat                                                      AS vat,
  t2.ordd_tdis                                                      AS total_discount,
  t2.ordd_tvat                                                      AS total_vat,
  t2.ordd_texc                                                      AS total_gst,
  t2.ordd_amnt                                                      AS net_amount
FROM tt_ordm AS t1
  INNER JOIN tt_ordd AS t2 ON t1.id = t2.ordm_id
  INNER JOIN tm_amim AS t3 ON t2.amim_id = t3.id
