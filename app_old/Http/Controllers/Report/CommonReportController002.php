...
class CommonReportController extends Controller
{
    ...
    public function commonReportFilter99(Request $request){
        ...
        if($time_period){
            if($time_period==0){
                ...
                // $start_date= Carbon::now()->subDays(21)->format('Y-m-d');
        ...
        $data=DB::connection($this->db)->select("SELECT t1.ordm_date, t5.slgp_name, concat(t4.aemp_usnm, ' - ', t4.aemp_name) as user_name,
                  concat(t3.amim_code, ' - ',t3.amim_name) as item_name, t3.amim_duft, round(sum(t2.ordd_qnty)/(t3.amim_duft),2) as order_qnty,
                   round(sum(t2.ordd_opds + t2.ordd_spdi + t2.ordd_dfdo),2) as discount, round(SUM(t2.ordd_oamt),2) as order_amnt FROM `tt_ordm` t1 
                   INNER JOIN tt_ordd t2 ON t1.id=t2.ordm_id INNER JOIN tm_amim t3 ON t2.amim_id=t3.id INNER JOIN tm_aemp t4 ON t1.aemp_id=t4.id 
                   INNER JOIN tm_slgp t5 ON t1.slgp_id=t5.id 
                   WHERE t1.ordm_date BETWEEN '$start_date' AND '$end_date' and t1.slgp_id='$sales_group_id' GROUP BY t2.amim_id, t1.aemp_id");
        return $data;
    }
 ...
 t3.amim_code,t3.amim_name, t1.ordd_qnty, t1.ordd_oamt FROM `tt_ordd` t1 INNER JOIN tt_ordm t2 ON t1.ordm_id=t2.id
...
//All report section start
    public function commonReportFilter(Request $request)
    {
        ...
        if ($reportType == "class_wise_order_report_amt"){
            ...
            $q2='';
            if($dirg_id !=""){
                $q2=" AND t1.zone_id IN (".implode(',',$zone_list).")";
            }
            if ($zone_id != "") {
                $q2 = " AND t1.zone_id = $zone_id";
            }
            if ($sales_group_id != "") {
                $q1= " AND t1.slgp_id = '$sales_group_id'";
            }
            $data=DB::connection($this->db)->select("SELECT 
                    attn_date,slgp_name,dirg_name,zone_name,aemp_usnm,aemp_name,aemp_mob1,start_time,end_time,
                    t4.atyp_name as status
                    FROM 
                    (
                    SELECT t3.attn_date,t4.slgp_name,t7.dirg_name,t6.zone_name,t1.aemp_usnm,t1.aemp_name,t1.aemp_mob1,
                    MIN(t3.attn_time) start_time,MAX(t3.attn_time) end_time,max(t3.atten_atyp) atten_atyp
                    FROM tm_aemp t1
                    LEFT JOIN tt_attn t3 ON t1.id=t3.aemp_id
                    INNER JOIN tm_slgp t4 ON t1.slgp_id=t4.id
                    INNER JOIN tm_zone t6 ON t1.zone_id=t6.id
                    INNER JOIN tm_dirg t7 ON t6.dirg_id=t7.id
                    WHERE t3.attn_date between '$start_date' AND '$end_date' ". $q1 . $q2 . "
                    GROUP BY t3.attn_date,t4.slgp_name,t7.dirg_name,t6.zone_name,t1.aemp_usnm,t1.aemp_name,t1.aemp_mob1
                    Order By t3.attn_date)ab,
                    tm_atyp t4
                    WHERE ab.atten_atyp=t4.id");
            return $data;
        }
            ...
            $data = DB::connection($this->db)->select("SELECT
                    t1.note_date,IF(t7.lfcl_id=2,concat(t7.ntpe_name,'/','InActv'),t7.ntpe_name) as note_type, t2.aemp_usnm, t2.aemp_name, t3.slgp_name, t3.slgp_code,
            ...
            if ($reportType == "sr_activity") {
                ...
                $temp_s_list = "SELECT
                          ...
                          round(ifnull((t1.v_outlet + t1.s_outet)*100/t1.t_outlet,0),2) AS c_percentage,
                ...
                FROM (SELECT
                  ...
                  (SELECT
                   ...
                   GROUP BY  t1.slgp_id) t4 ON t1.slgp_id = t4.slgp_id";
                    ...
                    $temp_s_list="SELECT t1.dsct_name,t1.than_name,t1.ward_name,
...
// All report section end here
    ...
    public function getTrackingRecordGvt(Request $request){
        ...
        //     $temp_v=$slgp_id[$i];
        //     $slgp_name=$slgp[$i]->slgp_name;
        //     $q2.=",sum(if(t2.slgp_id=$temp_v,t1.slgp_out,0))`slgpOlt$i`,sum(if(t1.slgp_id=$temp_v,t2.visit,0))`slgpv$i`,sum(if(t1.slgp_id=$temp_v,t2.memo,0))`slgpm$i`";         
        //     $q1.=",`slgpOlt$i`,`slgpv$i`,`slgpm$i` ";
        // }

        $q2.=",sum(if(t2.slgp_id=$slgp_id,t2.visit,0))slgpv,sum(if(t2.slgp_id=$slgp_id,t2.memo,0))slgpm";         
      //  $q1.=",slgpOlt,slgpv,slgpm";
       
       
        $data=DB::connection($this->db)->select("SELECT p2.disn_id,p.disn_name,p2.total_outlet ,stt.slgp_out slgpOlt,slgpv,slgpm FROM 
            (
            SELECT p.disn_id,p.disn_name ,slgpv,slgpm
            FROM
                (SELECT t2.disn_id,t2.disn_name ".$q2."
                FROM tbl_slgp_vsmo t2 WHERE ssvh_date between '$start_date' AND '$end_date'
                    GROUP BY t2.disn_id,t2.disn_name ORDER BY t2.disn_name asc
                )p
                )p,
            (
            SELECT t4.id disn_id,sum(slgp_out) slgp_out FROM tbl_mktm_slgp_wise_outlet t1 INNER JOIN tbl_mktm_wise_total_outlet t2 ON t1.mktm_id=t2.mktm_id
            INNER JOIN tm_dsct t3 On t2.dsct_id=t3.id INNER JOIN tm_disn t4 ON t3.disn_id=t4.id where t1.slgp_id=$slgp_id group by t4.id
            )stt,
    ...
    public function getGvtDeeperSalesData(Request $request){
        ...
        if($stage==-1){
            ...
            $data=DB::connection($this->db)->select(" SELECT p2.dsct_id,p.dsct_name,p2.total_outlet,stt.slgp_out slgpOlt,slgpv,slgpm FROM 
            (
            SELECT p.dsct_id,p.dsct_name ,slgpv,slgpm
            FROM
            ...
            (
                SELECT t3.id dsct_id,sum(slgp_out) slgp_out FROM tbl_mktm_slgp_wise_outlet t1 INNER JOIN tbl_mktm_wise_total_outlet t2 ON t1.mktm_id=t2.mktm_id
                INNER JOIN tm_dsct t3 On t2.dsct_id=t3.id INNER JOIN tm_disn t4 ON t3.disn_id=t4.id where t1.slgp_id=$slgp_id AND t4.id=$id group by t3.id
            )stt,
            (SELECT t1.dsct_id,sum(total_outlet)total_outlet FROM tbl_mktm_wise_total_outlet t1 group by t1.dsct_id
             )p2
            WHERE p.dsct_id=p2.dsct_id AND stt.dsct_id=p.dsct_id;");
        }
        else if($stage==1){
            $data=DB::connection($this->db)->select(" SELECT p2.than_id,p.than_name,p2.total_outlet,stt.slgp_olt slgpOlt,slgpv,slgpm FROM 
            (
            SELECT p.than_id,p.than_name ,slgpv,slgpm
            FROM
            ...
            (
            SELECT t1.slgp_id,t2.than_id,sum(slgp_out) slgp_olt FROM tbl_mktm_slgp_wise_outlet t1 INNER JOIN tbl_mktm_wise_total_outlet t2 ON t1.mktm_id=t2.mktm_id
              where t1.slgp_id=$slgp_id AND t2.dsct_id=$id group by t2.than_id,t1.slgp_id
            )stt,
            (SELECT t1.than_id,sum(total_outlet)total_outlet FROM tbl_mktm_wise_total_outlet t1 where t1.dsct_id=$id group by t1.than_id
             )p2
            WHERE p.than_id=p2.than_id AND stt.than_id=p.than_id;");
        }
        else if($stage==2){
            $data=DB::connection($this->db)->select("SELECT p2.ward_id,p.ward_name,p2.total_outlet,stt.slgp_olt slgpOlt,slgpv,slgpm FROM 
            (
            SELECT p.ward_id,p.ward_name ,slgpv,slgpm
            FROM
            ...
            (
            SELECT t1.slgp_id,t2.ward_id,sum(slgp_out) slgp_olt FROM tbl_mktm_slgp_wise_outlet t1 INNER JOIN tbl_mktm_wise_total_outlet t2 ON t1.mktm_id=t2.mktm_id
              where t1.slgp_id=$slgp_id AND t2.than_id=$id group by t2.ward_id,t1.slgp_id
            )stt,
            (SELECT t1.ward_id,sum(total_outlet)total_outlet FROM tbl_mktm_wise_total_outlet t1 where t1.than_id=$id group by t1.ward_id
             )p2
            WHERE p.ward_id=p2.ward_id AND stt.ward_id=p.ward_id
                ");
        }
        else if($stage==3){
            $data=DB::connection($this->db)->select("SELECT p2.mktm_id,p.mktm_name,p2.total_outlet,stt.slgp_olt slgpOlt,slgpv,slgpm FROM 
            (
            SELECT p.mktm_id,p.mktm_name ,slgpv,slgpm
            FROM
            ...
            (
            SELECT t1.slgp_id,t2.mktm_id,sum(slgp_out) slgp_olt FROM tbl_mktm_slgp_wise_outlet t1 INNER JOIN tbl_mktm_wise_total_outlet t2 ON t1.mktm_id=t2.mktm_id
              where t1.slgp_id=$slgp_id AND t2.ward_id=$id group by t2.mktm_id,t1.slgp_id
            )stt,
            (SELECT t1.mktm_id,sum(total_outlet)total_outlet FROM tbl_mktm_wise_total_outlet t1 where t1.ward_id=$id group by t1.mktm_id
             )p2
    ...
    public function getSRList(Request $request){
        ...
        $data=DB::connection($this->db)->select("SELECT t1.aemp_usnm,t1.id,t1.aemp_name from tm_aemp t1
                ...
                where t1.slgp_id=$slgp_id ".$q2.$q1. " AND t1.role_id=1
                GROUP BY t1.id,aemp_name");
        return $data;
    ...
    public function getHistoryReportData(Request $request){
        ...
        ifnull(t4.rout_name,'N/A') rout_id,ifnull(t3.site,0) rout_olt,
        ...
        LEFT JOIN tbl_rout_site_count t3 ON t3.rpln_day=DAYNAME(t2.dhbd_date) AND t3.aemp_id=t2.aemp_id
        LEFT join tm_rout t4 ON t3.rout_id=t4.id
        LEFT JOIN tl_rsmp t5 ON t4.id=t5.rout_id
        LEFT JOIN (SELECT ssvh_date,site_id,aemp_id from th_ssvh where ssvh_date between'$start_date' AND '$end_date' and ssvh_ispd in (1,0) and aemp_id=$id group by ssvh_date,site_id,aemp_id)t6
        ON t5.site_id=t6.site_id AND t2.dhbd_date=t6.ssvh_date
        WHERE t1.id=$id AND t2.dhbd_date between'$start_date' AND '$end_date'
        GROUP BY t2.dhbd_date,t1.id,t1.aemp_usnm,t1.aemp_name, t2.dhbd_tvit,t2.dhbd_memo,t4.rout_name,t3.site,
        t2.dhbd_tsit,t2.dhbd_line,t2.dhbd_tamt");
        // $data=DB::connection($this->db)->select("SELECT 
        //         t2.dhbd_date,t1.id,t1.aemp_usnm,t1.aemp_name,
        //         ifnull(t4.rout_name,'N/A') rout_id,ifnull(t3.site,0) rout_olt,
        //         t2.dhbd_tvit t_visit,t2.dhbd_memo t_memo,count(DISTINCT t6.site_id) rout_visit,
        //         format(ifnull((t2.dhbd_memo / t2.dhbd_tvit) * 100, 0), 2) AS strikeRate,
        //         round(ifnull(t2.dhbd_line / t2.dhbd_memo,0), 2)                     AS lpc,
        //         round(t2.dhbd_tamt/1000	,2)									 AS t_amnt,
        //         sum(if(a.site_id=t5.site_id,1,0)) rs_memo
        //         FROM tm_aemp  t1
        //         INNER JOIN th_dhbd_5 t2 ON t2.aemp_id=t1.id
        //         LEFT JOIN tbl_rout_site_count t3 ON t3.rpln_day=DAYNAME(t2.dhbd_date) AND t3.aemp_id=t2.aemp_id
        //         LEFT join tm_rout t4 ON t3.rout_id=t4.id
        //         LEFT JOIN tl_rsmp t5 ON t4.id=t5.rout_id
        //         LEFT JOIN (SELECT site_id,aemp_id from th_ssvh where ssvh_date between'$start_date' AND '$end_date' and ssvh_ispd in (1,0) and aemp_id=$id group by site_id,aemp_id)t6
        //         ON t5.site_id=t6.site_id
        //         LEFT JOIN(SELECT t1.aemp_id,t1.site_id FROM tt_ordm t1 WHERE t1.ordm_date between'$start_date' AND '$end_date' AND t1.aemp_id=$id)a
        //         ON t3.aemp_id=a.aemp_id AND t5.site_id
        //         WHERE t1.id=$id AND t2.dhbd_date between'$start_date' AND '$end_date'
        //         GROUP BY t2.dhbd_date,t1.id,t1.aemp_usnm,t1.aemp_name, t2.dhbd_tvit,t2.dhbd_memo,t4.rout_name,t3.site,
        //         t2.dhbd_tsit,t2.dhbd_line,t2.dhbd_tamt");
        return $data;
    ...
    public function getMapData(Request $request){
        ...
        UNION ALL
        (SELECT '' site_code,'0' site_name,t1.hloc_addr 'site_adrs',max(t1.geo_lat),max(t1.geo_lon),min(t1.hloc_time) log_time,'loc' v_type FROM th_hloc t1
        WHERE t1.aemp_id=$sr_id and t1.hloc_date='$date'  group by t1.hloc_addr,site_name ORDER BY t1.hloc_time asc))p 
