...
{
    private $access_key = 'GroupStatusController';
    private $currentUser;
    private $userMenu;
    ...
    public function summary()
    {
        if ($this->userMenu->wsmu_vsbl) {
            ...
            return view('report.group_status.summary')->with('permission', $this->userMenu);
        } else {
...
FROM tbld_employee AS t1 INNER JOIN tbld_sales_group_employee AS t2 ON t1.id = t2.emp_id
  ...
  LEFT JOIN (SELECT
               t3.id                   AS sales_group_id,
               t1.emp_id               AS emp_id,
               count(DISTINCT t1.date) AS att_day
             FROM tblt_attendance AS t1
               INNER JOIN tbld_sales_group_employee AS t2 ON t1.emp_id = t2.emp_id
               INNER JOIN tbld_sales_group AS t3 ON t2.sales_group_id = t3.id
            ...
            ) AS t4 ON t3.id = t4.sales_group_id AND t1.id = t4.emp_id
  LEFT JOIN (SELECT
...
/*LEFT JOIN (SELECT
            ...
            return view('report.group_status.active_summary_details')->with("att_data", $data)->with('permission', $this->userMenu);
       } else {
         return view('theme.access_limit');
       }
...
GROUP BY t1.id,t3.id,t3.name,t5.email,t1.name,t6.name");
            ...
            return view('report.group_status.inactive_summary_details')->with("att_data", $data)->with('permission', $this->userMenu);
       } else {
         return view('theme.access_limit');
       }
