    ...
    public function promotionNewCreate()
    {
        if ($this->userMenu->wsmu_crat) {
            ...
            $salesGroups = DB::connection($this->db)->select("SELECT
                    ...
                    ORDER BY t2.id, t2.slgp_name");
            $zones = DB::connection($this->db)->select("SELECT `id`,`zone_name`,`zone_code` FROM `tm_zone` WHERE 1");
            return view('Promotion.create2')->with("salesGroups", $salesGroups)->with("zones", $zones)->with("add_salesGroups", $salesGroups)->with('permission', $this->userMenu);
        } else {
            return redirect()->back()->with('danger', 'Access Limited');
        }
 ...
 '$slgp_idds','$order_qualifier','$offer_category', '2', '1','$cuser', '$uuser','1')";*/
        ...
        if ($item_type[0] == 'Quantity') {
            for ($i = 0; $i < $no_of_slab; $i++) {
                ...
                /*$sql2[] = "INSERT INTO `tm_prsb`(`prmr_id`, `prsb_text`, `prsb_fqty`, `prsb_tqty`, `prsb_famn`, `prsb_tamn`, `prsb_qnty`,
 `prsb_disc`, `prsb_modr`, `prsb_mosl`, `cont_id`, `lfcl_id`, `aemp_iusr`, `aemp_eusr`) VALUES ('1','$slab_text[$i]','$min_item_qty[$i]','0',
 '0','0','$offer_item_qty[$i]','','1',$i++,'2','1','$cuser', '$uuser')";*/
            }
        } else {
            ...
            $sql2[] = ['prmr_id' => $p_id, 'prsb_text' => $slab, 'prsb_fqty' => 0, 'prsb_tqty' => 0,
                ...
                /*$sql2[] = "INSERT INTO `tm_prsb`(`prmr_id`, `prsb_text`, `prsb_fqty`, `prsb_tqty`, `prsb_famn`, `prsb_tamn`, `prsb_qnty`,
 `prsb_disc`, `prsb_modr`, `prsb_mosl`, `cont_id`, `lfcl_id`, `aemp_iusr`, `aemp_eusr`) VALUES ('1','$slab_text[$i]','0','0',
 '$min_item_qty[$i]','0','0','$offer_item_qty[$i]','1',$i++,'2','1','$cuser', '$uuser')";*/
            }
        }
