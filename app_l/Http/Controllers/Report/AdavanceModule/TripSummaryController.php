...
{
    private $access_key = 'attendance_report';
    private $currentUser;
    private $userMenu;
...
WHERE t2.emp_id =$emp_id and t1.country_id= $country_id ORDER BY t1.id ASC ");
            ...
            return view('report.attendance.summary')->with("emps", $emp)->with("userDepartments", $userDepartment)->with("departments", $department)->with("userSaleGroups", $userSaleGroups)->with("saleGroups", $saleGroups)->with('permission', $this->userMenu);
        } else {
...
ORDER BY t1.date ASC");
            ...
            return view('report.attendance.summary_map')->with("attendances", $data)->with('permission', $this->userMenu);
        } else {
            return view('theme.access_limit');
        }
...
ORDER BY t1.date ASC");
            ...
            foreach ($data as $index=>$data1){
                $address="";
                $geocodeFromLatLong = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?latlng=' . trim($data1->lat) . ',' . trim($data1->lon) . '&sensor=true&key=AIzaSyAayiiP65gIAzdli9f9Z4OprlGve7L4lOw');
...
ORDER BY t1.date ASC");
    ...
    public function filterAttendanceSymmary(Request $request)
    {
        $country_id = $this->currentUser->employee()->cont_id;
        $where = "1 and t1.country_id=$country_id";
        if ($request->start_date != "" && $request->end_date != "") {
...
t3.email as user_name,
  ...
  t2.name,
  t1.date,
  count(t1.emp_id)                                     AS data_count,
  DATE_FORMAT(min(t1.date_time), '%d/%m/%Y : %l.%i%p') AS start_time,
  DATE_FORMAT(max(t1.date_time), '%d/%m/%Y : %l.%i%p') AS end_time,
  t4.name as role_id,
  group_concat(concat(t1.lat, '_', t1.lon))            AS location
FROM tblt_attendance AS t1
  INNER JOIN tbld_employee AS t2 ON t1.emp_id = t2.id  
  INNER JOIN users as t3 ON t2.user_id=t3.id
  INNER JOIN tbld_role as t4 ON t2.role_id=t4.id
 ...
 LEFT JOIN tbld_sales_group_employee as t6 ON t1.emp_id=t6.emp_id
WHERE  $where
GROUP BY t1.emp_id, t1.date,t2.id,t2.name,t3.email,t4.name
ORDER BY min(t1.date_time) ASC");
        foreach ($data as $index=>$data1){
            $address="";
            $myArray = explode(',', $data1->location);
            foreach ( $myArray as $index1=>$loc){
                $myArray = explode('_', $loc);
                $geocodeFromLatLong = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?latlng=' . trim($myArray[0]) . ',' . trim($myArray[1]) . '&sensor=true&key=AIzaSyAayiiP65gIAzdli9f9Z4OprlGve7L4lOw');
                $output = json_decode($geocodeFromLatLong);
                $status = $output->status;
                $address .= ($index1+1).". ";
                $address .= ($status == "OK") ? $output->results[0]->formatted_address."</br>" : '';
            }
            $data[$index]->address=$address;
        }
