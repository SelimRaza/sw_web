...
class ProductClassController extends Controller
{
    ...
    public function productClassFormatGen(Request $request)
    {
        //echo "afdsaf";

        if ($this->userMenu->wsmu_crat) {
            return Excel::download(new ProductClass(), 'product_class_format_' . date("Y-m-d H:i:s") . '.xlsx');
        } else {
            return redirect()->back()->with('danger', 'Access Limited');
        }
    }
    ...
    public function productClassUploadInsert(Request $request)
    {
        if ($this->userMenu->wsmu_crat) {
            if ($request->hasFile('import_file')) {
                DB::connection($this->db)->beginTransaction();
                try {
                    Excel::import(new ProductClass(), $request->file('import_file'));
