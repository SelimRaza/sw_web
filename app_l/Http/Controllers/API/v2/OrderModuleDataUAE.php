...
class OrderModuleDataUAE extends Controller
{
        ...
        if ($country != false) {
            ...
            try {
                ...
                if ($request->Order_Type == 'Productive') {
                    ...
                    // $otyp_id = $request->otyp_id == "" ? 1 : $request->otyp_id;
	...
	public
    function SubmitSiteWiseInvoiceGrvDelivery_vat_dis(Request $request)
    {
                ...
                try {
                    ...
                    if ($request->type_id == 1) {
                        ...
                        foreach($outletData_child as $orderLineData) {
                            ...
                            $amount = $orderLineData->Dev_QNTY * $orderLineData->Rate;
							$excise_amt= round((($orderLineData->Dev_QNTY * $orderLineData->Rate)*$orderLineData->gst_percent)/100,3);
							$vat_amt= round((((t4.ORDD_UPRC*t4.DELV_QNTY)-($orderLineData->DISCOUNT)+$excise_amt)*$orderLineData->vat_percent)/100,3);
                            $Net_amt = round(($amount+$excise_amt+$vat_amt-$orderLineData->DISCOUNT),3);
                            $Net_DLV_amt += $Net_amt;
                            $sql2 .= " Update tt_ordd set ordd_dqty=$orderLineData->Dev_QNTY,ordd_odat=$Net_amt  WHERE id=$orderLineData->trp_line; ";
                            $sql3 .= " Update dm_trip_detail set DELV_QNTY=$orderLineData->Dev_QNTY,DISCOUNT=$orderLineData->DISCOUNT,ORDD_OAMT=$Net_amt,TRIP_STATUS='D',ORDM_DRDT='$DRDT' WHERE OID=$orderLineData->trp_line;";
                        }

                          //$sql4 = " Update dm_trip_master set ORDM_DRDT='$DRDT',IBS_INVOICE='$IBS_INVOICE',DELV_AMNT=round($request->DELV_AMNT,$round_digit),DELIVERY_STATUS=11 WHERE ORDM_ORNM='$request->ORDM_ORNM';";
                          $sql4 = " Update dm_trip_master set ORDM_DRDT='$DRDT',IBS_INVOICE='$IBS_INVOICE',DELV_AMNT=round($Net_DLV_amt,3),DELIVERY_STATUS=11 WHERE ORDM_ORNM='$request->ORDM_ORNM';";
                          $sql8 = " Update dm_trip_detail set TRIP_STATUS='D',IBS_INVOICE='$IBS_INVOICE' WHERE ORDM_ORNM='$request->ORDM_ORNM' ; ";
                          } else {
                        $now = now();
                        $dm_ACMP_WH_ID_DM_CODE = DB::connection($db_conn)->table('dm_trip_master')->where(['TRIP_NO' => $request->tpm_code])->first();
                        ...
                        foreach ($outletData_child as $orderLineData) {
							...
							$vat_amt= round((((t4.ORDD_UPRC*t4.DELV_QNTY)-($orderLineData->DISCOUNT)+$excise_amt)*$orderLineData->vat_percent)/100,3);
                            $Net_amt = round(($amount+$excise_amt+$vat_amt-$orderLineData->DISCOUNT),3);
                            $Net_GRV_DLV_amt += $Net_amt;
							
                            $sql7 .= " Update tt_rtdd set rtdd_dqty='$orderLineData->Dev_QNTY',rtdd_damt='$Net_amt' WHERE id=$orderLineData->trp_line; ";
                        }
						$sql6 = "INSERT INTO `dm_collection` (`ID`, `ACMP_CODE`, `COLL_DATE`, `COLL_NUMBER`, 
                                `IBS_INVOICE`, `AEMP_ID`, `AEMP_USNM`, `DM_CODE`, `WH_ID`, `SITE_ID`, `SITE_CODE`, `COLLECTION_AMNT`,
                                `COLL_REC_HO`, `COLLECTION_TYPE`, `STATUS`, `CHECK_NUMBER`, `INVT_ID`,slgp_id)
                                 VALUES (NULL,  '$dm_ACMP_WH_ID_DM_CODE->ACMP_CODE', '$DRDT', '$COLL_NUMBER', 'N', $request->emp_id,
                                  '$request->emp_code', '$dm_ACMP_WH_ID_DM_CODE->DM_CODE', '$dm_ACMP_WH_ID_DM_CODE->WH_ID',
                                  '$request->site_id', '$request->site_code',  round($Net_GRV_DLV_amt,3),0, 'GRV', 11, 'N', 5,$request->slgp_id); ";
    ...
    function SubmitSiteWiseInvoiceCollection(Request $request)
    {
        ...
        //
		 ...
		 if (($request->ex_st_type == 43 OR $request->ex_st_type == 40)&& $request->ex_st_amt > 20) {
			return array(
                            'success' => 0,
                            'message' => 'Collection Failed! Excess amt more than 20 !',
                        ); 
		 }
            ...
            if ($orderSyncLog == null) {
                ...
                try {
                   ...
                   // $order_amount = array_sum(array_column($orderLines, 'total_price'));
					 ...
					 // $order_amount = array_sum(array_map(function($orderLine) {
                        // return $orderLine['P_Qty'] * $orderLine['Rate']+$orderLine['excise'] * $orderLine['vat'];
                    // }, $orderLines));
					
					$rates = array_column($orderLines, 'Rate');
					$qtys = array_column($orderLines, 'P_Qty');
					
					$excises = array_column($orderLines, 'excise');
					$vats = array_column($orderLines, 'vat');

					$order_amount = array_sum(array_map(function ($rate, $qty,$excise,$vat) {
					return $rate * $qty+$excise+$vat;
						}, $rates, $qtys,$excises,$vats));
                    ...
                    foreach ($orderLines as $orderLineData) {
                      ...
                      // $orderLine->ordd_amnt = $orderLineData->total_price;
                        ...
                        '$request->emp_code','$request->depo_id','$request->site_id','$request->site_code',$orderLineData->P_ID,'$orderLineData->product_code',
                         ...
                         0,$orderLineData->Rate,$orderLineData->excise_percent,$orderLineData->vat_percent,$order_amount_new,$orderLineData->promo_ref,
