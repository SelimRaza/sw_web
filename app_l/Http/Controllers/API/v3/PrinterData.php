...
class PrinterData extends Controller
{
  ...
  concat(t7.vatn_year, '-', LPAD(t1.acmp_id, 2, '0'), '-', LPAD(t7.vatn_icnt, 8, '0')) AS vat_invoice,
...
FROM tt_ordm AS t1
  ...
  INNER JOIN tt_invc AS t6 ON t1.ordm_ornm = t6.invc_code
  INNER JOIN tm_vati AS t7 ON t1.id = t7.ordm_id
