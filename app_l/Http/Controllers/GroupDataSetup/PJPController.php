...
use App\MasterData\BulkSiteUpload;
...
FROM tm_rout AS t1
  where t1.cont_id = $country_id");
    ...
    public function bulkSiteAdd(Request $request,$id){
        if ($this->userMenu->wsmu_updt) {
            if ($request->hasFile('bulk_site_file')) {
                DB::beginTransaction();
                try {
                    Excel::import(new BulkSiteUpload($id), $request->file('bulk_site_file'));
                    DB::commit();
