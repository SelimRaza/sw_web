            ...
            <div class="page-title">
                <div class="title_left">
                    <ol class="breadcrumb">
                        ...
                        <li class="active">
                            <strong>All No Order Reason</strong>
                        </li>
                    ...
                    <div class="x_panel">
                        <div class="x_title">
                            @if($permission->wsmu_crat)
                                ...
                                <a class="btn btn-success btn-sm" href="{{ URL::to('/no_order_reason/create')}}"><span
                                            class="fa fa-plus-circle" style="color: white; font-size: 1.3em"></span>&nbsp;&nbsp;<b>Add
                                        New</b></a>
                            @endif
                            <button class="btn btn-danger btn-sm" onclick="exportTableToCSV('No_order_reason_<?php echo date('Y_m_d'); ?>.csv','datatables')"
                            ...
                            <table id="datatables" class="table search-table font_color">
                                ...
                                @foreach($data as $index=>$data1)
                                    <tr class="tbl_body_gray">
                                        ...
                                        <td>{{$data1->nopr_name}}</td>
                                        <td>{{$data1->nopr_code}}</td>

                                        <td>
                                            @if($permission->wsmu_delt)
                                                <form style="display:inline"
                                                      action="{{route('no_order_reason.destroy',$data1->id)}}"
                                        ...
                                        <td>
                                            @if($permission->wsmu_read)
                                                <a href="{{route('no_order_reason.show',$data1->id)}}"
                                                   class="btn btn-primary btn-xs"><i class="fa fa-search"></i> View
                                                </a>&nbsp;|&nbsp;
                                            @endif
                                            @if($permission->wsmu_updt)
                                                <a href="{{route('no_order_reason.edit',$data1->id)}}"
