                    ...
                    <div class="x_panel">
                        <div class="x_title">
                            @if($permission->wsmu_crat)
                                <a class="btn btn-success btn-sm" href="{{ URL::to('/company/create')}}">Add New</a>
                            @endif
                            ...
                            <table id="datatable" class="table table-bordered projects" data-page-length='50'>
                                ...
                                @foreach($companies as $index=>$company)
                                    <tr>
                                        <td>{{$index+1}}</td>
                                        <td>{{$company->id}}</td>
                                        <td>{{$company->acmp_name}}</td>
                                        <td>{{$company->acmp_code}}</td>
                                        <td>
                                            ...
                                            @endif
                                            @if($permission->wsmu_updt)
                                                <a href="{{ URL::to('company/employee/'.$company->id)}}"
                                                   class="btn btn-info btn-xs"><i class="fa fa-pencil"></i>Employee Mapping
                                                </a>
                                            @endif
                                            @if($permission->wsmu_updt)
                                                <a href="{{ URL::to('companySiteMapping')}}"
