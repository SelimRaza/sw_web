            ...
            <div class="page-title">
                <div class="title_left">
                    <ol class="breadcrumb">
                        ...
                        <li class="active">
                            <strong>All Channel</strong>
                        </li>
                        @if($permission->wsmu_crat)
                            <li class="label-success">
                                <a href="{{ URL::to('/channel/create')}}">New Channel</a>
                            </li>
                        @endif
                    </ol>
                </div>
                        ...
                        <div class="x_content">
                            ...
                            <table class="table table-striped projects">
                                ...
                                @foreach($channels as $index=>$channel)
                                    <tr>
                                        <td>{{$index+1}}</td>
                                        <td>{{$channel->dsct_name}}</td>
                                        <td>{{$channel->dsct_code}}</td>
                                        <td>
                                            ...
                                            @if($permission->wsmu_delt)
                                                <form style="display:inline"
                                                      action="{{route('channel.destroy',$channel->id)}}"
                                                      class="pull-xs-right5 card-link" method="POST">
                                                    {{csrf_field()}}
                                                    {{method_field('DELETE')}}
                                                    <input class="btn btn-danger btn-xs" type="submit"
                                                           value="<?php echo $channel->status_id == 1 ? 'Active' : 'Inactive'?>"
