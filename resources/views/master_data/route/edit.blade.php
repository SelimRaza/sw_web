...
@section('content')
                    ...
                    <div class="x_panel">
                        ...
                        <div class="x_content">
                            ...
                            <form class="form-horizontal form-label-left"
                                  action="{{route('route.update',$route->id)}}" enctype="multipart/form-data"
                                  method="post">
                                {{csrf_field()}}
                                {{method_field('PUT')}}
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <select class="form-control" name="acmp_id" id="acmp_id"
                                            ...
                                            @foreach ($companies as $companies1)
                                                <option value="{{$companies1->acmp_id}}">{{$companies1->acmp_name}} </option>
                                            @endforeach
    ...
    <script>
        $(document).ready(function () {
            const base_id = '{{ $route->base_id }}';
            const acmp_id = '{{ $route->acmp_id }}';
            if (base_id !== '') {
                $('#base_id').val(base_id);
            }
            if (acmp_id !== '') {
