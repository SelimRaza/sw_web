...
@section('content')
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="x_panel"><div class="x_title">
                            <center><strong> ::: Item Search :::
                                </strong></center>
                            <div class="clearfix"></div>
                        </div>
                        <div class="x_content">
                            <div class="col-md-offset-3 col-sm-offset-3 col-md-7 col-sm-7 col-xs-12">
                                ...
                                <div class="item form-group col-md-12 col-sm-12">
                                    ...
                                    <div class="col-md-5 col-sm-5 col-xs-12">
                                        <input id="search_text" name="search_text" class="form-control col-md-7 col-xs-12 in_tg"
                                               style=" border-radius: 0; border-top-left-radius: 3px; border-bottom-left-radius: 3px;"
                                               placeholder="Item name or Code"
                                               type="text" value="{{ old('search_text') }}">
                                    </div>
                                    <div class="col-md-1 col-sm-1 col-xs-12">
                                        <button class="btn btn-success" onclick="loadItem()" style="float:right; margin-right: 0px;
                                        border-radius: 0; border-top-right-radius: 3px; border-bottom-right-radius: 3px;">Load Item</button>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-offset-2 col-sm-offset-2 col-md-8 col-sm-8 col-xs-12" style="margin-top: .75rem; padding: 0;text-align: left">

                                <h4 class="text-center">Items</h4>

                                <div id="other-items">
                                    <table class="table table-responsive table-borderless" style="border: none; padding-top: 10px; padding-left: 5px; margin: 0">
                                        <thead style="position: sticky; top: 0px;">
                                        <tr class="tbl_header_light">
                                            <th style="width: 45% !important; text-align: left">Item</th>
                                            <th style="width: 15% !important;">Pcs</th>
                                        ...
                                        <tfoot style=" position: sticky;
                                                           ...
                                                           height: initial;
                                                           background: white;">
                                        <tr id="other-item-footer" style="display: none">
                                            <td ></td>
                                            <td ></td>
                                            <td style="vertical-align: middle;">Total</td>
                                            <td style="padding: 1.5px;">
    ...
    <style>
        ...
        .fa-times-circle-o:hover, .fa-eye:hover{
            cursor: pointer;
        }
    </style>
    <script type="text/javascript">
                    ...
                    success:function(data){
                        ...
                        if(data){
                            for(var i=0;i<data.length;i++){
                                html+=`<option value="${data[i].id}:${data[i].amim_code}:${data[i].amim_name}">${data[i].amim_code}-${data[i].amim_name}</option>`;
                            }
                        }
        ...
        function itemPrice(){
            let amim = $('#amim_id').select2('data')[0]
            let amim_info = amim.id
            let infos = amim_info.split(':')
            let amim_id = infos[0]
            let amim_code = infos[1]
            let amim_name = infos[2]
            let item_id_exist = true
            let country_id = $('#country_id').val()

            $('.added-other-items-id').each(function() {
                if($(this).val() === amim_id)
                {
                    item_id_exist = false
                }
            ...
            if(amim_id != '' && item_id_exist && country_id != ''){
                $('#ajax_load').css('display','block');
                $.ajax({
                    type:"POST",
                    url:"{{URL::to('/')}}/itemPrice",
                    data: {
                        country_id: country_id,
                        amim_id: amim_id,
                        _token: token,
                    ...
                    success:function(data){
                        ...
                        if(data.length > 0){
                            let amim_price = data[0].amim_price
                            let amim_ctn_size = data[0].amim_ctn_size

                            let other_item_lists = $('#other-items-list')

                            let items = `<tr>
                                <td>
                                    ${amim_code}-${amim_name}
                                    <input type="hidden" value="${amim_id}" class="added-other-items-id"  id="other-item-id-${amim_id}" name="item_ids[]">
                                    <input type="hidden" value="${amim_price}" name="item_unit_prices[]"  id="other-item-price-${amim_id}">
                                    <input type="hidden"  name="item_dufts[]" value="${amim_ctn_size}" id="ctn-size-${amim_id}">
                                </td>
                                <td>
                                    <input type="hidden" id="total-item-${amim_id}" name="total_qtys[]">
