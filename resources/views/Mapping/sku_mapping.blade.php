        ...
        <div class="">
            <div class="page-title">
                ...
                <div class="title_right">
                    <a href="{{ URL::to('/sku-group-mapping')}}" class="btn btn primary" type="submit" style="text-decoration:underline;color:darkred;" target="_blank"> Sku-Group</a>
                    <a href="{{ URL::to('/sku-plmt-mapping')}}" class="btn btn primary" type="submit" style="text-decoration:underline;color:darkred;" target="_blank"> Sku-PriceList</a>
                </div>
                        ...
                        <div class="x_content">
                            <div class="col-md-6 col-sm-12 col-xs-12">
                                ...
                                <table class="table table-striped table-bordered">
                                    ...
                                    <tbody id="cont">
                                    @foreach($data1 as $i=>$dt)
                                        <tr>
                                            <td>{{$i+1}}</td>
                                            <td>{{$dt->amim_code}}</td>
                                            <td>{{$dt->amim_name}}</td>
                                            <td>{{$dt->slgp_code}}</td>
                                            <td><span >{{$dt->slgp_name}}</span></td>
                                            <td></td>
                                        </tr>
                                    @endforeach
                                    </tbody>
    ...
    <script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (let i = 0; i < data2.length; i++) {
                        ...
                        html2+= "<a target='_blank' href='{{ URL::to('/')}}/view/plmt-item/" + data2[i].amim_id + "/" + data2[i].plmt_id + "' class='btn btn-info btn-xs'><i class='fa fa-eye'>View</i>  </a></td></tr>";
       ...
       function removeItemFromPriceList(v){
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type:"GET",
                    url: "{{ URL::to('/')}}/removeItemFromPriceList/"+id,
                    dataType: "json",
                    success:function(data){
                        obj.remove();
                    },
       ...
       function removeItemFromSlgp(v){
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type:"GET",
                    url: "{{ URL::to('/')}}/removeItemFromSlgp/"+id,
                    dataType: "json",
                    success:function(data){
                        obj.remove();
                    },
