...
@section('content')
                ...
                <div class="col-md-12">
                    <div class="x_panel">
                        <form id="demo-form2" data-parsley-validate
                              ...
                              action="{{ URL::to('grv_report/pushToRoutePlan')}}" enctype="multipart/form-data"
                              method="post">
                            {{csrf_field()}}
                            {{method_field('POST')}}
    ...
    <script type="text/javascript">
        ...
        function filterData() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ URL::to('/')}}/grv_report/filterGRVSummary",
                data: {
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        html += "<td><a target='_blank' href='{{ URL::to('/')}}/printer/return/" + data[i].cont_id + "/" + data[i].order_id + "' class='btn btn-info btn-xs'><i class='fa fa-pencil'></i> Return Print </a></td>";

                        if (data[i].status_id == 11) {
                            html += "<td><a target='_blank' href='{{ URL::to('/')}}/printer/returnInvoice/" + data[i].cont_id + "/" + data[i].order_id + "' class='btn btn-info btn-xs'><i class='fa fa-pencil'></i> Return Invoice Print </a></td>";
                        }
                        html += '</tr>';
                        count++;
                    }
