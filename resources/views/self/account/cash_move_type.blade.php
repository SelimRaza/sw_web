                        ...
                        <div class="x_content">
                            ...
                            <table class="table table-striped projects">
                                ...
                                @foreach($cashMoveTypes as $cashMoveType)
                                    <tr>
                                        <td>{{$cashMoveType->id}}</td>
                                        <td>{{$cashMoveType->name}}</td>
                                        <td>{{$cashMoveType->code}}</td>
                                        <td>{{$cashMoveType->cashType()->name}}</td>
                                        <td>
                                            @if($permission->wsmu_delt)
                                                <form style="display:inline"
                                                      action="{{ URL::to('self_account/cash_move_type_delete/'.$cashMoveType->id)}}"
                                                      class="pull-xs-right5 card-link" method="GET">
                                                    {{csrf_field()}}
                                                    {{method_field('DELETE')}}
                                                    <input class="btn btn-danger btn-xs" type="submit"
                                                           value="<?php echo $cashMoveType->status_id == 1 ? 'Inactive' : 'Active'?>"
                                                    >
                                                    </input>
                                                </form>
                                            @endif
                    ...
                    <div class="x_panel">
                        ...
                        <div class="x_content">
                            ...
                            <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left"
                                  action="{{ URL::to('self_account/cash_move_type_add/'.$selfAccount->id)}}"
                                  method="GET">
                                {{csrf_field()}}
                                {{method_field('PUT')}}
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <select id="heard" class="form-control"  name="cash_type_id"
                                                required="required">
                                            @foreach ($cashTypes as $cashType)
                                                <option value="{{ $cashType->id }}">{{ $cashType->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
